...
void test_simple_file() {
  ...
  const auto& parsed_file = std::get<cornell::nix::nar_node_t::file_t>(parsed.root.data);
...
void test_symlink() {
  ...
  const auto& parsed_link = std::get<cornell::nix::nar_node_t::symlink_t>(parsed.root.data);
...
void test_simple_directory() {
  ...
  entry.node = std::make_unique<cornell::nix::nar_node_t>();
  cornell::nix::nar_node_t::file_t file;
  file.executable = false;
  ...
  const auto& parsed_dir = std::get<cornell::nix::nar_node_t::dir_t>(parsed.root.data);
