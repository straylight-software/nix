...
│   ├── expression.h        # Core expression types (variant-based)
│   └── symbol_table.h      # String interning for identifiers
├── parse/                  # Parsing pipeline
│   ├── grammar.h           # PEGTL grammar rules (from Lix, LGPL-2.1)
│   ├── selector.h          # Parse tree node selection
│   ├── tree.h              # Intermediate tree types
│   ├── convert.h           # PEGTL tree → our tree
│   ├── lower.h             # Our tree → AST
│   ├── parser.h            # Main parse() entry point
│   ├── actions.h           # Alternative PEGTL actions (direct AST build)
│   └── state.h             # Parser state utilities
├── compile/                # WASM compilation
│   ├── compiler.h          # AST → WASM via Binaryen (~2700 lines)
│   └── wasm_types.h        # WASM value representation
├── eval/                   # Tree-walking interpreter (reference impl)
│   ├── eval.h              # Evaluator (~1500 lines)
│   └── value.h             # Runtime value types
├── runtime/                # WASM execution runtime
│   ├── memory_layout.h     # Memory layout constants (shared w/ compiler)
│   ├── runtime.h           # Runtime context and value operations
│   ├── runtime.cpp         # Runtime function implementations
│   ├── wasm_executor.h     # Wasmtime-based WASM executor
...
Source Text
    ...
    ▼
┌─────────────────────┐
│   PEGTL parse       │   grammar.h + selector.h
│   (parse_tree)      │
└─────────────────────┘
    │
    ▼
...
Source Text
    ...
    ▼
┌─────────────────────┐
│   PEGTL parse       │   grammar.h + actions.h
...
### `wasm_types.h` - Value Representation
...
### `runtime.h` / `runtime.cpp` - Runtime Functions
...
cxx_library(
    name = "ast",
    exported_headers = ["ast/expression.h", "ast/symbol_table.h"],
    # header-only, no deps
)
...
cxx_library(
    name = "eval",
    exported_headers = ["eval/eval.h", "eval/value.h"],
    deps = [":ast"],
)
...
cxx_library(
    ...
    exported_headers = ["runtime/memory_layout.h", "runtime/runtime.h"],
    deps = [":compile"],
)
...
cxx_library(
    ...
    exported_headers = ["runtime/wasm_executor.h"],
...
- Namespaces: `straylight::language::{ast,parse,compile,eval,runtime,memory_layout}`
