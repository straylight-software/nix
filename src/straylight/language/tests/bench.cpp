...
using namespace straylight::language::runtime;
using namespace straylight::language::compile;
namespace mem = straylight::language::memory_layout;
...
void bench_memory_layout_align() {
  ankerl::nanobench::Bench().run("runtime: align_up", [&] {
    for (std::uint32_t idx = 0; idx < 1000; ++idx) {
      auto aligned = mem::align_up(idx);
...
void bench_symbol_intern() {
  ankerl::nanobench::Bench().run("ast: symbol intern (new)", [&] {
    ast::symbol_table symbols;
    for (int idx = 0; idx < 100; ++idx) {
      auto sym = symbols.intern("variable" + std::to_string(idx));
...
void bench_symbol_intern_existing() {
  ...
  for (int idx = 0; idx < 100; ++idx) {
    symbols.intern("variable" + std::to_string(idx));
  }
...
void bench_symbol_lookup() {
  ...
  for (int idx = 0; idx < 100; ++idx) {
    syms.push_back(symbols.intern("variable" + std::to_string(idx)));
  }
