...
#include <straylight/nix/url/config.h>
...
TEST_CASE("parse url with fragment", "[url][parse]") {
  auto result = url::parse("https://example.com/page#section1");
  REQUIRE(result.has_value());
...
TEST_CASE("parse url with userinfo", "[url][parse]") {
  auto result = url::parse("https://user@example.com/path");
  REQUIRE(result.has_value());
...
TEST_CASE("parse url with userinfo and password", "[url][parse]") {
  auto result = url::parse("https://user:pass@example.com/path");
  REQUIRE(result.has_value());
...
TEST_CASE("parse file url", "[url][parse]") {
  auto result = url::parse("file:///home/user/file.txt");
  REQUIRE(result.has_value());
...
TEST_CASE("parse ipv4 url", "[url][parse]") {
  auto result = url::parse("http://192.168.1.1:8080/api");
  REQUIRE(result.has_value());

  auto& u = *result;
  REQUIRE(u.auth.has_value());
  REQUIRE(u.auth->host == "192.168.1.1");
  REQUIRE(u.auth->type == url::host_type::ipv4);
  REQUIRE(u.auth->port == 8080);
}

TEST_CASE("parse ipv6 url", "[url][parse]") {
  auto result = url::parse("http://[::1]:8080/api");
  REQUIRE(result.has_value());

  auto& u = *result;
  REQUIRE(u.auth.has_value());
  REQUIRE(u.auth->type == url::host_type::ipv6);
  REQUIRE(u.auth->port == 8080);
}

TEST_CASE("can_parse validates urls", "[url][parse]") {
  REQUIRE(url::can_parse("https://example.com"));
  REQUIRE(url::can_parse("http://localhost:8080"));
  REQUIRE(url::can_parse("file:///path/to/file"));

  // Invalid URLs
  REQUIRE_FALSE(url::can_parse(""));
  REQUIRE_FALSE(url::can_parse("not a url"));
  REQUIRE_FALSE(url::can_parse("://missing-scheme"));
}
...
TEST_CASE("url to_string roundtrip", "[url][serialize]") {
  ...
  auto result = url::parse(original);
  REQUIRE(result.has_value());

  auto serialized = result->to_string();

  // Parse again to verify
  auto reparsed = url::parse(serialized);
...
TEST_CASE("is_valid_scheme", "[url][scheme]") {
  REQUIRE(url::is_valid_scheme("http"));
  REQUIRE(url::is_valid_scheme("https"));
  REQUIRE(url::is_valid_scheme("git+ssh"));
  REQUIRE(url::is_valid_scheme("a1"));
  REQUIRE(url::is_valid_scheme("a-b.c+d"));

  REQUIRE_FALSE(url::is_valid_scheme(""));
  REQUIRE_FALSE(url::is_valid_scheme("1http")); // Must start with letter
  REQUIRE_FALSE(url::is_valid_scheme("+http"));
  REQUIRE_FALSE(url::is_valid_scheme("http:"));
}

TEST_CASE("parse_scheme simple", "[url][scheme]") {
  auto parts = url::parse_scheme("https");
  REQUIRE_FALSE(parts.application.has_value());
  REQUIRE(parts.transport == "https");
}

TEST_CASE("parse_scheme compound", "[url][scheme]") {
  auto parts = url::parse_scheme("git+https");
...
TEST_CASE("is_special_scheme", "[url][scheme]") {
  REQUIRE(url::is_special_scheme("http"));
  REQUIRE(url::is_special_scheme("https"));
  REQUIRE(url::is_special_scheme("ws"));
  REQUIRE(url::is_special_scheme("wss"));
  REQUIRE(url::is_special_scheme("ftp"));
  REQUIRE(url::is_special_scheme("file"));

  REQUIRE_FALSE(url::is_special_scheme("git"));
  REQUIRE_FALSE(url::is_special_scheme("ssh"));
  REQUIRE_FALSE(url::is_special_scheme("mailto"));
}

TEST_CASE("default_port", "[url][scheme]") {
  REQUIRE(url::default_port("http") == 80);
  REQUIRE(url::default_port("https") == 443);
  REQUIRE(url::default_port("ws") == 80);
  REQUIRE(url::default_port("wss") == 443);
  REQUIRE(url::default_port("ftp") == 21);
  REQUIRE(url::default_port("file") == 0);
  REQUIRE(url::default_port("git") == 0);
}
...
TEST_CASE("parse_query basic", "[url][query]") {
  auto params = url::parse_query("key=value&foo=bar");
...
TEST_CASE("parse_query percent encoded", "[url][query]") {
  auto params = url::parse_query("key%20name=value%26data");
...
TEST_CASE("parse_query empty value", "[url][query]") {
  auto params = url::parse_query("key=");
  REQUIRE(params.size() == 1);
...
TEST_CASE("encode_query basic", "[url][query]") {
  url::query_params params = {{"key", "value"}, {"foo", "bar"}};
  auto encoded = url::encode_query(params);
...
TEST_CASE("normalize_git_url scp style", "[url][git]") {
  auto result = url::normalize_git_url("git@github.com:user/repo.git");
  REQUIRE(result == "ssh://git@github.com/user/repo.git");
}

TEST_CASE("normalize_git_url already normalized", "[url][git]") {
  auto result = url::normalize_git_url("https://github.com/user/repo.git");
  REQUIRE(result == "https://github.com/user/repo.git");
}

TEST_CASE("normalize_git_url ssh url", "[url][git]") {
  auto result = url::normalize_git_url("ssh://git@github.com/user/repo.git");
  REQUIRE(result == "ssh://git@github.com/user/repo.git");
}
...
TEST_CASE("url canonicalize removes dot segments", "[url][canonicalize]") {
  auto result = url::parse("https://example.com/a/b/../c/./d");
  REQUIRE(result.has_value());
...
TEST_CASE("percent encoding property tests", "[url][property][encoding]") {
  rc::prop("percent encode/decode roundtrip", []() {
    ...
    auto encoded = url::percent_encode(input);
    auto decoded = url::percent_decode(encoded);
  ...
  rc::prop("double encoding is not idempotent", []() {
    ...
    auto once = url::percent_encode(input);
    auto twice = url::percent_encode(once);
...
TEST_CASE("query string property tests", "[url][property][query]") {
  rc::prop("query encode/decode roundtrip with alphanumeric", []() {
    ...
    url::query_params original = {{key, value}};

    auto encoded = url::encode_query(original);
    auto decoded = url::parse_query(encoded);
...
TEST_CASE("scheme validation property tests", "[url][property][scheme]") {
  rc::prop("valid scheme names start with letter", []() {
    ...
    RC_ASSERT(url::is_valid_scheme(scheme));
  });
  ...
  rc::prop("invalid scheme names start with non-letter", []() {
    ...
    RC_ASSERT_FALSE(url::is_valid_scheme(scheme));
  });
  ...
  rc::prop("parsed scheme transport is suffix after +", []() {
    ...
    auto parts = url::parse_scheme(compound);
...
TEST_CASE("url parser fuzz tests", "[url][fuzz]") {
  ...
  rc::prop("can_parse never crashes on garbage", []() {
    ...
    [[maybe_unused]] bool valid = url::can_parse(garbage);
  });
  ...
  rc::prop("percent_decode never crashes on garbage", []() {
    ...
    // Should not crash
    auto decoded = url::percent_decode(garbage);
...
TEST_CASE("zone_id extraction", "[url][ipv6][zone]") {
  auto extracted = url::detail::extract_zone_id("http://[fe80::1%25eth0]:8080/path");
...
TEST_CASE("zone_id extraction no zone", "[url][ipv6][zone]") {
  auto extracted = url::detail::extract_zone_id("http://[::1]:8080/path");
...
TEST_CASE("zone_id restoration", "[url][ipv6][zone]") {
  ...
  url::detail::restore_zone_id(auth, "eth0");
