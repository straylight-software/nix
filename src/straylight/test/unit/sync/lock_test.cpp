...
namespace locks = straylight::nix::sync;
...
TEST(acquire_and_release) {
  ...
  {
    auto lock = locks::exclusive_lock::acquire(lock_path);
...
TEST(try_acquire_succeeds_when_free) {
  ...
  auto lock = locks::exclusive_lock::try_acquire(lock_path);
  ASSERT(lock.has_value());
  ASSERT(lock->is_held());
}
...
TEST(try_acquire_fails_when_held) {
  ...
  auto lock1 = locks::exclusive_lock::acquire(lock_path);
  ASSERT(lock1.has_value());

  // Second try_acquire should fail immediately
  auto lock2 = locks::exclusive_lock::try_acquire(lock_path);
  ASSERT(!lock2.has_value());
  ASSERT_EQ(lock2.error(), locks::lock_error::lock_failed);
}
...
TEST(move_semantics) {
  ...
  // Move construct
  locks::exclusive_lock lock2 = std::move(*lock1);
  ASSERT(lock2.is_held());
  ASSERT(!lock1->is_held());

  // Move assign
  locks::exclusive_lock lock3 = locks::exclusive_lock::try_acquire(test_dir / "dummy.lock").value();
...
TEST(shared_lock_multiple_readers) {
  ...
  auto r1 = locks::shared_lock::acquire(lock_path);
  ASSERT(r1.has_value());

  auto r2 = locks::shared_lock::acquire(lock_path);
  ASSERT(r2.has_value());

  auto r3 = locks::shared_lock::acquire(lock_path);
  ASSERT(r3.has_value());
}
...
TEST(shared_exclusive_conflict) {
  ...
  auto shared = locks::shared_lock::acquire(lock_path);
  ASSERT(shared.has_value());

  // Exclusive should fail while shared is held
  auto exclusive = locks::exclusive_lock::try_acquire(lock_path);
...
TEST(exclusive_shared_conflict) {
  ...
  auto exclusive = locks::exclusive_lock::acquire(lock_path);
  ASSERT(exclusive.has_value());

  // Shared should fail while exclusive is held
  auto shared = locks::shared_lock::try_acquire(lock_path);
  ASSERT(!shared.has_value());
}
...
TEST(lock_guard_raii) {
  ...
  {
    locks::lock_guard guard(lock_path);
  ...
  auto lock3 = locks::exclusive_lock::try_acquire(lock_path);
