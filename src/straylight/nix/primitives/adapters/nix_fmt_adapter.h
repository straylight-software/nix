// straylight::nix::primitives::adapters::nix_fmt_adapter
//
// Compatibility shim to migrate Nix codebase from boost::format to std::format.
//
// This adapter provides drop-in replacements for:
// - nix::fmt_()        -> Uses straylight::nix::primitives::fmt()
// - nix::hint_fmt_t    -> Uses straylight::nix::primitives::Hint
// - nix::magenta_t<T>  -> Uses straylight::nix::primitives::Magenta<T>
// - nix::uncolored_t<T> -> Uses straylight::nix::primitives::Uncolored<T>
//
// Migration strategy:
...
#include "straylight/nix/primitives/format.h"
...
inline constexpr std::string_view ANSI_NORMAL = straylight::nix::primitives::kAnsiNormal;
inline constexpr std::string_view ANSI_BOLD = straylight::nix::primitives::kAnsiBold;
inline constexpr std::string_view ANSI_FAINT = straylight::nix::primitives::kAnsiFaint;
inline constexpr std::string_view ANSI_ITALIC = "\033[3m";
inline constexpr std::string_view ANSI_RED = straylight::nix::primitives::kAnsiRed;
inline constexpr std::string_view ANSI_GREEN = straylight::nix::primitives::kAnsiGreen;
inline constexpr std::string_view ANSI_WARNING =
    straylight::nix::primitives::kAnsiMagenta; // Nix uses magenta for warnings
inline constexpr std::string_view ANSI_BLUE = straylight::nix::primitives::kAnsiBlue;
inline constexpr std::string_view ANSI_MAGENTA = straylight::nix::primitives::kAnsiMagenta;
inline constexpr std::string_view ANSI_CYAN = straylight::nix::primitives::kAnsiCyan;
...
template <typename T>
using magenta_t = straylight::nix::primitives::Magenta<T>;
...
template <typename T>
using uncolored_t = straylight::nix::primitives::Uncolored<T>;

/// Additional color wrapper using the generic Colored type
template <typename T>
using colored_t = straylight::nix::primitives::Colored<T>;
...
std::ostream& operator<<(std::ostream& out, const magenta_t<T>& m) {
  return out << straylight::nix::primitives::kAnsiMagenta << m.value
             << straylight::nix::primitives::kAnsiNormal;
}

template <typename T>
std::ostream& operator<<(std::ostream& out, const uncolored_t<T>& u) {
  return out << straylight::nix::primitives::kAnsiNormal << u.value;
}

template <typename T>
std::ostream& operator<<(std::ostream& out, const colored_t<T>& c) {
  return out << c.color << c.value << straylight::nix::primitives::kAnsiNormal;
}
...
std::string fmt_(const std::string& format_str, const Args&... args) {
  return straylight::nix::primitives::fmt(format_str, args...);
}
...
public:
  ...
  hint_fmt_t& operator%(const T& value) {
    if (finalized_) {
      ...
      oss << straylight::nix::primitives::kAnsiMagenta << value
          << straylight::nix::primitives::kAnsiNormal;
      str_ += oss.str();
    } else {
...
private:
  ...
  static std::string format_with_magenta(const std::string& format_str, const Args&... args) {
    ...
    return straylight::nix::primitives::fmt(format_str, wrap_in_magenta(args)...);
  }
...
using straylight::nix::primitives::format;

/// Modern hint function (for gradual migration).
using straylight::nix::primitives::hint;
using straylight::nix::primitives::Hint;

// Convenience color wrapper factories (matching new style)
using straylight::nix::primitives::Colored;
using straylight::nix::primitives::Magenta;
using straylight::nix::primitives::Uncolored;
