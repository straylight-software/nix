// straylight::nix::primitives::adapters::nix_strings_adapter
//
// Compatibility adapter that allows Nix code to use straylight::nix::primitives
...
// 2. Use `using namespace straylight::nix::primitives::nix_compat;` in .cpp files
...
namespace straylight::nix::primitives::nix_compat {
...
[[nodiscard]] inline std::string chomp(std::string_view s) {
  auto trimmed = trim_right(s, nix_whitespace);
  return std::string{trimmed};
}
...
[[nodiscard]] C tokenizeString(std::string_view s,
  ...
  auto tokens = tokenize(s, separators);
...
[[nodiscard]] C splitString(std::string_view s, std::string_view separators) {
  ...
  while (pos <= s.size()) {
    auto end = find_first_of(s.substr(pos), separators);
...
using primitives::contains;

/// Find substring position
using primitives::find;
using primitives::rfind;

/// Character set search
using primitives::find_first_not_of;
using primitives::find_first_of;
using primitives::find_last_not_of;
using primitives::find_last_of;
...
inline std::string_view trim_view(std::string_view s,
                                  std::string_view whitespace = nix_whitespace) {
  return primitives::trim(s, whitespace);
}

inline std::string_view trim_left_view(std::string_view s,
                                       std::string_view whitespace = nix_whitespace) {
  return trim_left(s, whitespace);
}

inline std::string_view trim_right_view(std::string_view s,
                                        std::string_view whitespace = nix_whitespace) {
  return trim_right(s, whitespace);
}

/// Zero-copy split (returns views, not in Nix API)
/// Use this for better performance when you don't need owned strings
using primitives::split_to_views;

/// Replace first occurrence only (not in Nix API)
using primitives::replace_first;
...
namespace strings = primitives::nix_compat;
}
