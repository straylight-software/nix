...
inline auto deserialize_path_info(std::span<const std::byte> data) -> store_result<path_info> {
  try {
    return primitives::deserialize<path_info>(data);
...
template <>
constexpr auto serialize(auto& archive, const straylight::nix::primitives::path_info& info) {
  return archive(info.path, info.nar_hash, info.registration_time, info.deriver, info.nar_size,
                 info.ultimate, info.sigs, info.ca);
}

template <>
constexpr auto serialize(auto& archive, straylight::nix::primitives::path_info& info) {
  return archive(info.path, info.nar_hash, info.registration_time, info.deriver, info.nar_size,
                 info.ultimate, info.sigs, info.ca);
}
...
constexpr auto serialize(auto& archive,
                         const straylight::nix::primitives::register_path_payload& payload) {
  return archive(payload.info, payload.references);
}

template <>
constexpr auto serialize(auto& archive,
                         straylight::nix::primitives::register_path_payload& payload) {
  return archive(payload.info, payload.references);
}
...
template <>
constexpr auto serialize(auto& archive,
                         const straylight::nix::primitives::invalidate_path_payload& payload) {
  return archive(payload.path);
}

template <>
constexpr auto serialize(auto& archive,
                         straylight::nix::primitives::invalidate_path_payload& payload) {
  return archive(payload.path);
}
...
template <>
constexpr auto serialize(auto& archive,
                         const straylight::nix::primitives::add_derivation_payload& payload) {
  return archive(payload.drv_path, payload.output_name, payload.output_path);
}

template <>
constexpr auto serialize(auto& archive,
                         straylight::nix::primitives::add_derivation_payload& payload) {
  return archive(payload.drv_path, payload.output_name, payload.output_path);
}

// log_entry
template <>
constexpr auto serialize(auto& archive, const straylight::nix::primitives::log_entry& entry) {
  return archive(entry.op, entry.sequence, entry.timestamp, entry.data);
}

template <>
constexpr auto serialize(auto& archive, straylight::nix::primitives::log_entry& entry) {
  return archive(entry.op, entry.sequence, entry.timestamp, entry.data);
}
