...
url_result parse(std::string_view url_str) {
  ...
  auto parsed = primitives::parse(url_str);
  if (!parsed) {
    return result;
  }
  ...
  if (!parsed->query.empty()) {
    result.query = primitives::encode_query(parsed->query);
  }
...
bool can_parse(std::string_view url) noexcept {
  return primitives::can_parse(url);
}
...
url_result parse(std::string_view url_str) {
  ...
  auto parsed = primitives::url_view::parse(url_str);
  if (!parsed) {
    return result;
  }
...
bool can_parse(std::string_view url) noexcept {
  return primitives::url_view::can_parse(url);
}
