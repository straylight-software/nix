...
auto legacy_store::ensure_db() -> legacy_result<void> {
  ...
  auto result = Database::open(db_path(), OpenMode::read_write_create);
  if (!result) {
    return std::unexpected(legacy_error::database_error);
  }

  db_ = std::make_unique<Database>(std::move(*result));
...
auto legacy_store::acquire_write_lock() -> legacy_result<exclusive_lock> {
