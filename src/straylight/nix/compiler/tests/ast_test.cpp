...
using namespace straylight::nix::compiler::ast;
...
TEST_CASE("expression_binary_operation construction", "[ast][expr]") {
  ...
  REQUIRE(binop.op_ == binary_operator::add);
}
...
TEST_CASE("expression_unary_operation construction", "[ast][expr]") {
  ...
  REQUIRE(unop.op_ == unary_operator::negate);
}
...
TEST_CASE("get_position extracts position from any expression", "[ast][expr]") {
  ...
  REQUIRE(get_position(int_expr).byte_offset_ == 10);
  REQUIRE(get_position(str_expr).byte_offset_ == 20);
}
...
TEST_CASE("attribute_path with static segments", "[ast][attr]") {
  ...
  attribute_path path;
  path.segments_.push_back(attribute_name{{0, 0, 0}, symbols.intern("foo")});
  path.segments_.push_back(attribute_name{{0, 0, 0}, symbols.intern("bar")});
  path.segments_.push_back(attribute_name{{0, 0, 0}, symbols.intern("baz")});
...
TEST_CASE("attribute_path with dynamic segment", "[ast][attr]") {
  ...
  attribute_path path;
  path.segments_.push_back(attribute_name{pos, std::move(dynamic_expr)});
...
TEST_CASE("binding_attribute construction", "[ast][binding]") {
  ...
  attribute_path path;
  path.segments_.push_back(attribute_name{pos, symbols.intern("x")});
...
TEST_CASE("expression_select construction", "[ast][expr]") {
  ...
  attribute_path path;
  path.segments_.push_back(attribute_name{pos, symbols.intern("foo")});
