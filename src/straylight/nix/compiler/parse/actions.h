#pragma once
///@file straylight/nix/compiler/parse/actions.h
...
// TODO[b7r6]: consider absl::InlinedVector or folly::small_vector if perf regresses
...
#include "straylight/nix/compiler/parse/state.h"
...
struct expression_state {
  // expression stack
  // TODO[b7r6]: consider absl::InlinedVector<..., 4> if perf regresses
  std::vector<std::pair<ast::source_position, ast::expression>> expressions;

  // operator stack for precedence parsing
  // TODO[b7r6]: consider absl::InlinedVector<..., 2> if perf regresses
  std::vector<operator_entry> operators;
...
struct attr_path_state : subexpression_state {
  ...
  void push_static(ast::source_position position, ast::symbol sym) {
    path.segments_.emplace_back(ast::attribute_name{position, sym});
  }

  void push_dynamic(ast::source_position position, ast::expression expr) {
    path.segments_.emplace_back(ast::attribute_name{position, std::move(expr)});
  }
};
