...
| Directory | Namespace | Description | |-----------|-----------|-------------| | `ast/` |
`compiler::ast` | Abstract syntax tree types | | `parse/` | `compiler::parse` | Tree-sitter based
parser | | `compile/` | `compiler::compile` | AST to WASM compilation | | `runtime/` |
`compiler::runtime` | WASM execution engine | | `eval/` | `compiler::eval` | Evaluation utilities |
...
| Type | Description | |------|-------------| | `evaluator` | Top-level Nix evaluator | |
`eval_string()` | Evaluate Nix expression string | | `eval_file()` | Evaluate Nix file | |
`eval_result<T>` | Result type with error info | | `eval_error` | Error with location info |
