...
auto rt_make_closure(runtime_context& ctx, std::uint32_t func_index, std::uint32_t env_offset,
  ...
  // Encode module_id in the upper 16 bits of func_index for cross-module lambda support.
...
auto rt_make_thunk(runtime_context& ctx, std::uint32_t func_index, std::uint32_t env_offset,
  ...
  // Encode module_id in the upper 16 bits of func_index for cross-module thunk support.
  // Format: (module_id << 16) | local_func_index
  auto encoded_func_index =
      (static_cast<std::uint32_t>(ctx.current_module_id) << 16) | (func_index & 0xFFFF);
...
auto rt_make_list(runtime_context& ctx, std::uint32_t offset, std::uint32_t count) -> nix_value {
  ...
  for (std::uint32_t idx = 0; idx < count; ++idx) {
    auto elem = ctx.read_value(offset + idx * mem::VALUE_SIZE);
...
auto rt_make_attrs(runtime_context& ctx, std::uint32_t offset, std::uint32_t count) -> nix_value {
  ...
  for (std::uint32_t idx = 0; idx < count; ++idx) {
    auto key_offset = ctx.read_u32(offset + idx * mem::ATTRSET_ENTRY_SIZE);
    auto value =
        ctx.read_value(offset + idx * mem::ATTRSET_ENTRY_SIZE + mem::ATTRSET_ENTRY_VALUE_OFFSET);

    auto entry_offset = attrs_ptr + mem::ATTRSET_ENTRIES_OFFSET + idx * mem::ATTRSET_ENTRY_SIZE;
...
auto rt_make_attrs_dynamic(runtime_context& ctx, std::uint32_t offset, std::uint32_t count)
  ...
  for (std::uint32_t idx = 0; idx < count; ++idx) {
    auto key_value = ctx.read_value(offset + idx * mem::ATTRSET_DYNAMIC_ENTRY_SIZE);
...
auto rt_update(runtime_context& ctx, nix_value a, nix_value b) -> nix_value {
  ...
  for (std::uint32_t idx = 0; idx < b_count; ++idx) {
    auto entry = b_ptr + mem::ATTRSET_ENTRIES_OFFSET + idx * mem::ATTRSET_ENTRY_SIZE;
  ...
  for (std::uint32_t idx = 0; idx < a_count; ++idx) {
    auto entry = a_ptr + mem::ATTRSET_ENTRIES_OFFSET + idx * mem::ATTRSET_ENTRY_SIZE;
    auto key_off = ctx.read_u32(entry + mem::ATTRSET_ENTRY_KEY_OFFSET);
    auto key = ctx.read_string(key_off);
    // Check if key exists in b
    bool in_b = false;
    for (std::uint32_t jdx = 0; jdx < b_count; ++jdx) {
      auto b_entry = b_ptr + mem::ATTRSET_ENTRIES_OFFSET + jdx * mem::ATTRSET_ENTRY_SIZE;
  ...
  for (std::uint32_t idx = 0; idx < count; ++idx) {
    auto entry_off = attrs_ptr + mem::ATTRSET_ENTRIES_OFFSET + idx * mem::ATTRSET_ENTRY_SIZE;
    ctx.write_i32(entry_off + mem::ATTRSET_ENTRY_KEY_OFFSET,
                  static_cast<std::int32_t>(entries[idx].first));
    ctx.write_value(entry_off + mem::ATTRSET_ENTRY_VALUE_OFFSET, entries[idx].second);
  }
...
auto rt_concat(runtime_context& ctx, nix_value a, nix_value b) -> nix_value {
  ...
  for (std::uint32_t idx = 0; idx < a_count; ++idx) {
    auto elem = ctx.read_value(a_ptr + mem::LIST_ELEMENTS_OFFSET + idx * mem::VALUE_SIZE);
    ctx.write_value(list_ptr + mem::LIST_ELEMENTS_OFFSET + idx * mem::VALUE_SIZE, elem);
  }

  // Copy elements from b
  for (std::uint32_t idx = 0; idx < b_count; ++idx) {
    auto elem = ctx.read_value(b_ptr + mem::LIST_ELEMENTS_OFFSET + idx * mem::VALUE_SIZE);
    ctx.write_value(list_ptr + mem::LIST_ELEMENTS_OFFSET + (a_count + idx) * mem::VALUE_SIZE, elem);
  }
...
auto rt_tail(runtime_context& ctx, nix_value v) -> nix_value {
  ...
  for (std::uint32_t idx = 0; idx < new_count; ++idx) {
    auto elem = ctx.read_value(ptr + mem::LIST_ELEMENTS_OFFSET + (idx + 1) * mem::VALUE_SIZE);
    ctx.write_value(list_ptr + mem::LIST_ELEMENTS_OFFSET + idx * mem::VALUE_SIZE, elem);
  }
...
auto rt_attr_names(runtime_context& ctx, nix_value set) -> nix_value {
  ...
  for (std::uint32_t idx = 0; idx < count; ++idx) {
    auto str_ptr = allocate_string(ctx, names[idx]);
    auto str_val = make_value(value_tag::string, str_ptr);
    ctx.write_value(list_ptr + mem::LIST_ELEMENTS_OFFSET + idx * mem::VALUE_SIZE, str_val);
  }
...
auto rt_attr_values(runtime_context& ctx, nix_value set) -> nix_value {
  ...
  for (std::uint32_t idx = 0; idx < count; ++idx) {
    ctx.write_value(list_ptr + mem::LIST_ELEMENTS_OFFSET + idx * mem::VALUE_SIZE,
                    entries[idx].second);
  }
...
auto rt_replace_strings(runtime_context& ctx, nix_value from, nix_value to, nix_value str)
  ...
  while (pos < input.size()) {
    bool matched = false;
    for (std::size_t idx = 0; idx < from_strs.size(); ++idx) {
      const auto& f = from_strs[idx];
      if (f.empty()) {
  ...
  for (std::size_t idx = 0; idx < from_strs.size(); ++idx) {
    if (from_strs[idx].empty() && pos == input.size()) {
      result += to_strs[idx];
      break;
    }
  }
...
auto rt_list_to_attrs(runtime_context& ctx, nix_value list) -> nix_value {
  ...
  for (std::uint32_t idx = 0; idx < new_count; ++idx) {
    auto& [key, value] = entries[idx];
    auto key_ptr = allocate_string(ctx, key);
    auto entry = new_ptr + mem::ATTRSET_ENTRIES_OFFSET + idx * mem::ATTRSET_ENTRY_SIZE;
...
auto rt_cat_attrs(runtime_context& ctx, nix_value name, nix_value list) -> nix_value {
  ...
  for (std::uint32_t idx = 0; idx < result_count; ++idx) {
    ctx.write_value(result_ptr + mem::LIST_ELEMENTS_OFFSET + idx * mem::VALUE_SIZE, values[idx]);
  }
...
auto rt_partition(runtime_context& ctx, nix_value pred, nix_value list) -> nix_value {
  ...
  auto make_list = [&ctx](const std::vector<nix_value>& elems) -> nix_value {
    ...
    ctx.write_i32(list_ptr + mem::LIST_COUNT_OFFSET, static_cast<std::int32_t>(count));
    for (std::uint32_t idx = 0; idx < count; ++idx) {
      ctx.write_value(list_ptr + mem::LIST_ELEMENTS_OFFSET + idx * mem::VALUE_SIZE, elems[idx]);
    }
    return make_value(value_tag::list, list_ptr);
  };
...
auto rt_group_by(runtime_context& ctx, nix_value f, nix_value list) -> nix_value {
  ...
  for (auto& [key, elems] : groups) {
    ...
    for (std::uint32_t jdx = 0; jdx < elem_count; ++jdx) {
      ctx.write_value(elem_list_ptr + mem::LIST_ELEMENTS_OFFSET + jdx * mem::VALUE_SIZE,
                      elems[jdx]);
    }
...
auto rt_reverse(runtime_context& ctx, nix_value list) -> nix_value {
  ...
  for (std::uint32_t idx = 0; idx < count; ++idx) {
    auto elem = ctx.read_value(list_ptr + mem::LIST_ELEMENTS_OFFSET + idx * mem::VALUE_SIZE);
    ctx.write_value(result_ptr + mem::LIST_ELEMENTS_OFFSET + (count - 1 - idx) * mem::VALUE_SIZE,
                    elem);
  }
...
auto rt_take(runtime_context& ctx, nix_value n, nix_value list) -> nix_value {
  ...
  for (std::uint32_t idx = 0; idx < actual_count; ++idx) {
    auto elem = ctx.read_value(list_ptr + mem::LIST_ELEMENTS_OFFSET + idx * mem::VALUE_SIZE);
    ctx.write_value(result_ptr + mem::LIST_ELEMENTS_OFFSET + idx * mem::VALUE_SIZE, elem);
  }
...
auto rt_drop(runtime_context& ctx, nix_value n, nix_value list) -> nix_value {
  ...
  for (std::uint32_t idx = 0; idx < remaining; ++idx) {
    auto elem = ctx.read_value(list_ptr + mem::LIST_ELEMENTS_OFFSET +
                               (idx + static_cast<std::uint32_t>(drop_count)) * mem::VALUE_SIZE);
    ctx.write_value(result_ptr + mem::LIST_ELEMENTS_OFFSET + idx * mem::VALUE_SIZE, elem);
  }
...
auto rt_range(runtime_context& ctx, nix_value a, nix_value b) -> nix_value {
  ...
  for (std::uint32_t idx = 0; idx < count; ++idx) {
    auto val = make_value(value_tag::integer,
                          static_cast<std::uint32_t>(start + static_cast<std::int32_t>(idx)));
    ctx.write_value(result_ptr + mem::LIST_ELEMENTS_OFFSET + idx * mem::VALUE_SIZE, val);
  }
...
auto rt_zip_lists(runtime_context& ctx, nix_value list1, nix_value list2) -> nix_value {
  ...
  for (std::uint32_t idx = 0; idx < min_count; ++idx) {
    auto elem1 = ctx.read_value(ptr1 + mem::LIST_ELEMENTS_OFFSET + idx * mem::VALUE_SIZE);
    auto elem2 = ctx.read_value(ptr2 + mem::LIST_ELEMENTS_OFFSET + idx * mem::VALUE_SIZE);
    ...
    ctx.write_value(result_ptr + mem::LIST_ELEMENTS_OFFSET + idx * mem::VALUE_SIZE,
...
auto rt_compare_versions(runtime_context& ctx, nix_value a, nix_value b) -> nix_value {
  ...
  for (std::size_t idx = 0; idx < max_len; ++idx) {
    auto pa = idx < parts_a.size() ? parts_a[idx] : "";
    auto pb = idx < parts_b.size() ? parts_b[idx] : "";
...
auto rt_split_version(runtime_context& ctx, nix_value v) -> nix_value {
  ...
  for (std::uint32_t idx = 0; idx < count; ++idx) {
    auto str_ptr = allocate_string(ctx, parts[idx]);
    ctx.write_value(list_ptr + mem::LIST_ELEMENTS_OFFSET + idx * mem::VALUE_SIZE,
...
private:
  ...
  auto parse_array() -> nix_value {
    ...
    for (std::uint32_t idx = 0; idx < count; ++idx) {
      ctx_.write_value(list_ptr + mem::LIST_ELEMENTS_OFFSET + idx * mem::VALUE_SIZE, elements[idx]);
    }
...
auto rt_filter(runtime_context& ctx, nix_value pred, nix_value list) -> nix_value {
  ...
  for (std::uint32_t idx = 0; idx < result_count; ++idx) {
    ctx.write_value(result_ptr + mem::LIST_ELEMENTS_OFFSET + idx * mem::VALUE_SIZE, kept[idx]);
  }
...
auto rt_sort(runtime_context& ctx, nix_value comparator, nix_value list) -> nix_value {
  ...
  for (std::uint32_t idx = 0; idx < count; ++idx) {
    ctx.write_value(result_ptr + mem::LIST_ELEMENTS_OFFSET + idx * mem::VALUE_SIZE, elements[idx]);
  }
...
auto rt_generic_closure(runtime_context& ctx, nix_value attrs) -> nix_value {
  ...
  // Special case: empty list has payload 0
  if (start_ptr != 0) {
    auto start_count = ctx.read_u32(start_ptr + mem::LIST_COUNT_OFFSET);
    for (std::uint32_t idx = 0; idx < start_count; ++idx) {
      auto elem = ctx.read_value(start_ptr + mem::LIST_ELEMENTS_OFFSET + idx * mem::VALUE_SIZE);
  ...
  while (!work_list.empty()) {
    ...
    if (new_ptr != 0) {
      ...
      for (std::uint32_t idx = 0; idx < new_count; ++idx) {
        auto new_elem = ctx.read_value(new_ptr + mem::LIST_ELEMENTS_OFFSET + idx * mem::VALUE_SIZE);
  ...
  for (std::uint32_t idx = 0; idx < count; ++idx) {
    ctx.write_value(result_ptr + 4 + idx * 8, result[idx]);
  }
...
auto rt_match(runtime_context& ctx, nix_value regex, nix_value str) -> nix_value {
  ...
  try {
    ...
    for (std::uint32_t idx = 0; idx < count; ++idx) {
      auto& submatch = match_result[idx + 1];
      if (submatch.matched) {
        auto str_ptr = allocate_string(ctx, submatch.str());
        ctx.write_value(result_ptr + 4 + idx * 8, make_value(value_tag::string, str_ptr));
      } else {
        // Unmatched optional group returns null
        ctx.write_value(result_ptr + 4 + idx * 8, constants::null_value);
      }
    }
  ...
  try {
    ...
    for (; it != end; ++it) {
      ...
      if (group_count == 0) {
        ...
        for (std::uint32_t idx = 0; idx < group_count; ++idx) {
          auto& submatch = match_result[idx + 1];
          if (submatch.matched) {
            auto str_ptr = allocate_string(ctx, submatch.str());
            ctx.write_value(list_ptr + 4 + idx * 8, make_value(value_tag::string, str_ptr));
          } else {
            ctx.write_value(list_ptr + 4 + idx * 8, constants::null_value);
          }
        }
        result_vec.push_back(make_value(value_tag::list, list_ptr));
      }
    ...
    ctx.write_i32(result_ptr, static_cast<std::int32_t>(count));
    for (std::uint32_t idx = 0; idx < count; ++idx) {
      ctx.write_value(result_ptr + 4 + idx * 8, result_vec[idx]);
    }
