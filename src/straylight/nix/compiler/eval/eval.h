...
#include "straylight/nix/compiler/eval/value.h"
...
public:
  ...
  void deep_force(value_ptr val) {
    ...
    if (is_list(val)) {
      ...
      for (const auto& [key, val_item] : as_attrs(val).attrs) {
        deep_force(val_item);
      }
    }
  }
    ...
    return std::visit(
        [this](const auto& x) -> std::string {
          ...
          if constexpr (std::is_same_v<T, value_null>) {
            ...
            std::string result = "{ ";
            for (const auto& [key, val] : x.attrs) {
              result += key + " = " + print_value(val) + "; ";
            }
            result += "}";
            return result;
...
private:
    ...
    builtins_map["genList"] =
        value::make_builtin("genList", 2, [this](std::vector<value_ptr>& args) {
          ...
          for (std::int64_t idx = 0; idx < n; ++idx) {
            result.push_back(apply(func, value::make_int(idx)));
          }
          return value::make_list(std::move(result));
        });
        ...
        value::make_builtin("replaceStrings", 3, [this](std::vector<value_ptr>& args) {
          ...
          while (pos < input.size()) {
            ...
            for (std::size_t idx = 0; idx < from_strs.size(); ++idx) {
              ...
              if (input.compare(pos, from_strs[idx].size(), from_strs[idx]) == 0) {
                result += to_strs[idx];
                pos += from_strs[idx].size();
                replaced = true;
                break;
              }
            }
  ...
  auto eval_variant(const ast::expression_path& expr, env_ptr) -> value_ptr {
    return value::make_path(expr.value_);
  }
  ...
  auto eval_variant(const ast::expression_identifier& expr, env_ptr env) -> value_ptr {
    auto name = symbols_.lookup(expr.name_);
  ...
  auto eval_variant(const ast::expression_attribute_set& expr, env_ptr env) -> value_ptr {
    auto attrs_env = expr.is_recursive_ ? std::make_shared<environment>(env) : env;
    std::unordered_map<std::string, value_ptr> attrs;

    // First pass: create thunks for all bindings
    for (const auto& binding_var : expr.bindings_) {
      if (auto* b = std::get_if<ast::binding_attribute>(&binding_var)) {
        // Extract path segments as strings
        std::vector<std::string> path;
        for (const auto& seg : b->path_.segments_) {
          if (auto* sym = std::get_if<ast::symbol>(&seg.value_)) {
        ...
        if (!path.empty()) {
          auto thunk_env = expr.is_recursive_ ? attrs_env : env;
          auto thunk = value::make_thunk(b->value_, thunk_env);
  ...
  auto eval_variant(const ast::expression_select& expr, env_ptr env) -> value_ptr {
    auto subject = force(eval_expr(expr.subject_, env));

    for (const auto& seg : expr.path_.segments_) {
      if (!is_attrs(subject)) {
        if (expr.default_value_) {
          return eval_expr(*expr.default_value_, env);
        }
  ...
  auto eval_variant(const ast::expression_application& expr, env_ptr env) -> value_ptr {
    auto func = eval_expr(expr.function_, env);
  ...
  auto apply(value_ptr func, value_ptr arg) -> value_ptr {
    ...
    if (is_closure(func)) {
      ...
      auto& pattern = *c.lambda->argument_pattern_;
      if (auto* simple = std::get_if<ast::pattern_simple>(&pattern)) {
        ...
        if (attrs_pat->argument_name_) {
          new_env->bind(symbols_.lookup(*attrs_pat->argument_name_), arg_val);
        }

        // Bind formals
        for (const auto& formal : attrs_pat->formals_) {
          std::string name(symbols_.lookup(formal.name_));
          auto it = as_attrs(arg_val).attrs.find(name);
          if (it != as_attrs(arg_val).attrs.end()) {
            new_env->bind(name, it->second);
          } else if (formal.default_value_) {
            new_env->bind(name, value::make_thunk(*formal.default_value_, new_env));
          } else {
      ...
      return eval_expr(c.lambda->body_, new_env);
    }
  ...
  auto eval_variant(const ast::expression_let& expr, env_ptr env) -> value_ptr {
    ...
    for (const auto& binding_var : expr.bindings_) {
      if (auto* b = std::get_if<ast::binding_attribute>(&binding_var)) {
        if (b->path_.segments_.size() == 1) {
          auto& seg = b->path_.segments_[0];
          if (auto* sym = std::get_if<ast::symbol>(&seg.value_)) {
            auto name = symbols_.lookup(*sym);
            let_env->bind(name, value::make_thunk(b->value_, let_env));
          }
        }
      } else if (auto* inh = std::get_if<ast::binding_inherit>(&binding_var)) {
        if (inh->from_expression_) {
    ...
    return eval_expr(expr.body_, let_env);
  }
  ...
  auto eval_variant(const ast::expression_if& expr, env_ptr env) -> value_ptr {
    ...
    if (as_bool(cond)) {
      ...
      return eval_expr(expr.else_branch_, env);
    }
  }
  ...
  auto eval_variant(const ast::expression_binary_operation& expr, env_ptr env) -> value_ptr {
    ...
    auto left = force(eval_expr(expr.left_, env));
    auto right = force(eval_expr(expr.right_, env));
    ...
    switch (expr.op_) {
      ...
      case ast::binary_operator::update:
        if (is_attrs(left) && is_attrs(right)) {
          ...
          for (const auto& [key, val] : as_attrs(right).attrs) {
            result[key] = val;
          }
          return value::make_attrs(std::move(result));
        }
  ...
  auto eval_variant(const ast::expression_unary_operation& expr, env_ptr env) -> value_ptr {
    auto operand = force(eval_expr(expr.operand_, env));
