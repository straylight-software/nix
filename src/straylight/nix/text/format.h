...
template <typename T>
struct std::formatter<straylight::nix::text::Magenta<T>> : std::formatter<T> {
  template <typename FormatContext>
  auto format(straylight::nix::text::Magenta<T> const& m, FormatContext& ctx) const {
    auto out = ctx.out();
    out = std::copy(straylight::nix::text::kAnsiMagenta.begin(),
                    straylight::nix::text::kAnsiMagenta.end(), out);
    ctx.advance_to(out);
    out = std::formatter<T>::format(m.value, ctx);
    out = std::copy(straylight::nix::text::kAnsiNormal.begin(),
                    straylight::nix::text::kAnsiNormal.end(), out);
    return out;
  }
};

template <typename T>
struct std::formatter<straylight::nix::text::Uncolored<T>> : std::formatter<T> {
  template <typename FormatContext>
  auto format(straylight::nix::text::Uncolored<T> const& u, FormatContext& ctx) const {
    auto out = ctx.out();
    out = std::copy(straylight::nix::text::kAnsiNormal.begin(),
                    straylight::nix::text::kAnsiNormal.end(), out);
    ctx.advance_to(out);
...
template <typename T>
struct std::formatter<straylight::nix::text::Colored<T>> : std::formatter<T> {
  template <typename FormatContext>
  auto format(straylight::nix::text::Colored<T> const& c, FormatContext& ctx) const {
    auto out = ctx.out();
