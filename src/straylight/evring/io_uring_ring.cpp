...
public:
  ...
  void enqueue(const operation& operation_to_enqueue) override {
    ...
    switch (operation_to_enqueue.type) {
      ...
      case operation_type::poll_add: {
        const auto& parameters = std::get<poll_add_parameters>(operation_to_enqueue.parameters);
        int file_descriptor = get_file_descriptor(operation_to_enqueue.resource_handle);
        io_uring_prep_poll_add(sqe, file_descriptor, parameters.poll_mask);
        break;
      }
    }
