...
TEST_CASE("adversarial: deep nesting", "[adversarial][nesting]") {
  ...
  SECTION("deeply nested let expressions") {
    std::string expr = "1";
    for (int i = 0; i < 50; ++i) {
      expr = "let x" + std::to_string(i) + " = " + std::to_string(i) + "; in " + expr;
    }
    auto result = eval_nix(expr);
    REQUIRE(result.success);
  }
...
TEST_CASE("adversarial: attrset edge cases", "[adversarial][attrset]") {
  ...
  SECTION("many attributes") {
    std::string attrs = "{";
    for (int i = 0; i < 100; ++i) {
      attrs += " attr" + std::to_string(i) + " = " + std::to_string(i) + ";";
    }
    attrs += " }";
...
TEST_CASE("adversarial: memory stress", "[adversarial][memory]") {
  SECTION("many strings") {
    std::string expr = "let ";
    for (int i = 0; i < 100; ++i) {
      expr += "s" + std::to_string(i) + " = \"string" + std::to_string(i) + "\"; ";
    }
    expr += "in s99";
    auto result = eval_nix(expr);
    REQUIRE(result.success);
  ...
  SECTION("many attrsets") {
    std::string expr = "let ";
    for (int i = 0; i < 50; ++i) {
      expr += "a" + std::to_string(i) + " = { x = " + std::to_string(i) + "; }; ";
    }
    expr += "in a49.x";
    auto result = eval_nix(expr);
    REQUIRE(result.success);
