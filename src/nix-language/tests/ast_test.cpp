...
using namespace nix::language::ast;
...
TEST_CASE("source_position basic construction", "[ast][position]") {
  source_position pos{100, 5, 10};

  REQUIRE(pos.byte_offset == 100);
  REQUIRE(pos.line == 5);
...
TEST_CASE("expression_integer construction", "[ast][expr]") {
  source_position pos{0, 1, 1};
  expression_integer expr{pos, 42};

  REQUIRE(expr.position.byte_offset == 0);
  REQUIRE(expr.value == 42);
}

TEST_CASE("expression_integer negative values", "[ast][expr]") {
  source_position pos{0, 1, 1};
  expression_integer expr{pos, -9223372036854775807LL};

  REQUIRE(expr.value == -9223372036854775807LL);
}

TEST_CASE("expression_float construction", "[ast][expr]") {
  source_position pos{0, 1, 1};
  expression_float expr{pos, 3.14159};

  REQUIRE(expr.value == 3.14159);
}

TEST_CASE("expression_string construction", "[ast][expr]") {
  source_position pos{0, 1, 1};
  expression_string expr{pos, "hello world"};

  REQUIRE(expr.value == "hello world");
}

TEST_CASE("expression_string with special characters", "[ast][expr]") {
  source_position pos{0, 1, 1};
  expression_string expr{pos, "line1\nline2\ttab"};

  REQUIRE(expr.value == "line1\nline2\ttab");
}
...
TEST_CASE("expression_identifier construction", "[ast][expr]") {
  ...
  source_position pos{0, 1, 1};
  expression_identifier expr{pos, name};

  REQUIRE(symbols.lookup(expr.name) == "myVar");
}
...
TEST_CASE("expression_list construction", "[ast][expr]") {
  ...
  elements.push_back(
      std::make_unique<expression_node>(expression_variant{expression_integer{pos, 1}}));
  elements.push_back(
      std::make_unique<expression_node>(expression_variant{expression_integer{pos, 2}}));
  elements.push_back(
      std::make_unique<expression_node>(expression_variant{expression_integer{pos, 3}}));

  expression_list list{pos, std::move(elements)};
...
TEST_CASE("expression_binary_operation construction", "[ast][expr]") {
  ...
  auto left = std::make_unique<expression_node>(expression_variant{expression_integer{pos, 1}});
  auto right = std::make_unique<expression_node>(expression_variant{expression_integer{pos, 2}});

  expression_binary_operation binop{pos, binary_operator::add, std::move(left), std::move(right)};

  REQUIRE(binop.op == binary_operator::add);
}

TEST_CASE("expression_unary_operation construction", "[ast][expr]") {
  source_position pos{0, 1, 1};

  auto operand = std::make_unique<expression_node>(expression_variant{expression_integer{pos, 42}});

  expression_unary_operation unop{pos, unary_operator::negate, std::move(operand)};

  REQUIRE(unop.op == unary_operator::negate);
}
...
TEST_CASE("expression_node wraps variants correctly", "[ast][expr]") {
  ...
  auto node = std::make_unique<expression_node>(expression_variant{expression_integer{pos, 42}});

  REQUIRE(std::holds_alternative<expression_integer>(node->data));

  auto& int_expr = std::get<expression_integer>(node->data);
  REQUIRE(int_expr.value == 42);
}

TEST_CASE("get_position extracts position from any expression", "[ast][expr]") {
  source_position pos1{10, 2, 5};
  source_position pos2{20, 3, 10};

  auto int_expr =
      std::make_unique<expression_node>(expression_variant{expression_integer{pos1, 42}});
  auto str_expr =
      std::make_unique<expression_node>(expression_variant{expression_string{pos2, "test"}});

  REQUIRE(get_position(int_expr).byte_offset == 10);
  REQUIRE(get_position(str_expr).byte_offset == 20);
}
...
TEST_CASE("attribute_path with static segments", "[ast][attr]") {
  ...
  attribute_path path;
  path.segments.push_back(attribute_name{{0, 0, 0}, symbols.intern("foo")});
  path.segments.push_back(attribute_name{{0, 0, 0}, symbols.intern("bar")});
  path.segments.push_back(attribute_name{{0, 0, 0}, symbols.intern("baz")});

  REQUIRE(path.segments.size() == 3);
  REQUIRE_FALSE(path.segments[0].is_dynamic());
  REQUIRE_FALSE(path.segments[1].is_dynamic());
  REQUIRE_FALSE(path.segments[2].is_dynamic());
}
...
TEST_CASE("attribute_path with dynamic segment", "[ast][attr]") {
  ...
  attribute_path path;
  path.segments.push_back(attribute_name{pos, std::move(dynamic_expr)});

  REQUIRE(path.segments.size() == 1);
  REQUIRE(path.segments[0].is_dynamic());
}
...
TEST_CASE("pattern_simple construction", "[ast][pattern]") {
  ...
  pattern_simple simple{arg};

  REQUIRE(symbols.lookup(simple.argument_name) == "x");
}
...
TEST_CASE("pattern_attrset construction", "[ast][pattern]") {
  ...
  formals.push_back(formal_parameter{pos, symbols.intern("a"), std::nullopt});
  formals.push_back(formal_parameter{pos, symbols.intern("b"), std::nullopt});

  pattern_attrset pattern{symbols.intern("args"), std::move(formals), true};
...
TEST_CASE("pattern_attrset with default values", "[ast][pattern]") {
  ...
  auto default_value =
      std::make_unique<expression_node>(expression_variant{expression_integer{pos, 42}});

  std::vector<formal_parameter> formals;
  formals.push_back(formal_parameter{pos, symbols.intern("x"), std::move(default_value)});

  pattern_attrset pattern{std::nullopt, std::move(formals), false};
...
TEST_CASE("binding_attribute construction", "[ast][binding]") {
  ...
  attribute_path path;
  path.segments.push_back(attribute_name{pos, symbols.intern("x")});

  auto value = std::make_unique<expression_node>(expression_variant{expression_integer{pos, 42}});

  binding_attribute binding{pos, std::move(path), std::move(value)};
...
TEST_CASE("binding_inherit construction", "[ast][binding]") {
  ...
  std::vector<attribute_name> attrs;
  attrs.push_back(attribute_name{pos, symbols.intern("a")});
  attrs.push_back(attribute_name{pos, symbols.intern("b")});

  binding_inherit inherit{pos, std::nullopt, std::move(attrs)};
...
TEST_CASE("binding_inherit with from expression", "[ast][binding]") {
  ...
  std::vector<attribute_name> attrs;
  attrs.push_back(attribute_name{pos, symbols.intern("lib")});

  binding_inherit inherit{pos, std::move(from), std::move(attrs)};
...
TEST_CASE("expression_attribute_set construction", "[ast][expr]") {
  ...
  bindings.push_back(binding_attribute{pos, std::move(path), std::move(value)});

  expression_attribute_set attrset{pos, false, std::move(bindings)};
...
TEST_CASE("expression_attribute_set recursive", "[ast][expr]") {
  ...
  expression_attribute_set attrset{pos, true, {}};
...
TEST_CASE("expression_let construction", "[ast][expr]") {
  ...
  std::vector<binding_variant> bindings;
  bindings.push_back(binding_attribute{pos, std::move(path), std::move(binding_value)});

  auto body = std::make_unique<expression_node>(
      expression_variant{expression_identifier{pos, symbols.intern("x")}});

  expression_let let_expr{pos, std::move(bindings), std::move(body)};
...
TEST_CASE("expression_if construction", "[ast][expr]") {
  ...
  auto cond =
      std::make_unique<expression_node>(expression_variant{expression_identifier{pos, symbol{0}}});
  auto then_branch =
      std::make_unique<expression_node>(expression_variant{expression_integer{pos, 1}});
  auto else_branch =
      std::make_unique<expression_node>(expression_variant{expression_integer{pos, 2}});

  expression_if if_expr{pos, std::move(cond), std::move(then_branch), std::move(else_branch)};

  REQUIRE(if_expr.condition != nullptr);
  REQUIRE(if_expr.then_branch != nullptr);
...
TEST_CASE("expression_lambda construction", "[ast][expr]") {
  ...
  auto pattern = std::make_unique<pattern_variant>(pattern_simple{symbols.intern("x")});
  auto body = std::make_unique<expression_node>(
      expression_variant{expression_identifier{pos, symbols.intern("x")}});

  expression_lambda lambda{pos, std::move(pattern), std::move(body)};

  REQUIRE(lambda.argument_pattern != nullptr);
...
TEST_CASE("expression_application construction", "[ast][expr]") {
  ...
  args.push_back(std::make_unique<expression_node>(expression_variant{expression_integer{pos, 1}}));
  args.push_back(std::make_unique<expression_node>(expression_variant{expression_integer{pos, 2}}));

  expression_application app{pos, std::move(func), std::move(args)};

  REQUIRE(app.arguments.size() == 2);
}
...
TEST_CASE("expression_with construction", "[ast][expr]") {
  ...
  expression_with with_expr{pos, std::move(ns), std::move(body)};

  REQUIRE(with_expr.namespace_expression != nullptr);
  REQUIRE(with_expr.body != nullptr);
}
...
TEST_CASE("expression_assert construction", "[ast][expr]") {
  ...
  auto cond =
      std::make_unique<expression_node>(expression_variant{expression_identifier{pos, symbol{0}}});
  auto body = std::make_unique<expression_node>(expression_variant{expression_integer{pos, 42}});

  expression_assert assert_expr{pos, std::move(cond), std::move(body)};

  REQUIRE(assert_expr.condition != nullptr);
  REQUIRE(assert_expr.body != nullptr);
}
...
TEST_CASE("expression_select construction", "[ast][expr]") {
  ...
  attribute_path path;
  path.segments.push_back(attribute_name{pos, symbols.intern("foo")});

  expression_select select{pos, std::move(subject), std::move(path), std::nullopt};
...
TEST_CASE("expression_select with default", "[ast][expr]") {
  ...
  auto default_val =
      std::make_unique<expression_node>(expression_variant{expression_integer{pos, 0}});

  expression_select select{pos, std::move(subject), std::move(path), std::move(default_val)};
...
TEST_CASE("binary_operator enum values", "[ast][enum]") {
  REQUIRE(static_cast<std::uint8_t>(binary_operator::add) !=
          static_cast<std::uint8_t>(binary_operator::subtract));
  REQUIRE(static_cast<std::uint8_t>(binary_operator::logical_and) !=
          static_cast<std::uint8_t>(binary_operator::logical_or));
}
