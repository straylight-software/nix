#pragma once
/// @file nix-language/parse/parser.hh
...
#include "nix-language/parse/convert.hh"
#include "nix-language/parse/grammar.hh"
#include "nix-language/parse/lower.hh"
#include "nix-language/parse/selector.hh"
...
public:
  ...
  explicit parse_error(const std::string& message) : std::runtime_error(message), position{1, 1} {}
};
  ...
  try {
    ...
    if (!positions.empty()) {
      ...
      throw parse_error(std::string(error.message()),
                        ast::source_position{static_cast<std::uint32_t>(pos.line),
