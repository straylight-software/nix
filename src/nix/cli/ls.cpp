...
struct MixLs : virtual Args, MixJSON, MixLongListing {
  ...
  void listText(ref<SourceAccessor> accessor, CanonPath path) {
    std::function<void(const SourceAccessor::Stat&, const CanonPath&, std::string_view, bool)>
        doPath;
    ...
    doPath = [&](const SourceAccessor::Stat& st, const CanonPath& curPath, std::string_view relPath,
                 bool showDirectory) {
...
struct CmdLsStore : StoreCommand, MixLs {
  ...
  CmdLsStore() { expectArgs({.label = "path", .handler = {&path}, .completer = completePath}); }
