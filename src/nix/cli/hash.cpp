...
struct CmdHashPath : CmdHashBase {
  CmdHashPath() : CmdHashBase(FileIngestionMethod::NixArchive) {
...
struct CmdHashFile : CmdHashBase {
  CmdHashFile() : CmdHashBase(FileIngestionMethod::Flat) {}
};
...
struct CmdToBase : Command {
  ...
  void run() override {
    ...
    for (const auto& s : args)
      logger->cout(
          Hash::parseAny(s, hashAlgo).to_string(hashFormat, hashFormat == HashFormat::SRI));
  }
};
