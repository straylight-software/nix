...
auto make_uint64_payload(::std::uint64_t value) -> ::std::string {
  ...
  for (auto idx = 0; idx < 8; ++idx) {
    result[idx] = static_cast<char>((value >> (idx * 8)) & 0xFF);
  }
  return result;
}
...
auto make_string_payload(const ::std::string& s) -> ::std::string {
  auto result = make_uint64_payload(s.size());
  result += s;
...
TEST_CASE("bug: read_error with invalid type causes assertion failure", "[fuzz][serialise][bug]") {
  SECTION("type is not 'Error'") {
    ...
    auto payload = make_string_payload("NotError");
    ...
    nix::string_source_t source(payload);
  ...
  SECTION("type is empty string") {
    auto payload = make_string_payload("");
...
TEST_CASE("bug: read_error with non-zero have_pos causes assertion failure",
  ...
  auto payload = make_string_payload("Error");
  payload += make_uint64_payload(0);  // level
...
TEST_CASE("bug: read_num integer overflow", "[fuzz][serialise][bug]") {
  SECTION("uint64_max for int") {
    auto payload = make_uint64_payload(UINT64_MAX);
    nix::string_source_t source(payload);

    // This should throw SerialisationError
    CHECK_THROWS_AS(nix::read_num<int>(source), nix::SerialisationError);
  }

  SECTION("negative via uint64") {
    // INT64_MIN as uint64 is 0x8000000000000000
    auto payload = make_uint64_payload(static_cast<::std::uint64_t>(INT64_MIN));
    nix::string_source_t source(payload);

    // Reading as unsigned int should fail (value too large)
    CHECK_THROWS_AS(nix::read_num<unsigned int>(source), nix::SerialisationError);
  }

  SECTION("uint32_max + 1 for uint32") {
    auto payload = make_uint64_payload(static_cast<::std::uint64_t>(UINT32_MAX) + 1);
    nix::string_source_t source(payload);

    CHECK_THROWS_AS(nix::read_num<::std::uint32_t>(source), nix::SerialisationError);
  }
}
...
TEST_CASE("bug: read_string memory exhaustion", "[fuzz][serialise][bug]") {
  ...
  auto payload = make_uint64_payload(1ULL << 40);
  ...
  CHECK_THROWS(nix::read_string(source));
}
...
TEST_CASE("fuzz: read_num with arbitrary data", "[fuzz][serialise]") {
  rc::prop("arbitrary 8 bytes should not crash read_num", []() {
    ...
    nix::string_source_t source(bytes);

    // Should either succeed or throw, never crash
    try {
      [[maybe_unused]] auto val = nix::read_num<::std::uint64_t>(source);
...
TEST_CASE("fuzz: read_string with arbitrary length prefix", "[fuzz][serialise]") {
  rc::prop("arbitrary length prefix should not crash", []() {
    ...
    auto payload = make_uint64_payload(len);
    ...
    try {
      [[maybe_unused]] auto s = nix::read_string(source, 1024 * 1024); // Limit to 1MB
    } catch (const nix::EndOfFile&) {
      // Expected
    } catch (const nix::SerialisationError&) {
      // Also acceptable
...
TEST_CASE("bug: framed_source with malicious chunk size", "[fuzz][serialise][bug]") {
  SECTION("chunk size is MAX_UINT32") {
    // Claim chunk is 4GB
    auto payload = make_uint64_payload(UINT32_MAX);
    nix::string_source_t underlying(payload);
  ...
  SECTION("chunk size is negative (as signed)") {
    ...
    auto payload = make_uint64_payload(static_cast<::std::uint64_t>(-1));
    nix::string_source_t underlying(payload);
...
TEST_CASE("bug: sized_source underflow", "[fuzz][serialise]") {
  ...
  nix::string_source_t underlying(data);

  // Create sized source with size 0
  nix::sized_source_t sized(underlying, 0);

  // Reading from size-0 source should throw EndOfFile
  char read_buffer[16];
  CHECK_THROWS_AS(sized.read(read_buffer, sizeof(read_buffer)), nix::EndOfFile);
...
TEST_CASE("fuzz: string roundtrip", "[fuzz][serialise]") {
  rc::prop("write_string then read_string roundtrips", []() {
    ...
    nix::write_string(s, sink);

    nix::string_source_t source(sink.str());
    auto result = nix::read_string(source);
...
TEST_CASE("fuzz: uint64 roundtrip", "[fuzz][serialise]") {
  rc::prop("write then read uint64 roundtrips", []() {
    ...
    nix::string_source_t source(sink.str());
    auto result = nix::read_num<::std::uint64_t>(source);
...
TEST_CASE("bug: empty source handling", "[fuzz][serialise][bug]") {
  ...
  nix::string_source_t source(empty);

  SECTION("read_num on empty source") {
    CHECK_THROWS_AS(nix::read_num<int>(source), nix::EndOfFile);
  }

  SECTION("read_string on empty source") {
    CHECK_THROWS_AS(nix::read_string(source), nix::EndOfFile);
  }
}
...
TEST_CASE("bug: truncated data handling", "[fuzz][serialise][bug]") {
  SECTION("truncated uint64") {
    auto partial = ::std::string(4, '\x00'); // Only 4 bytes, need 8
    nix::string_source_t source(partial);

    CHECK_THROWS_AS(nix::read_num<::std::uint64_t>(source), nix::EndOfFile);
  }

  SECTION("truncated string data") {
    // Claim 100 bytes but only provide 10
    auto payload = make_uint64_payload(100);
  ...
  SECTION("truncated string padding") {
    ...
    auto payload = make_uint64_payload(5);
    ...
    try {
      [[maybe_unused]] auto s = nix::read_string(source);
    } catch (...) {
