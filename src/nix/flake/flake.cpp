...
static void forceTrivialValue(EvalState& state, Value& value, const pos_idx_t pos) {
  if (value.isTrivial())
    state.forceValue(value, pos);
}

static void expectType(EvalState& state, ValueType type, Value& value, const pos_idx_t pos) {
...
parseFlakeInputs(EvalState& state, Value* value, const pos_idx_t pos,
                 const InputAttrPath& lockRootAttrPath, const source_path_t& flakeDir, bool allowSelf);
    ...
    default:
      if (attr.name == state.symbols.create("publicKeys")) {
        ...
        attrs.emplace(state.symbols[attr.name],
                      printValueAsJSON(state, true, *attr.value, attr.pos, emptyContext).dump());
      } else
        state
            .error<TypeError>(
...
static FlakeInput parseFlakeInput(EvalState& state, Value* value, const pos_idx_t pos,
                                  const InputAttrPath& lockRootAttrPath,
                                  const source_path_t& flakeDir) {
    ...
    try {
      if (attr.name == sUrl) {
        ...
        if (attr.value->type() == nString)
          ...
          throw Error("expected a string or a path but got %s at %s", showType(attr.value->type()),
  ...
  if (attrs.count("type"))
    try {
      ...
      e.addTrace(state.positions[pos], hint_fmt_t("while evaluating flake input"));
      throw;
    }
  else {
    attrs.erase("url");
...
static std::pair<std::map<FlakeId, FlakeInput>, fetchers::Attrs>
parseFlakeInputs(EvalState& state, Value* value, const pos_idx_t pos,
                 const InputAttrPath& lockRootAttrPath, const source_path_t& flakeDir,
                 bool allowSelf) {
  ...
  for (auto& inputAttr : *value->attrs()) {
    ...
    if (inputName == "self") {
      ...
      inputs.emplace(inputName, parseFlakeInput(state, inputAttr.value, inputAttr.pos,
                                                lockRootAttrPath, flakeDir));
    }
  }
...
static Flake readFlake(EvalState& state, const FlakeRef& originalRef, const FlakeRef& resolvedRef,
                       const FlakeRef& lockedRef, const source_path_t& rootDir,
                       const InputAttrPath& lockRootAttrPath) {
  auto flakeDir = rootDir / canon_path_t(resolvedRef.subdir);
        ...
        for (auto& formal : formals->formals) {
          if (formal.name != state.s.self)
            flake.inputs.emplace(
                ...
                FlakeInput{.ref = parseFlakeRef(state.fetchSettings,
  ...
  for (auto& attr : *vInfo.attrs()) {
    if (attr.name != state.s.description && attr.name != sInputs && attr.name != sOutputs &&
      ...
      throw Error("flake '%s' has an unsupported attribute '%s', at %s", resolvedRef,
...
static FlakeRef applySelfAttrs(const FlakeRef& ref, const Flake& flake) {
  ...
  string_set_t allowedAttrs{"submodules", "lfs"};
...
static Flake getFlake(EvalState& state, const FlakeRef& originalRef,
                      fetchers::UseRegistries useRegistries, const InputAttrPath& lockRootAttrPath,
                      bool requireLockable) {
  // Fetch a lazy tree first.
  auto cachedInput = state.inputCache->getAccessor(state.fetchSettings, *state.store,
                                                   originalRef.input, useRegistries);

  auto subdir =
      fetchers::maybeGetStrAttr(cachedInput.extraAttrs, "dir").value_or(originalRef.subdir);
  ...
  auto flake = readFlake(state, originalRef, resolvedRef, lockedRef, {cachedInput.accessor},
                         lockRootAttrPath);
  ...
  return readFlake(state, originalRef, resolvedRef, lockedRef,
                   state.storePath(state.mountInput(lockedRef.input, originalRef.input,
                                                    cachedInput.accessor, requireLockable)),
                   lockRootAttrPath);
}
...
static LockFile readLockFile(const fetchers::settings_t& fetchSettings,
                             const source_path_t& lockFilePath) {
...
LockedFlake lockFlake(const settings_t& settings, EvalState& state, const FlakeRef& topRef,
                      const LockFlags& lockFlags) {
  auto useRegistries = lockFlags.useRegistries.value_or(settings.useRegistries);
  auto useRegistriesTop =
      useRegistries ? fetchers::UseRegistries::All : fetchers::UseRegistries::No;
  auto useRegistriesInputs =
      useRegistries ? fetchers::UseRegistries::Limited : fetchers::UseRegistries::No;
  ...
  try {
    ...
    auto oldLockFile = readLockFile(state.fetchSettings,
    ...
    std::function<void(const FlakeInputs& flakeInputs, ref<Node> node,
                       ...
                       const source_path_t& sourcePath, bool trustLock)>
        computeLocks;
    ...
    computeLocks = [&](
      ...
      for (auto& [id, input2] : flakeInputs) {
        ...
        try {
          ...
          if (!input.ref)
            input.ref = FlakeRef::fromAttrs(state.fetchSettings,
                                            {{"type", "indirect"}, {"id", std::string(id)}});

          auto overriddenParentPath =
              input.ref->input.isRelative()
                  ? std::optional<InputAttrPath>(hasOverride ? i->second.parentInputAttrPath
                                                             : inputAttrPathPrefix)
                  : std::nullopt;
          ...
          auto getInputFlake = [&](const FlakeRef& ref,
                                   const fetchers::UseRegistries useRegistries) {
          ...
          if (oldLock && oldLock->originalRef.canonicalize() == input.ref->canonicalize() &&
            ...
            auto childNode =
                make_ref<LockedNode>(oldLock->lockedRef, oldLock->originalRef, oldLock->isFlake,
                                     oldLock->buildTime, oldLock->parentInputAttrPath);
            ...
            auto mustRefetch = lb != lockFlags.inputUpdates.end() &&
            ...
            if (mustRefetch) {
              ...
              computeLocks(fakeInputs, childNode, inputAttrPath, oldLock, followsPrefix, sourcePath,
                           true);
            }
            ...
            if (input.isFlake) {
              ...
              auto childNode = make_ref<LockedNode>(inputFlake.lockedRef, ref, true,
              ...
              auto [path, lockedRef] = [&]() -> std::tuple<source_path_t, FlakeRef> {
                ...
                if (auto resolvedPath = resolveRelativePath()) {
                  ...
                  return {state.storePath(state.mountInput(lockedRef.input, input.ref->input,
                                                           cachedInput.accessor, true, true)),
                          lockedRef};
                }
              }();

              auto childNode = make_ref<LockedNode>(lockedRef, ref, false, input.buildTime,
    ...
    computeLocks(flake.inputs, newLockFile.root, {},
                 lockFlags.recreateLockFile ? nullptr : oldLockFile.root.get_ptr(), {}, flake.path,
                 false);
          ...
          if (auto unlockedInput = newLockFile.isUnlocked(state.fetchSettings)) {
            ...
            if (lockFlags.outputLockFilePath) {
              ...
              topRef.input.putFile(
                  canon_path_t((topRef.subdir == "" ? "" : topRef.subdir + "/") + "flake.lock"),
...
static ref<SourceAccessor> makeInternalFS() {
  auto internalFS = make_ref<memory_source_accessor_t>(memory_source_accessor_t{});
  internalFS->setPathDisplay("«flakes-internal»", "");
  internalFS->addFile(canon_path_t("call-flake.nix"),
...
static Value* requireInternalFile(EvalState& state, canon_path_t path) {
  source_path_t p{internalFS, path};
...
void callFlake(EvalState& state, const LockedFlake& lockedFlake, Value& vRes) {
  ...
  for (auto& [node, sourcePath] : lockedFlake.nodePaths) {
    ...
    override.alloc(state.symbols.create("dir")).mkString(canon_path_t(subdir).rel(), state.mem);
  ...
  Value* vCallFlake = requireInternalFile(state, canon_path_t("call-flake.nix"));
...
LockedFlake::getFingerprint(Store& store, const fetchers::settings_t& fetchSettings) const {
  ...
  *fingerprint += fmt(";%s;%s", flake.lockedRef.subdir, lockFile);
  ...
  return hashString(hash_algorithm_t::SHA256, *fingerprint);
}
    ...
    if (search == state.evalCaches.end()) {
      search = state.evalCaches
                   .emplace(fingerprint.value(),
                            make_ref<eval_cache::EvalCache>(fingerprint, state, rootLoader))
                   .first;
    }
