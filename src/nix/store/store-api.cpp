...
bool Store::isValidPath(const StorePath& storePath) {
  ...
  if (diskCache) {
    auto res = diskCache->lookupNarInfo(config.getReference().render(/*FIXME withParams=*/false),
                                        std::string(storePath.hashPart()));
    if (res.first != NarInfoDiskCache::oUnknown) {
      stats.narInfoReadAverted++;
      pathInfoCache->lock()->upsert(storePath, res.first == NarInfoDiskCache::oInvalid
                                                   ? PathInfoCacheValue{}
  ...
  if (diskCache && !valid)
    ...
    diskCache->upsertNarInfo(config.getReference().render(/*FIXME withParams=*/false),
                             std::string(storePath.hashPart()), 0);
...
Store::queryPathInfoFromClientCache(const StorePath& storePath) {
...
void Store::queryPathInfo(const StorePath& storePath,
  ...
  queryPathInfoUncached(
      storePath, {[this, storePath, hashPart,
...
StorePathSet Store::exportReferences(const StorePathSet& storePaths,
...
static std::string makeCopyPathMessage(const StoreConfig& srcCfg, const StoreConfig& dstCfg,
  ...
  return fmt("copying path '%s' from '%s' to '%s'", storePath, srcCfg.getHumanReadableURI(),
             dstCfg.getHumanReadableURI());
}

void copyStorePath(Store& srcStore, Store& dstStore, const StorePath& storePath, RepairFlag repair,
                   CheckSigsFlag checkSigs) {
  ...
  if (info->ca && info->references.empty()) {
    ...
    info2->path = dstStore.makeFixedOutputPathFromCA(info->path.name(),
  ...
  auto source = sinkToSource(
      ...
      [&]() {
        throw EndOfFile("NAR for '%s' fetched from '%s' is incomplete",
                        srcStore.printStorePath(storePath), srcStore.config.getHumanReadableURI());
      });

  dstStore.addToStore(*info, *source, repair, checkSigs);
}

std::map<StorePath, StorePath> copyPaths(Store& srcStore, Store& dstStore,
...
std::map<StorePath, StorePath> copyPaths(Store& srcStore, Store& dstStore,
                                         const StorePathSet& storePaths, RepairFlag repair,
                                         CheckSigsFlag checkSigs, SubstituteFlag substitute) {
    ...
    if (currentPathInfo.ca && currentPathInfo.references.empty()) {
      ...
      if (storePathForDst != storePathForSrc)
        debug("replaced path '%s' to '%s' for substituter '%s'",
              srcStore.printStorePath(storePathForSrc), dstStore.printStorePath(storePathForDst),
...
void copyClosure(Store& srcStore, Store& dstStore, const RealisedPath::Set& paths,
...
void copyClosure(Store& srcStore, Store& dstStore, const StorePathSet& storePaths,
                 RepairFlag repair, CheckSigsFlag checkSigs, SubstituteFlag substitute) {
  if (&srcStore == &dstStore)
    return;
...
static Derivation readDerivationCommon(Store& store, const StorePath& drvPath,
