...
struct OutputsSpec {
  ...
  public:
    ...
    Names(const BaseType& s) : BaseType(s) { assert(!empty()); }

    /**
     * Needs to be "inherited manually"
     */
    Names(BaseType&& s) : BaseType(std::move(s)) { assert(!empty()); }
  ...
  bool operator<(const OutputsSpec& other) const { return raw < other.raw; }
...
struct ExtendedOutputsSpec {
  ...
  static std::optional<std::pair<std::string_view, ExtendedOutputsSpec>>
