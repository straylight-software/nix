...
using WeakGoals = std::set<WeakGoalPtr, std::owner_less<WeakGoalPtr>>;
...
public:
   ...
   * Coroutines are functions that use `co_await`/`co_return` (and `co_yield`, but not supported by
   * @ref Co).
   *
   ...
   * in the case of the initial suspension, or `await_suspend` can specify another coroutine to jump
   * to, which is how tail calls are implemented.
   *
   * @note Resources:
  ...
  struct [[nodiscard]] Co {
     ...
     * coroutine. In @ref promise_type::final_awaiter we check if it's set and if so we return to
     * it.
     *
     * To explain in more understandable terms:
     * When we `co_await Co_returning_function()`, this function is called on the resultant @ref Co
     * of the _called_ function, and C++ automatically passes the caller in.
     *
  ...
  struct InitialSuspend {
    ...
    void await_suspend(handle_type handle_) { handle = handle_; }
...
public:
  ...
  Goal(Worker& worker, Co init) : worker(worker), top_co(std::move(init)) {
