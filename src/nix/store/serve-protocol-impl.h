...
#define SERVE_USE_LENGTH_PREFIX_SERIALISER(TEMPLATE, T)                                            \
  TEMPLATE T ServeProto::Serialise<T>::read(const StoreDirConfig& store,                           \
                                            ServeProto::ReadConn conn) {                           \
    return LengthPrefixedProtoHelper<ServeProto, T>::read(store, conn);                            \
  }                                                                                                \
  TEMPLATE void ServeProto::Serialise<T>::write(const StoreDirConfig& store,                       \
...
#define COMMA_ ,
SERVE_USE_LENGTH_PREFIX_SERIALISER(template <typename T COMMA_ typename Compare>,
