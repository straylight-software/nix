          ...
          [&](const StoreReference::Auto&) -> ref<StoreConfig> {
            ...
            if (access(stateDir.c_str(), R_OK | W_OK) == 0)
              ...
              return make_ref<UDSRemoteStore::config_t>(params);
                     ...
                     !get_env("NIX_STORE_DIR").has_value() && !get_env("NIX_STATE_DIR").has_value()) {
              ...
              if (!path_exists(chrootStore)) {
                ...
                warn("'%s' does not exist, so Nix will use '%s' as a chroot store", stateDir,
                     chrootStore);
              } else
                debug("'%s' does not exist, so Nix will use '%s' as a chroot store", stateDir,
                      chrootStore);
