...
getCredentialsFromProvider(std::shared_ptr<Aws::Crt::Auth::ICredentialsProvider> provider) {
  ...
  provider->GetCredentials([prom](std::shared_ptr<Aws::Crt::Auth::Credentials> credentials,
                                  int errorCode) {
    if (errorCode != 0 || !credentials) {
      ...
      prom->set_value(AwsCredentials(std::string(accessKeyId.data(), accessKeyId.size()),
  ...
  if (fut.wait_for(timeout) == std::future_status::timeout) {
    throw AwsAuthError("Timeout waiting for AWS credentials (%d seconds)",
...
AwsCredentialProviderImpl::createProviderForProfile(const std::string& profile) {
  debug("[pid=%d] creating new AWS credential provider for profile '%s'", getpid(),
...
AwsCredentials AwsCredentialProviderImpl::getCredentialsRaw(const std::string& profile) {
  ...
  credentialProviderCache.try_emplace_and_cvisit(
      profile, nullptr, [&](auto& kv) { provider = kv.second = createProviderForProfile(profile); },
  ...
  if (!provider) {
    ...
    throw AwsAuthError("Failed to create AWS credentials provider for %s",
