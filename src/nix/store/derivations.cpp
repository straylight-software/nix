...
std::optional<store_path_t> derivation_output_t::path(const store_dir_config_t& store,
  ...
  return std::visit(
      overloaded{
          ...
          [](const derivation_output_t::Impure&) -> std::optional<store_path_t> {
            return std::nullopt;
          },
      },
      raw);
...
store_path_t derivation_output_t::CAFixed::path(const store_dir_config_t& store,
                                                std::string_view drv_name,
                                                OutputNameView output_name) const {
...
store_path_t write_derivation(store_t& store, const derivation_t& drv, RepairFlag repair,
                              bool read_only) {
...
parse_derivation_output(const store_dir_config_t& store, std::string_view path_s,
...
static derivation_output_t parse_derivation_output(
    const store_dir_config_t& store, string_view_stream_t& str,
...
parse_derived_path_map_node(const store_dir_config_t& store, string_view_stream_t& str,
  ...
  derivation_t parse_derivation(const store_dir_config_t& store, std::string&& s,
    ...
    switch (str.peek()) {
      case 'e':
        expect(str, std::string_view{"erive("});
        version = derivation_a_term_version_t::traditional;
        break;
      case 'r': {
        expect(str, std::string_view{"rvWithVersion("});
        auto version_s = parse_string(str);
        if (*version_s == std::string_view{"xp-dyn-drv"}) {
...
std::string
derivation_t::unparse(const store_dir_config_t& store, bool mask_outputs,
          ...
          if (has_dynamic_drv_dep(*this)) {
            s += std::string_view{"DrvWithVersion("};
            // Only version we have so far
            print_unquoted_string(s, std::string_view{"xp-dyn-drv"});
            s += ',';
          } else {
            s += std::string_view{"Derive("};
          }
            ...
            std::visit(
                overloaded{
                    [&](const derivation_output_t::InputAddressed& doi) {
                      s += ',';
                      print_unquoted_string(s, mask_outputs ? std::string_view{""}
                                                            : store.printStorePath(doi.path));
                      s += ',';
                    ...
                    [&](const derivation_output_t::CAFixed& dof) {
                      ...
                      print_unquoted_string(
                          s, mask_outputs ? std::string_view{""}
                                          : store.printStorePath(dof.path(store, name, i.first)));
                      s += ',';
                    ...
                    [&](const derivation_output_t::Impure& doi) {
                      ...
                      s += ',';
                      print_unquoted_string(s, std::string_view{"impure"});
                    }},
                i.second.raw);
            s += ')';
          }
          ...
          auto unparseEnv = [&](const string_pairs_t aterm_env) {
            for (auto& i : aterm_env) {
              ...
              s += ',';
              print_string(s, mask_outputs && outputs.count(i.first) ? std::string_view{""}
                                                                     : i.second);
              s += ')';
            }
          };
...
std::string output_path_name(std::string_view drv_name, OutputNameView output_name) {
          ...
          if (output_name != std::string_view{"out"}) {
            res += '-';
            res += output_name;
          }
          return res;
          ...
          auto decide = [&](DerivationType newTy) {
            if (!ty)
              ...
              throw Error("can't mix derivation output types");
                     ...
                     DerivationType::ContentAddressed{.sandboxed = false, .fixed = true})
...
DrvHash hash_derivation_modulo(store_t& store, const derivation_t& drv, bool mask_outputs) {
          ...
          if (type.isFixed()) {
            ...
            for (const auto& i : drv.outputs) {
              ...
              auto hash =
                  hash_string(hash_algorithm_t::SHA256,
          ...
          auto kind = std::visit(overloaded{[](const DerivationType::InputAddressed& ia) {
                                              /* This might be a "pesimistically" deferred output,
                                                 so we don't "taint" the kind yet. */
                                              return DrvHash::Kind::regular;
                                            },
                                            [](const DerivationType::ContentAddressed& ca) {
                                              return ca.fixed ? DrvHash::Kind::regular
          ...
          auto hash =
              hash_string(hash_algorithm_t::SHA256, drv.unparse(store, mask_outputs, &inputs2));
...
static derivation_output_t read_derivation_output(source_t& in, const store_dir_config_t& store) {
          ...
          return parse_derivation_output(store, path_s, hash_algo, hash,
...
basic_derivation_t::outputsAndOptPaths(const store_dir_config_t& store) const {
          ...
          for (auto& [output_name, output] : outputs)
            outsAndOptPaths.insert(std::make_pair(
                output_name, std::make_pair(output, output.path(store, name, output_name))));
...
source_t& read_derivation(source_t& in, const store_dir_config_t& store, basic_derivation_t& drv,
          ...
          drv.input_srcs = CommonProto::Serialise<store_path_set_t>::read(
              store, CommonProto::ReadConn{.from = in});
...
void write_derivation(sink_t& out, const store_dir_config_t& store, const basic_derivation_t& drv) {
          ...
          for (auto& i : drv.outputs) {
            ...
            std::visit(overloaded{
                           ...
                           [&](const derivation_output_t::CAFloating& dof) {
                             out << ""
                                 << (std::string{dof.method.renderPrefix()} +
                                     print_hash_algo(dof.hash_algo))
                                 << "";
                           },
                           [&](const derivation_output_t::Deferred&) {
                           ...
                           [&](const derivation_output_t::Impure& doi) {
                             out << ""
                                 << (std::string{doi.method.renderPrefix()} +
                                     print_hash_algo(doi.hash_algo))
                                 << "impure";
                           },
                       },
...
static void rewrite_derivation(store_t& store, basic_derivation_t& drv,
          ...
          for (auto& [output_name, output] : drv.outputs) {
            if (std::holds_alternative<derivation_output_t::Deferred>(output.raw)) {
              ...
              if (!h)
                throw Error(
                    "derivation '%s' output '%s' has no hash (derivations.cc/rewriteDerivation)",
...
std::optional<basic_derivation_t> derivation_t::try_resolve(store_t& store,
                                                            store_t* eval_store) const {
                             ...
                             [&](ref<const SingleDerivedPath> drv_path,
                               ...
                               try {
                                 return resolve_derived_path(
                                     store, SingleDerivedPath::Built{drv_path, output_name},
                                     eval_store);
                               } catch (Error&) {
                                 return std::nullopt;
                               }
...
try_resolve_input(store_t& store, store_path_set_t& input_srcs, string_map_t& input_rewrites,
                  const DownstreamPlaceholder* placeholder_opt,
                  ref<const SingleDerivedPath> drv_path,
          ...
          auto get_placeholder = [&](const std::string& output_name) {
            return placeholder_opt
                       ? DownstreamPlaceholder::unknownDerivation(*placeholder_opt, output_name)
                       : [&] {
          ...
          for (auto& [output_name, childNode] : input_node.childMap) {
            ...
            if (!try_resolve_input(store, input_srcs, input_rewrites, &nextPlaceholder,
                                   make_ref<const SingleDerivedPath>(
                                       SingleDerivedPath::Built{drv_path, output_name}),
...
std::optional<basic_derivation_t> derivation_t::try_resolve(
    store_t& store, std::function<std::optional<store_path_t>(ref<const SingleDerivedPath> drv_path,
                                                              const std::string& output_name)>
            ...
            auto envHasRightPath = [&](const store_path_t& actual, bool isDeferred = false) {
              ...
              if (j->second != store.printStorePath(actual)) {
                if (isDeferred)
                  ...
                  throw Error(
                      "derivation has incorrect environment variable '%s', should be '%s' but is "
                      "actually '%s'",
                      output_name, store.printStorePath(actual), j->second);
              }
            };
                  ...
                  if constexpr (std::is_same_v<Output, derivation_output_t::InputAddressed>) {
                    ...
                    if constexpr (fillIn)
                      ...
                      warn("derivation has incorrect deferred output, should be '%s'.\nThis will "
                           "be an "
                           "error in future versions of Nix; compatibility of CA derivations will "
                           "be "
                           "broken.",
                           store.printStorePath(out_path));
                  } else {
...
namespace nlohmann {
        void adl_serializer<nix::derivation_output_t>::to_json(json & res,
                                                               const nix::derivation_output_t& o) {
          res = nlohmann::json::object();
          std::visit(nix::overloaded{
                         [&](const nix::derivation_output_t::InputAddressed& doi) {
                           res["path"] = doi.path;
                         },
                         [&](const nix::derivation_output_t::CAFixed& dof) {
                           res = dof.ca;
          // FIXME print refs?
          /* it would be nice to output the path for user convenience, but
             this would require us to know the store dir. */
                ...
                res["path"] = dof.path(store, drv_name, output_name);
                         ...
                         [&](const nix::derivation_output_t::CAFloating& dof) {
                           res["method"] = std::string{dof.method.render()};
                           res["hashAlgo"] = nix::print_hash_algo(dof.hash_algo);
                         },
                         [&](const nix::derivation_output_t::Deferred&) {},
                         [&](const nix::derivation_output_t::Impure& doi) {
                           res["method"] = std::string{doi.method.render()};
                           res["hashAlgo"] = nix::print_hash_algo(doi.hash_algo);
                           res["impure"] = true;
                         },
                     },
                     o.raw);
        ...
        nix::derivation_output_t adl_serializer<nix::derivation_output_t>::from_json(
            const json& _json, const nix::experimental_feature_settings_t& xp_settings) {
          ...
          auto methodAlgo =
              [&]() -> std::pair<nix::content_address_method_t, nix::hash_algorithm_t> {
            nix::content_address_method_t method = nix::content_address_method_t::parse(
                nix::get_string(nix::value_at(json, "method")));
            if (method == nix::content_address_method_t::raw_t::Text)
              xp_settings.require(nix::xp_t::dynamic_derivations,
                                  "text-hashed derivation output in JSON");
          ...
          if (keys == (std::set<std::string_view>{"path"})) {
            return nix::derivation_output_t::InputAddressed{
                .path = nix::value_at(json, "path"),
            };
          }
...
#if 0
        if (dof.path(store, drv_name, output_name) != static_cast<nix::store_path_t>(nix::value_at(json, "path")))
            throw nix::Error("Path doesn't match derivation output");
#endif
            return dof;
          }

          else if (keys == (std::set<std::string_view>{"method", "hashAlgo"})) {
            xp_settings.require(nix::xp_t::ca_derivations);
            auto [method, hash_algo] = methodAlgo();
            return nix::derivation_output_t::CAFloating{
                .method = std::move(method),
                .hash_algo = std::move(hash_algo),
            };
          }
            ...
            return nix::derivation_output_t::Deferred{};
          }

          else if (keys == (std::set<std::string_view>{"method", "hashAlgo", "impure"})) {
            xp_settings.require(nix::xp_t::impure_derivations);
            auto [method, hash_algo] = methodAlgo();
            return nix::derivation_output_t::Impure{
                .method = std::move(method),
                .hash_algo = hash_algo,
            };
          }
            ...
            throw nix::Error("invalid JSON for derivation output");
          }
        }
        ...
        derivation_t adl_serializer<derivation_t>::from_json(
            const json& _json, const experimental_feature_settings_t& xp_settings) {
...

          {
            ...
            if (value_at(json, "version") != expectedJsonVersionDerivation)
              throw Error(
                  "Unsupported derivation JSON format version %d, only format version %d is "
                  "currently supported.",
          ...
          try {
            ...
            for (auto& [output_name, output] : outputs) {
              res.outputs.insert_or_assign(
                  output_name, adl_serializer<derivation_output_t>::from_json(output, xp_settings));
            }
          } catch (Error& e) {
