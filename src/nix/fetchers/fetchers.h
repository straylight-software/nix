...
public:
  ...
  bool operator<(const Input& other) const { return attrs < other.attrs; }
   ...
   */
  std::tuple<StorePath, ref<SourceAccessor>, Input> fetchToStore(const Settings& settings,
                                                                 Store& store) const;
...
private:
  std::pair<ref<SourceAccessor>, Input> getAccessorUnchecked(const Settings& settings,
                                                             Store& store) const;
...
public:
   ...
   */
  void putFile(const CanonPath& path, std::string_view contents,
...
struct InputScheme {
  ...
  virtual std::optional<Input> inputFromURL(const Settings& settings, const ParsedURL& url,
                                            bool requireTree) const = 0;

  virtual std::optional<Input> inputFromAttrs(const Settings& settings,
  ...
  virtual Input applyOverrides(const Input& input, std::optional<std::string> ref,
                               std::optional<Hash> rev) const;

  virtual void clone(const Settings& settings, Store& store, const Input& input,
                     const std::filesystem::path& destDir) const;

  virtual std::optional<std::filesystem::path> getSourcePath(const Input& input) const;

  virtual void putFile(const Input& input, const CanonPath& path, std::string_view contents,
                       std::optional<std::string> commitMsg) const;

  virtual std::pair<ref<SourceAccessor>, Input> getAccessor(const Settings& settings, Store& store,
  ...
  virtual bool isLocked(const Settings& settings, const Input& input) const { return false; }

  virtual std::optional<std::string> isRelative(const Input& input) const { return std::nullopt; }

  virtual std::optional<std::string> getAccessToken(const fetchers::Settings& settings,
