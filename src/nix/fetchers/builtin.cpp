...
static void builtin_fetch_tree(const BuiltinBuilderContext& ctx) {
  ...
  auto input = input_t::fromAttrs(
      my_fetch_settings, json_to_attrs(ctx.drv.structured_attrs->structured_attrs.at("input")));
  ...
  auto source =
      sink_to_source([&](sink_t& sink) { accessor->dump_path(canon_path_t::root, sink); });
