...
TEST_CASE("github: shouldUseSsh returns false when narHash present", "[fetchers][github][ssh]") {
  ...
  auto input = input_t::fromURL(settings, url);
  ...
  CHECK(input.getNarHash()->to_string(hash_format_t::sri, true) ==
...
TEST_CASE("github: input without narHash allows SSH fallback", "[fetchers][github][ssh]") {
  ...
  auto url = parse_url("github:owner/repo/abc123");
  auto input = input_t::fromURL(settings, url);
...
TEST_CASE("github: preferSshForGitForges respected", "[fetchers][github][ssh]") {
  ...
  auto url = parse_url("github:owner/repo");
  auto input = input_t::fromURL(settings, url);

  // Input should be valid
  CHECK(get_str_attr(input.attrs, "owner") == "owner");
  CHECK(get_str_attr(input.attrs, "repo") == "repo");
}
...
TEST_CASE("github: basic URL parsing", "[fetchers][github]") {
  ...
  SECTION("owner/repo") {
    auto url = parse_url("github:nixos/nix");
    auto input = input_t::fromURL(settings, url);

    CHECK(get_str_attr(input.attrs, "owner") == "nixos");
    CHECK(get_str_attr(input.attrs, "repo") == "nix");
    CHECK_FALSE(input.getRef().has_value());
    CHECK_FALSE(input.getRev().has_value());
  }

  SECTION("owner/repo/ref") {
    auto url = parse_url("github:nixos/nix/master");
    auto input = input_t::fromURL(settings, url);
  ...
  SECTION("owner/repo/rev (40-char hex)") {
    auto url = parse_url("github:nixos/nix/abc123def456abc123def456abc123def456abc1");
    auto input = input_t::fromURL(settings, url);
  ...
  SECTION("owner/repo?ref=branch") {
    auto url = parse_url("github:nixos/nix?ref=staging");
    auto input = input_t::fromURL(settings, url);
  ...
  SECTION("owner/repo?host=github.example.com") {
    auto url = parse_url("github:myorg/myrepo?host=github.example.com");
    auto input = input_t::fromURL(settings, url);

    CHECK(maybe_get_str_attr(input.attrs, "host").value_or("") == "github.example.com");
  }
}
...
TEST_CASE("gitlab: basic URL parsing", "[fetchers][gitlab]") {
  ...
  SECTION("owner/repo") {
    auto url = parse_url("gitlab:owner/repo");
    auto input = input_t::fromURL(settings, url);
  ...
  SECTION("with custom host") {
    auto url = parse_url("gitlab:owner/repo?host=gitlab.mycompany.com");
    auto input = input_t::fromURL(settings, url);

    CHECK(maybe_get_str_attr(input.attrs, "host").value_or("") == "gitlab.mycompany.com");
  }
}
...
TEST_CASE("sourcehut: basic URL parsing", "[fetchers][sourcehut]") {
  ...
  SECTION("owner/repo") {
    auto url = parse_url("sourcehut:~user/repo");
    auto input = input_t::fromURL(settings, url);

    CHECK(get_str_attr(input.attrs, "owner") == "~user");
    CHECK(get_str_attr(input.attrs, "repo") == "repo");
  }
}
...
TEST_CASE("github: type preserved in attrs", "[fetchers][github]") {
  ...
  CHECK(get_str_attr(input.attrs, "type") == "github");
}
...
TEST_CASE("gitlab: type preserved in attrs", "[fetchers][gitlab]") {
  ...
  CHECK(get_str_attr(input.attrs, "type") == "gitlab");
}

TEST_CASE("sourcehut: type preserved in attrs", "[fetchers][sourcehut]") {
  settings_t settings;

  auto url = parse_url("sourcehut:owner/repo");
  auto input = input_t::fromURL(settings, url);

  CHECK(get_str_attr(input.attrs, "type") == "sourcehut");
}
...
TEST_CASE("github: rejects invalid URLs", "[fetchers][github][error]") {
  ...
  SECTION("missing repo") {
    auto url = parse_url("github:owner");
    CHECK_THROWS(input_t::fromURL(settings, url));
  }

  SECTION("unknown query param") {
    auto url = parse_url("github:owner/repo?unknown=value");
    CHECK_THROWS(input_t::fromURL(settings, url));
  }

  SECTION("both ref and rev in URL") {
    // Can't have both ?ref= and ?rev=
    auto url = parse_url("github:owner/repo?ref=main&rev=abc123");
    CHECK_THROWS(input_t::fromURL(settings, url));
  }
}
