...
std::optional<memory_source_accessor_t::stat_t>
memory_source_accessor_t::maybe_lstat(const canon_path_t& path) {
  const auto* f = open(path, std::nullopt);
  return f ? std::optional{f->lstat()} : std::nullopt;
}

memory_source_accessor_t::dir_entries_t
memory_source_accessor_t::read_directory(const canon_path_t& path) {
  auto* f = open(path, std::nullopt);
  if (!f) {
    throw Error("file '%s' does not exist", path);
  }
