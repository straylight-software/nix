...
nix::ref<nix::memory_source_accessor_t> make_test_accessor() {
  auto accessor = nix::make_ref<nix::memory_source_accessor_t>();
  accessor->add_file(nix::canon_path_t("/file.txt"), "hello world");
  accessor->add_file(nix::canon_path_t("/dir/nested.txt"), "nested content");
  accessor->add_file(nix::canon_path_t("/dir/subdir/deep.txt"), "deep content");
  return accessor;
}

// Create an empty MemorySourceAccessor
nix::ref<nix::memory_source_accessor_t> make_empty_accessor() {
  auto accessor = nix::make_ref<nix::memory_source_accessor_t>();
  // Initialize with an empty root directory
  accessor->root =
      nix::memory_source_accessor_t::file_t{nix::memory_source_accessor_t::file_t::directory_t{}};
  return accessor;
}
...
TEST_CASE("source path construction with root", "[source-path][construction]") {
  ...
  nix::source_path_t path(accessor);
...
TEST_CASE("source path construction with explicit path", "[source-path][construction]") {
  auto accessor = make_empty_accessor();
  nix::source_path_t path(accessor, nix::canon_path_t("/foo/bar"));
...
TEST_CASE("source path construction preserves accessor", "[source-path][construction]") {
  auto accessor = make_empty_accessor();
  nix::source_path_t path(accessor, nix::canon_path_t("/test"));
...
TEST_CASE("source path basename for simple path", "[source-path][basename]") {
  auto accessor = make_empty_accessor();
  nix::source_path_t path(accessor, nix::canon_path_t("/foo"));
...
TEST_CASE("source path basename for nested path", "[source-path][basename]") {
  auto accessor = make_empty_accessor();
  nix::source_path_t path(accessor, nix::canon_path_t("/foo/bar/baz.txt"));
...
TEST_CASE("source path parent of nested path", "[source-path][parent]") {
  auto accessor = make_empty_accessor();
  nix::source_path_t path(accessor, nix::canon_path_t("/foo/bar/baz"));

  nix::source_path_t parent = path.parent();
...
TEST_CASE("source path parent preserves accessor reference", "[source-path][parent]") {
  auto accessor = make_empty_accessor();
  nix::source_path_t path(accessor, nix::canon_path_t("/a/b/c/d/e"));

  nix::source_path_t p1 = path.parent();
  nix::source_path_t p2 = p1.parent();
  nix::source_path_t p3 = p2.parent();
...
TEST_CASE("source path operator slash with canon path", "[source-path][concatenation]") {
  auto accessor = make_empty_accessor();
  nix::source_path_t base(accessor, nix::canon_path_t("/foo"));

  nix::source_path_t result = base / nix::canon_path_t("/bar/baz");
...
TEST_CASE("source path operator slash with string view", "[source-path][concatenation]") {
  ...
  nix::source_path_t result = base / "bar";
...
TEST_CASE("source path operator slash from root", "[source-path][concatenation]") {
  auto accessor = make_empty_accessor();
  nix::source_path_t root(accessor);

  nix::source_path_t result = root / "foo";
...
TEST_CASE("source path chained operator slash", "[source-path][concatenation]") {
  ...
  nix::source_path_t result = root / "a" / "b" / "c";
...
TEST_CASE("source path equality same accessor same path", "[source-path][comparison]") {
  auto accessor = make_empty_accessor();
  nix::source_path_t p1(accessor, nix::canon_path_t("/foo/bar"));
  nix::source_path_t p2(accessor, nix::canon_path_t("/foo/bar"));
...
TEST_CASE("source path inequality same accessor different path", "[source-path][comparison]") {
  auto accessor = make_empty_accessor();
  nix::source_path_t p1(accessor, nix::canon_path_t("/foo"));
  nix::source_path_t p2(accessor, nix::canon_path_t("/bar"));
...
TEST_CASE("source path inequality different accessor same path", "[source-path][comparison]") {
  ...
  nix::source_path_t p1(accessor1, nix::canon_path_t("/foo"));
  nix::source_path_t p2(accessor2, nix::canon_path_t("/foo"));
...
TEST_CASE("source path three-way comparison ordering", "[source-path][comparison]") {
  auto accessor = make_empty_accessor();
  nix::source_path_t foo(accessor, nix::canon_path_t("/foo"));
  nix::source_path_t foo_bar(accessor, nix::canon_path_t("/foo/bar"));
  nix::source_path_t foo_baz(accessor, nix::canon_path_t("/foo/baz"));
...
TEST_CASE("source path stream output", "[source-path][string]") {
  auto accessor = make_empty_accessor();
  nix::source_path_t path(accessor, nix::canon_path_t("/test/path"));
...
TEST_CASE("source path hash consistency", "[source-path][hash]") {
  ...
  REQUIRE(std::hash<nix::source_path_t>{}(p1) == std::hash<nix::source_path_t>{}(p2));
  REQUIRE(nix::hash_value(p1) == nix::hash_value(p2));
}
...
TEST_CASE("source path usable in unordered_set", "[source-path][hash]") {
  auto accessor = make_empty_accessor();
  std::unordered_set<nix::source_path_t> paths;

  paths.insert(nix::source_path_t(accessor, nix::canon_path_t("/a")));
  paths.insert(nix::source_path_t(accessor, nix::canon_path_t("/b")));
  paths.insert(nix::source_path_t(accessor, nix::canon_path_t("/a"))); // duplicate
...
TEST_CASE("source path read file", "[source-path][fs]") {
  ...
  nix::source_path_t path(accessor, nix::canon_path_t("/file.txt"));
...
TEST_CASE("source path path exists", "[source-path][fs]") {
  ...
  nix::source_path_t existing(accessor, nix::canon_path_t("/file.txt"));
  REQUIRE(existing.path_exists());

  nix::source_path_t nonexistent(accessor, nix::canon_path_t("/nonexistent"));
...
TEST_CASE("source path lstat for file", "[source-path][fs]") {
  ...
  REQUIRE(stat.type == nix::source_accessor_t::t_regular);
}

TEST_CASE("source path lstat for directory", "[source-path][fs]") {
  auto accessor = make_test_accessor();
  nix::source_path_t path(accessor, nix::canon_path_t("/dir"));

  auto stat = path.lstat();
  REQUIRE(stat.type == nix::source_accessor_t::t_directory);
}

TEST_CASE("source path maybe lstat returns nullopt for nonexistent", "[source-path][fs]") {
  auto accessor = make_test_accessor();
  nix::source_path_t path(accessor, nix::canon_path_t("/nonexistent"));
...
TEST_CASE("source path traversal dot-dot is normalized", "[source-path][security]") {
  ...
  nix::source_path_t base(accessor, nix::canon_path_t("/foo/bar"));
  nix::source_path_t traversal = base / nix::canon_path_t("/../../../etc/passwd");
...
TEST_CASE("source path traversal cannot escape via concatenation", "[source-path][security]") {
  auto accessor = make_empty_accessor();
  nix::source_path_t base(accessor, nix::canon_path_t("/safe/dir"));

  // Multiple traversal attempts
  nix::source_path_t t1 = base / nix::canon_path_t("/../../..");
  REQUIRE_FALSE(t1.path.abs().contains(".."));

  nix::source_path_t t2 = base / nix::canon_path_t("/../../../../../../../etc");
...
TEST_CASE("source path traversal stays within root after deep nesting", "[source-path][security]") {
  ...
  nix::canon_path_t deep("/a/b/c/d/e/f/g/h/i/j");
  nix::source_path_t path(accessor, deep);

  // Try to traverse way beyond root
  nix::source_path_t traversal =
      path / nix::canon_path_t("/../../../../../../../../../../../../root");
...
TEST_CASE("source path rejects null bytes in path component", "[source-path][security]") {
  auto accessor = make_empty_accessor();
  nix::source_path_t base(accessor, nix::canon_path_t("/safe"));
...
TEST_CASE("source path null byte at start of component", "[source-path][security]") {
  ...
  REQUIRE_THROWS(nix::canon_path_t(std::string_view(evil.c_str(), evil.size())));
}
...
TEST_CASE("source path with unicode characters", "[source-path][unicode]") {
  ...
  nix::source_path_t path_emoji(accessor,
                                nix::canon_path_t("/dir/file_\xF0\x9F\x98\x80.txt")); // emoji
  REQUIRE(path_emoji.path.abs() == "/dir/file_\xF0\x9F\x98\x80.txt");

  nix::source_path_t path_cjk(accessor, nix::canon_path_t("/\xE4\xB8\xAD\xE6\x96\x87")); // Chinese
  REQUIRE_FALSE(path_cjk.path.is_root());

  nix::source_path_t path_arabic(accessor,
                                 nix::canon_path_t("/\xD8\xB9\xD8\xB1\xD8\xA8\xD9\x8A")); // Arabic
...
TEST_CASE("source path with unicode normalization forms", "[source-path][unicode]") {
  ...
  nix::source_path_t nfc(accessor, nix::canon_path_t("/caf\xC3\xA9"));
  // NFD: e + combining acute U+0065 U+0301
  nix::source_path_t nfd(accessor, nix::canon_path_t("/cafe\xCC\x81"));
...
TEST_CASE("source path with unicode lookalikes", "[source-path][unicode][security]") {
  ...
  nix::source_path_t latin(accessor, nix::canon_path_t("/a"));
  nix::source_path_t cyrillic(accessor, nix::canon_path_t("/\xD0\xB0"));
...
TEST_CASE("source path with bidirectional unicode", "[source-path][unicode][security]") {
  ...
  bidi_path += "exe.txt";
  nix::source_path_t path(accessor, nix::canon_path_t(bidi_path));
...
TEST_CASE("source path with zero width characters", "[source-path][unicode][security]") {
  ...
  nix::source_path_t path(accessor, nix::canon_path_t("/test\xE2\x80"
                                                      "\x8B"
                                                      "file"));
  REQUIRE_FALSE(path.path.is_root());

  // Zero-width joiner U+200D
  nix::source_path_t path2(accessor, nix::canon_path_t("/test\xE2\x80"
                                                       "\x8D"
                                                       "file"));
...
TEST_CASE("source path with very long path", "[source-path][edge-case]") {
  ...
  nix::source_path_t path(accessor, nix::canon_path_t(long_path));
  REQUIRE_FALSE(path.path.is_root());
...
TEST_CASE("source path with very long component name", "[source-path][edge-case]") {
  ...
  nix::source_path_t path(accessor, nix::canon_path_t("/" + long_component));
...
TEST_CASE("source path with special characters in component", "[source-path][edge-case]") {
  ...
  nix::source_path_t path1(accessor, nix::canon_path_t("/file with spaces"));
  REQUIRE(path1.base_name() == "file with spaces");

  nix::source_path_t path2(accessor, nix::canon_path_t("/file\twith\ttabs"));
  REQUIRE(path2.base_name() == "file\twith\ttabs");

  nix::source_path_t path3(accessor, nix::canon_path_t("/file\nwith\nnewlines"));
  REQUIRE(path3.base_name() == "file\nwith\nnewlines");

  nix::source_path_t path4(accessor, nix::canon_path_t("/!@#$%^&*()"));
...
TEST_CASE("source path property tests", "[source-path][property]") {
  ...
  rc::prop("source path always has valid accessor", [&]() {
    ...
    nix::canon_path_t canon(components);
    nix::source_path_t path(accessor, canon);
  ...
  rc::prop("source path slash operator preserves accessor", [&]() {
    ...
    nix::canon_path_t base_canon(base_components);
    nix::source_path_t base(accessor, base_canon);
    nix::source_path_t extended = base / component;
  ...
  rc::prop("source path equality is symmetric", [&]() {
    auto components = *generate_path_components();
    nix::canon_path_t canon(components);
    nix::source_path_t p1(accessor, canon);
    nix::source_path_t p2(accessor, canon);
  ...
  rc::prop("source path hash equality consistent with equality", [&]() {
    ...
    if (p1 == p2) {
      RC_ASSERT(std::hash<nix::source_path_t>{}(p1) == std::hash<nix::source_path_t>{}(p2));
    }
  });
  ...
  rc::prop("source path ordering is transitive", [&]() {
    ...
    nix::source_path_t p1(accessor, nix::canon_path_t(comp1));
    nix::source_path_t p2(accessor, nix::canon_path_t(comp2));
    nix::source_path_t p3(accessor, nix::canon_path_t(comp3));
...
TEST_CASE("source path fuzz path components", "[source-path][fuzz]") {
  ...
  rc::prop("fuzz: arbitrary strings as path components", [&]() {
    ...
    // Should not crash
    nix::source_path_t base(accessor, nix::canon_path_t("/base"));
    nix::source_path_t extended = base / raw;
...
TEST_CASE("source path fuzz unicode sequences", "[source-path][fuzz][unicode]") {
  ...
  rc::prop("fuzz: random utf-8 like sequences", [&]() {
    ...
    try {
      nix::source_path_t base(accessor, nix::canon_path_t("/base"));
      nix::source_path_t extended = base / str;
      RC_ASSERT(&*extended.accessor == &*accessor);
...
TEST_CASE("source path fuzz traversal attempts", "[source-path][fuzz][security]") {
  ...
  rc::prop("fuzz: cannot escape root via any combination of dot-dot", [&]() {
    ...
    nix::canon_path_t canon(path);
    nix::source_path_t source_path(accessor, canon);
...
TEST_CASE("source path fuzz hash collisions", "[source-path][fuzz][hash]") {
  ...
  rc::prop("fuzz: different paths have different hashes (probabilistic)", [&]() {
    ...
    if (!(p1 == p2)) {
      ...
      auto h1 = std::hash<nix::source_path_t>{}(p1);
      auto h2 = std::hash<nix::source_path_t>{}(p2);
...
TEST_CASE("source path fuzz control characters", "[source-path][fuzz][security]") {
  ...
  rc::prop("fuzz: control characters in paths", [&]() {
    ...
    nix::source_path_t base(accessor, nix::canon_path_t("/safe"));
    nix::source_path_t result = base / component;
...
TEST_CASE("source path stress many accessors", "[source-path][stress]") {
  rc::prop("stress: paths from many accessors", [&]() {
    ...
    std::vector<nix::ref<nix::memory_source_accessor_t>> accessors;
    std::vector<nix::source_path_t> paths;

    for (int i = 0; i < count; ++i) {
      auto acc = make_empty_accessor();
      accessors.push_back(acc);
      paths.emplace_back(acc, nix::canon_path_t("/test"));
    }
