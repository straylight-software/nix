...
private:
  ...
  explicit StringData(size_type size) : size_(size) {}
...
public:
  ...
  constexpr std::string_view view() const noexcept { return std::string_view(data_, size_); }

  template <size_t N>
  struct Static;

  static StringData& make(std::pmr::memory_resource& resource, std::string_view s) {
    auto& res = *new (resource.allocate(sizeof(StringData) + s.size() + 1, alignof(StringData)))
                    StringData(s.size());
...
struct ValueBase {
  ...
  struct StringWithContext {
    ...
    struct Context {
      ...
      Context(size_type size) : size_(size) {}
    ...
    public:
      ...
      iterator end() const { return elems + size(); }
...
#undef NIX_VALUE_STORAGE_FOR_EACH_FIELD
  ...
  InternalType getInternalType() const noexcept { return internalType; }
};
...
inline constexpr bool useBitPackedValueStorage =
    (ptrSize == 8) && (__STDCPP_DEFAULT_NEW_ALIGNMENT__ >= 16);
...
class alignas(16) ValueStorage<ptrSize, std::enable_if_t<detail::useBitPackedValueStorage<ptrSize>>>
  ...
  void setUntaggablePayload(T* firstPtrField, U untaggableField) noexcept {
    ...
    finish(static_cast<int>(discriminator) | firstFieldPayload,
  ...
  void setPairOfPointersPayload(T* firstPtrField, U* secondPtrField) noexcept {
    ...
    finish(static_cast<int>(pdPairOfPointers) | firstFieldPayload,
  ...
  InternalType getInternalType() const noexcept {
    switch (auto pd = getPrimaryDiscriminator()) {
      ...
      case pdPairOfPointers:
        return static_cast<InternalType>(tListSmall +
...
#define NIX_VALUE_STORAGE_DEF_PAIR_OF_PTRS(TYPE, SET, MEMBER_A, MEMBER_B)                          \
  ...
  NIX_VALUE_STORAGE_DEF_PAIR_OF_PTRS(PrimOpApplicationThunk, setPairOfPointersPayload, .left,
                                     .right)
...
#undef NIX_VALUE_STORAGE_DEF_PAIR_OF_PTRS
  ...
  void getStorage(bool& boolean) const noexcept { boolean = p1; }

  void getStorage(Null& null) const noexcept {}

  void getStorage(NixFloat& fpoint) const noexcept { fpoint = std::bit_cast<NixFloat>(p1); }
  ...
  void setStorage(Null path) noexcept { setSingleDWordPayload<tNull>(0); }
  ...
  void setStorage(List list) noexcept { setUntaggablePayload<pdListN>(list.elems, list.size); }

  void setStorage(StringWithContext string) noexcept {
    setUntaggablePayload<pdString>(string.context, string.str);
  }

  void setStorage(Path path) noexcept { setUntaggablePayload<pdPath>(path.accessor, path.path); }
  ...
  ValueStorage(const ValueStorage& v) { *this = v; }
...
public:
  ListView(SmallList list) : raw(list) {}
  ...
  std::size_t size() const noexcept {
    return std::visit(overloaded{[](const SmallList& list) -> std::size_t {
                                   return list.back() == nullptr ? 1 : 2;
                                 },
...
public:
  ...
  inline bool isThunk() const { return isa<tThunk>(); }

  inline bool isApp() const { return isa<tApp>(); }
  ...
  // type() == nFunction
  inline bool isLambda() const { return isa<tLambda>(); }

  inline bool isPrimOp() const { return isa<tPrimOp>(); }

  inline bool isPrimOpApp() const { return isa<tPrimOpApp>(); }
   ...
   */
  inline bool isValid() const noexcept { return !isa<tUninitialized>(); }

  inline void mkInt(NixInt::Inner n) noexcept { mkInt(NixInt{n}); }

  inline void mkInt(NixInt n) noexcept { setStorage(NixInt{n}); }
  ...
  inline void mkLambda(Env* e, ExprLambda* f) noexcept { setStorage(Lambda{.env = e, .fun = f}); }
  ...
  inline void mkExternal(ExternalValueBase* e) noexcept { setStorage(e); }
  ...
  const char* c_str() const noexcept { return getStorage<StringWithContext>().str->data(); }
  ...
  NixInt integer() const noexcept { return getStorage<NixInt>(); }

  NixFloat fpoint() const noexcept { return getStorage<NixFloat>(); }

  Lambda lambda() const noexcept { return getStorage<Lambda>(); }

  // FIXME: remove this since reading it is racy.
  ClosureThunk thunk() const noexcept { return getStorage<ClosureThunk>(); }

  PrimOpApplicationThunk primOpApp() const noexcept { return getStorage<PrimOpApplicationThunk>(); }

  // FIXME: remove this since reading it is racy.
  FunctionApplicationThunk app() const noexcept { return getStorage<FunctionApplicationThunk>(); }

  const char* pathStr() const noexcept { return getStorage<Path>().path->c_str(); }
