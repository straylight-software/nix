...
/**
 * A GC compatible vector that may used a reserved portion of `nItems` on the stack instead of
...
using SmallValueVector = SmallVector<value_t*, nItems>;
 ...
 */
template <size_t nItems>
using SmallTemporaryValueVector = SmallVector<value_t, nItems>;

/**
 * For functions where we do not expect deep recursion, we can use a sizable
 * part of the stack a free allocation space.
 *
 * Note: this is expected to be multiplied by sizeof(value_t), or about 24 bytes.
 */
