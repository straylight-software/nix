...
bool compare(std::span<const std::byte> generated, std::span<const std::byte> expected,
  ...
  for (size_t i = 0; i < generated.size(); ++i) {
    if (generated[i] != expected[i]) {
      std::cerr << "FAIL " << name << ": byte mismatch at offset " << i << " (got 0x" << std::hex
                << static_cast<int>(generated[i]) << ", expected 0x"
...
int main() {
  ...
  // Test 1: Client Hello
  {
    ...
    auto expected = read_file("captures/client_hello.bin");
  ...
  // Test 2: Server Hello
  {
    ...
    auto expected = read_file("captures/server_hello.bin");
  ...
  // Test 3: IsValidPath request
  {
    ...
    auto captured = read_file("captures/isvalidpath_request.bin");
    if (captured.size() >= 16) {
  ...
  // Test 4: QueryPathInfo request
  {
    ...
    auto captured = read_file("captures/querypathinfo_request.bin");
    if (captured.size() >= 16) {
      uint64_t len;
      std::memcpy(&len, captured.data() + 8, 8);
      std::string path(reinterpret_cast<const char*>(captured.data() + 16), len);
  ...
  // Test 5: QueryReferrers request
  {
    ...
    auto captured = read_file("captures/queryreferrers_request.bin");
    if (captured.size() >= 16) {
      uint64_t len;
      std::memcpy(&len, captured.data() + 8, 8);
      std::string path(reinterpret_cast<const char*>(captured.data() + 16), len);
  ...
  // Test 6: AddTempRoot request
  {
    ...
    auto captured = read_file("captures/addtemproot_request.bin");
    if (captured.size() >= 16) {
      uint64_t len;
      std::memcpy(&len, captured.data() + 8, 8);
      std::string path(reinterpret_cast<const char*>(captured.data() + 16), len);
  ...
  // Test 7: AddIndirectRoot request
  {
    ...
    auto captured = read_file("captures/addindirectroot_request.bin");
    if (captured.size() >= 16) {
      uint64_t len;
      std::memcpy(&len, captured.data() + 8, 8);
      std::string path(reinterpret_cast<const char*>(captured.data() + 16), len);
  ...
  // Test 8: FindRoots request (no payload)
  {
    ...
    auto captured = read_file("captures/findroots_request.bin");
  ...
  // Test 9: NarFromPath request (synthetic)
  {
    ...
    auto captured = read_file("captures/narfrompath_request.bin");
    if (captured.size() >= 16) {
      uint64_t len;
      std::memcpy(&len, captured.data() + 8, 8);
      std::string path(reinterpret_cast<const char*>(captured.data() + 16), len);
  ...
  // Test 10: QueryMissing request
  {
    ...
    auto captured = read_file("captures/querymissing_request.bin");
    if (captured.size() >= 24) {
      ...
      size_t offset = 16;
      for (uint64_t i = 0; i < num_paths && offset < captured.size(); ++i) {
        uint64_t len;
  ...
  // Test 11: BuildPaths request
  {
    ...
    auto captured = read_file("captures/buildpaths_request.bin");
    if (captured.size() >= 24) {
      uint64_t num_paths;
      std::memcpy(&num_paths, captured.data() + 8, 8);
      ...
      std::vector<std::string> paths;
      size_t offset = 16;
      for (uint64_t i = 0; i < num_paths && offset < captured.size() - 8; ++i) {
        uint64_t len;
        std::memcpy(&len, captured.data() + offset, 8);
        paths.emplace_back(reinterpret_cast<const char*>(captured.data() + offset + 8), len);
        size_t padding = (8 - (len % 8)) % 8;
  ...
  // Test 12: BuildPathsWithResults request
  {
    ...
    auto captured = read_file("captures/buildpathswithresults_request.bin");
    if (captured.size() >= 24) {
      uint64_t num_paths;
      std::memcpy(&num_paths, captured.data() + 8, 8);
  ...
  // Test 13: SetOptions request
  {
    ...
    settings.keep_failed = false;
    settings.keep_going = false;
    settings.try_fallback = false;
    settings.verbosity = 3;
    settings.max_build_jobs = 32;
    settings.max_silent_time = 0;
    settings.use_build_hook = true;
    settings.verbose_build = 7;
    settings.log_type = 0;
    settings.print_build_trace = 0;
    settings.build_cores = 0;
    settings.use_substitutes = true;
    settings.overrides = {{"extra-platforms", "aarch64-linux"},
                          {"sandbox", "false"},
    ...
    auto captured = read_file("captures/setoptions_request.bin");
  ...
  // Test 14: AddToStoreNar request
  {
    ...
    req.path = "/nix/store/v4wqkf7dq9619yyv1wbf1jvpw4dhbs2f-tf.txt";
    req.deriver = ""; // none
    req.nar_hash = "60e5106c2a1d4ef9f82b58df02be7a482f35438ff9e6556194ab02e355256352";
    req.references = {};
    req.registration_time = 0;
    req.nar_size = 136;
    req.ultimate = false;
    req.signatures = {};
    req.ca = "fixed:sha256:081b9nklhcc0gck2a6j821xij57djvnfxg1fps0f5l2bbpd1sgp1";
    req.repair = false;
    req.dont_check_sigs = false;

    write_add_to_store_nar_request(w, req);

    auto captured = read_file("captures/addtostorenar_request.bin");
