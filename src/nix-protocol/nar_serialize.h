...
struct FsObject {
  ...
  std::variant<RegularFile, Symlink, Directory> data;

  bool operator==(const FsObject&) const = default;

  // Private constructor
  explicit FsObject(std::variant<RegularFile, Symlink, Directory> d) : data(std::move(d)) {}
  ...
  static FsObject file(std::string_view contents) {
    return FsObject{
        RegularFile{.contents = std::vector<std::byte>(
  ...
  static FsObject symlink(std::string_view target) {
    return FsObject{Symlink{.target = std::string(target)}};
  }
  ...
  static FsObject directory(std::vector<std::pair<std::string, FsObject>> entries) {
    ...
    return FsObject{Directory{.entries = std::move(entries)}};
  }
  ...
  std::visit(
      [&w](auto&& arg) {
        ...
        if constexpr (std::is_same_v<T, RegularFile>) {
          w.write_regular_file(arg.contents, arg.executable);
        } else if constexpr (std::is_same_v<T, Symlink>) {
          w.write_symlink(arg.target);
        } else if constexpr (std::is_same_v<T, Directory>) {
          w.begin_directory();
          for (const auto& [name, child] : arg.entries) {
            w.begin_entry(name);
            child.write_node(w);
            w.end_entry();
          }
...
struct NarError {
  ...
  NarError(NarErrorKind k, std::string msg) : kind(k), message(std::move(msg)) {}
...
private:
  ...
  NarResult<FsObject> read_regular_file() {
    ...
    return FsObject{FsObject::RegularFile{.contents = std::move(get_value(contents_result)),
  ...
  NarResult<FsObject> read_directory() {
    ...
    return FsObject{FsObject::Directory{.entries = std::move(entries)}};
  }
  ...
  NarResult<FsObject> read_symlink() {
    ...
    return FsObject{FsObject::Symlink{.target = std::move(get_value(target_result))}};
  }
};
