...
bool compare(std::span<const std::byte> generated, std::span<const std::byte> expected,
  ...
  for (size_t i = 0; i < generated.size(); ++i) {
    if (generated[i] != expected[i]) {
      std::cerr << "FAIL " << name << ": byte mismatch at offset 0x" << std::hex << i << " (got 0x"
...
int main() {
  ...
  // Content is "hello world\n" (12 bytes including newline)
  {
    ...
    auto expected = read_file("nar_captures/regular_file.nar");
  ...
  // Captured: echo "#!/bin/bash" > script && chmod +x script && nix-store --dump script
  {
    ...
    auto expected = read_file("nar_captures/executable_file.nar");
  ...
  // Captured: ln -s /etc/passwd link && nix-store --dump link
  {
    ...
    auto expected = read_file("nar_captures/symlink.nar");
  ...
  // Captured: touch empty && nix-store --dump empty
  {
    ...
    auto expected = read_file("nar_captures/empty_file.nar");
  ...
  // Captured: mkdir empty_dir && nix-store --dump empty_dir
  {
    ...
    auto expected = read_file("nar_captures/empty_directory.nar");
  ...
  //   nix-store --dump dir
  {
    ...
    auto expected = read_file("nar_captures/directory.nar");
  ...
  // Test 8: NAR is deterministic
  {
    ...
    for (int i = 0; i < 10; ++i) {
      ...
      if (buf1 != buf2) {
        std::cerr << "FAIL determinism: iteration " << i << " produced different output\n";
        deterministic = false;
      }
    }
    if (deterministic) {
  ...
  for (const char* name : nar_files) {
    std::string path = std::string("nar_captures/") + name;
    ...
    if (nar::is_error(parse_result)) {
      std::cerr << "FAIL " << name << ": parse error: " << nar::get_error(parse_result).message
                << "\n";
      ++failed;
      continue;
    }
