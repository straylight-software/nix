...
pub mod wire {
  use super::*;
  use crate::broker::{BrokerError, Operation, OperationResult, SeqNo};
  ...
  pub fn encode_request(request: &Request) -> Vec<u8> {
    ...
    buf.extend_from_slice(unsafe {
      std::slice::from_raw_parts(&header as *const _ as *const u8, RequestHeader::SIZE)
    });
  ...
  pub fn decode_request(buf: &[u8]) -> Option<Request> {
    ...
    let header: RequestHeader = unsafe { std::ptr::read(buf.as_ptr() as *const RequestHeader) };
  ...
  fn decode_operation(op_type: u32, payload: &[u8]) -> Option<Operation> {
    match op_type {
      ...
      2 if payload.len() >= 16 => Some(Operation::Alloc {
      ...
      6 if payload.len() >= 12 => Some(Operation::UnmapMemory {
        h_client: u32::from_le_bytes(payload[0..4].try_into().ok()?),
        h_device: u32::from_le_bytes(payload[4..8].try_into().ok()?),
        h_memory: u32::from_le_bytes(payload[8..12].try_into().ok()?),
      }),
      ...
      10 if payload.len() >= 12 => Some(Operation::AllocOsEvent {
        h_client: u32::from_le_bytes(payload[0..4].try_into().ok()?),
        h_device: u32::from_le_bytes(payload[4..8].try_into().ok()?),
        fd: u32::from_le_bytes(payload[8..12].try_into().ok()?),
      }),
      11 if payload.len() >= 12 => Some(Operation::FreeOsEvent {
        h_client: u32::from_le_bytes(payload[0..4].try_into().ok()?),
        h_device: u32::from_le_bytes(payload[4..8].try_into().ok()?),
        fd: u32::from_le_bytes(payload[8..12].try_into().ok()?),
      }),
      12 if payload.len() >= 4 => {
        ...
        for i in 0..count {
          ...
          if payload.len() >= offset + 4 {
            gpu_ids.push(u32::from_le_bytes(
      ...
      13 if payload.len() >= 7 => Some(Operation::StatusCode {
