...
impl NvmlServer {
  ...
  fn handle_client(&self, stream: &mut UnixStream) -> io::Result<()> {
    ...
    let request = unsafe { std::ptr::read_unaligned(req_buf.as_ptr() as *const NvmlQueryRequest) };
  ...
  fn build_gpu_info_response(&self) -> NvmlGpuInfoResponse {
    ...
    response.memory_free = self
      .gpu_info
      .config
      .fb_size
      .saturating_sub(self.gpu_info.state.memory_used);
...
mod tests {
  ...
  fn test_nvml_server_response() {
    ...
    let response =
      unsafe { std::ptr::read_unaligned(response_buf.as_ptr() as *const NvmlGpuInfoResponse) };
