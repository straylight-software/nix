...
pub enum Operation {
  ...
  CheckVersion { cmd: u32, version_string: Vec<u8> },
  ...
  AttachGpusToFd { gpu_ids: Vec<u32> },
...
pub enum OperationResult {
  ...
  CardInfoResult { num_gpus: u32, card_info: Vec<u8> },

  /// Version check completed
  VersionCheckResult { reply: u32, version_string: Vec<u8> },
...
impl BrokerState {
  ...
  fn op_check_version(
    ...
    use crate::rm_api::{NV_RM_API_VERSION_CMD_QUERY, NV_RM_API_VERSION_REPLY_RECOGNIZED};
...
mod proptests {
  ...
  fn operation(client: ClientId) -> impl Strategy<Value = Operation> {
    prop_oneof![
      ...
      (handle(), handle(), handle(), 0x41u32..0x50).prop_map(|(r, p, n, c)| Operation::Alloc {
        h_root: r,
        h_parent: p,
        h_new: n,
        class: c
