...
impl UringEventLoop {
  ...
  fn submit_poll(&mut self, index: u32) -> io::Result<()> {
    let reg = self
      .fds
      .get_mut(&index)
      .ok_or_else(|| io::Error::new(io::ErrorKind::NotFound, "client not registered"))?;

    if reg.poll_pending {
      return Ok(()); // Already polling
    }

    let poll_e = opcode::PollAdd::new(types::Fd(reg.fd), libc::POLLIN as _)
      .build()
...
mod tests {
  ...
  fn test_event_loop_basic() {
    ...
    assert!(
      events
        .iter()
        .any(|e| matches!(e, Event::ClientReady { client_index: 0 }))
    );
    ...
    assert!(
      events
        .iter()
        .any(|e| matches!(e, Event::ClientReady { client_index: 1 }))
    );
  }
  ...
  fn test_run_once() {
    ...
    let processed = event_loop
      .run_once(Some(100), |client_index| {
