  ...
  match op_type {
    ...
    3 => {
      ...
      if payload.len() >= 8 {
        ...
        Some(Operation::Free { h_root, h_object })
      } else {
        None
      }
    }
...
impl<D: Driver + Send + 'static> VsockServer<D> {
  ...
  fn create_vsock_listener(&self) -> io::Result<RawFd> {
    use libc::{
      AF_VSOCK, SO_REUSEADDR, SOCK_STREAM, SOL_SOCKET, VMADDR_CID_ANY, bind, listen, setsockopt,
      sockaddr_vm, socket,
    };
...
mod tests {
  ...
  fn test_encode_allocated() {
    let response = Response {
      client: ClientId(1),
      seq: SeqNo(0),
      result: Ok(OperationResult::Allocated {
        real_handle: RealHandle(0x1234),
      }),
    };
