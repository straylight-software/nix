...
use crate::uring_transport::{
  ClientConnectionInfo, ENTRY_SIZE, RingEntry, RingHeader, SharedMemoryLayout,
};
...
impl GuestShim {
  ...
  fn send_request(&mut self, request: Request) -> io::Result<()> {
    let shmem = self
      .shmem
      .as_mut()
      .ok_or_else(|| io::Error::new(io::ErrorKind::NotConnected, "Not connected to broker"))?;
    ...
    let idx = header
      .producer_reserve()
      .ok_or_else(|| io::Error::new(io::ErrorKind::WouldBlock, "Request ring full"))?;
  ...
  fn try_recv_response(&mut self) -> io::Result<Option<Response>> {
    ...
    let response = decode_response(data)
      .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "Failed to decode response"))?;
  ...
  fn extract_size(&self, params: &[u8]) -> u64 {
    if params.len() >= 32 {
      u64::from_le_bytes([
        params[24], params[25], params[26], params[27], params[28], params[29], params[30],
        params[31],
      ])
    } else {
      0
...
fn decode_response(data: &[u8]) -> Option<Response> {
  ...
  let result = if success == 1 {
    ...
    Err(BrokerError::InvalidOperation(
      "Broker returned error".to_string(),
    ))
  };
...
impl GuestSharedMemory {
  ...
  pub fn request_ring_header(&self) -> &RingHeader {
    unsafe { &*(self.ptr.add(self.layout.request_header_offset) as *const RingHeader) }
  }

  /// Get response ring header (guest is consumer)
  pub fn response_ring_header(&self) -> &RingHeader {
    unsafe { &*(self.ptr.add(self.layout.response_header_offset) as *const RingHeader) }
  }
...
mod tests {
  ...
  fn test_ioctl_number_build() {
    ...
    let cmd = IoctlRequest::build_ioctl_nr(NvEscape::Control, 32, IoctlDirection::ReadWrite);
