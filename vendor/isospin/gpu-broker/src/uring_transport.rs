...
impl SharedMemoryRegion {
  ...
  fn request_ring_header_mut(&mut self) -> &mut RingHeader {
    unsafe { &mut *(self.ptr.add(self.layout.request_header_offset) as *mut RingHeader) }
  }
  ...
  fn response_ring_header_mut(&mut self) -> &mut RingHeader {
    unsafe { &mut *(self.ptr.add(self.layout.response_header_offset) as *mut RingHeader) }
  }
  ...
  match &response.result {
    Ok(result) => {
      ...
      match result {
        ...
        OperationResult::CardInfoResult {
          num_gpus,
          card_info,
        } => {
        ...
        OperationResult::VersionCheckResult {
          reply,
          version_string,
        } => {
