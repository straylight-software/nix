...
impl Env {
  pub fn new(
    ...
    let chroot_base = arguments
      ...
      .ok_or_else(|| JailerError::ArgumentParsing(MissingValue("chroot-base-dir".to_string())))?;
    ...
    let cgroup_ver = arguments
      ...
      .ok_or_else(|| JailerError::ArgumentParsing(MissingValue("cgroup-version".to_string())))?;
  ...
  fn save_exec_file_pid(&mut self, pid: i32, chroot_exec_file: PathBuf) -> Result<(), JailerError> {
    let chroot_exec_file_str = chroot_exec_file
      .to_str()
      .ok_or_else(|| JailerError::ExtractFileName(chroot_exec_file.clone()))?;
    let pid_file_path = PathBuf::from(format!("{}{}", chroot_exec_file_str, PID_FILE_EXTENSION));
  ...
  fn join_netns(path: &str) -> Result<(), JailerError> {
    ...
    let netns = File::open(path).map_err(|err| JailerError::FileOpen(PathBuf::from(path), err))?;
  ...
  fn copy_cache_info(&self) -> Result<(), JailerError> {
    ...
    let jailer_cache_dir = Path::new(self.chroot_dir()).join("sys/devices/system/cpu/cpu0/cache/");
...
mod tests {
  ...
  fn create_env(mock_proc_mounts: &Path) -> Env {
    ...
    args
      .parse(&make_args(&ArgVals::new(pseudo_exec_file_path.as_str())))
      .unwrap();
  ...
  fn test_new_env() {
    ...
    let another_good_env = Env::new(&args, 0, 0, mock_cgroups.proc_mounts_path.to_str().unwrap())
    ...
    args = arg_parser.arguments().clone();
    args
      .parse(&make_args(&inexistent_exec_file_arg_vals))
      .unwrap();
    Env::new(&args, 0, 0, mock_cgroups.proc_mounts_path.to_str().unwrap()).unwrap_err();
  ...
  #[test]
  #[ignore] // Requires root for chroot/mount operations
  fn test_setup_jailed_folder() {
    ...
    assert_eq!(
      ...
      format!("Failed to create directory \\0foo\\0: file name contained an unexpected NUL byte")
    );
  ...
  #[test]
  #[ignore] // Requires root for mknod and chown
  ...
  fn test_copy_exec_to_chroot() {
    ...
    let mut env = Env::new(&args, 0, 0, mock_cgroups.proc_mounts_path.to_str().unwrap()).unwrap();
  ...
  fn test_parse_resource_limits() {
    ...
    for val in invalid_values.iter() {
      ...
      assert_eq!(
        ...
        format!(
          "{:?}",
          JailerError::ResLimitValue(val.to_string(), "invalid digit found in string".to_string())
        )
      );
    }
  ...
  fn test_save_exec_file_pid() {
    ...
    env
      .save_exec_file_pid(pid, PathBuf::from(exec_file_name))
      .unwrap();
