...
use crate::pdu::tcp::{Flags as TcpFlags, TcpError as TcpSegmentError, TcpSegment};
use crate::tcp::{
...
impl Connection {
  ...
  pub fn control_segment_or_timeout_status(&self) -> NextSegmentStatus {
    if self.synack_pending() || self.rst_pending() || self.can_send_first_fin() || self.pending_ack
    {
  ...
  pub fn write_next_segment<'a, R: ByteBuffer + ?Sized + Debug>(
    ...
    if let Some((read_buf, payload_seq)) = payload_src {
      ...
      let seq_to_send = if self.highest_ack_received != self.first_not_sent && self.rto_expired(now)
      {
        self.rto_count += 1;
        if self.rto_count >= self.rto_count_max {
          self.reset();
...
pub(crate) mod tests {
  ...
  fn check_established(c: &Connection) {
    assert_eq!(
      c.status_flags,
      ConnStatusFlags::SYN_RECEIVED | ConnStatusFlags::SYNACK_SENT | ConnStatusFlags::ESTABLISHED
    );
  }
  ...
  fn test_connection() {
    ...
    data
      .set_flags_after_ns(TcpFlags::ACK | TcpFlags::SYN)
    ...
    ctrl
      .set_ack_number(conn_isn.wrapping_add(1 + u32::from(mss)))
      .set_flags_after_ns(TcpFlags::ACK);
    assert_eq!(
      t.receive_segment(&mut c, &ctrl).unwrap(),
      (None, RecvStatusFlags::empty())
    ...
    ctrl
      .set_flags_after_ns(TcpFlags::FIN)
      .set_sequence_number(c.ack_to_send.0.wrapping_sub(1));
    assert_eq!(
      t.receive_segment(&mut c, &ctrl).unwrap(),
