...
use crate::pdu::ipv4::{IPv4Packet, Ipv4Error as IPv4PacketError, PROTOCOL_TCP};
...
impl TcpIPv4Handler {
  ...
  pub fn receive_packet<T: NetworkBytes + Debug, F: FnOnce(Request) -> Response>(
    ...
    let outcome = if let Some(endpoint) = self.connections.get_mut(&tuple) {
      ...
      RecvSegmentOutcome::UnexpectedSegment(!segment.flags_after_ns().intersects(TcpFlags::RST))
    };
...
mod tests {
  ...
  fn test_handler() {
    ...
    // conditions first.
    let mut p =
      IPv4Packet::write_header(buf.as_mut(), PROTOCOL_TCP, remote_addr, wrong_local_addr).unwrap();
