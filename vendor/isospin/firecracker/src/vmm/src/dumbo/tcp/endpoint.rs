...
use crate::tcp::{MAX_WINDOW_SIZE, NextSegmentStatus, seq_after};
...
impl Endpoint {
  ...
  pub fn receive_segment<T: NetworkBytes + Debug, F: FnOnce(Request) -> Response>(
    ...
    let (value, status) =
      match self
        .connection
        .receive_segment(s, &mut self.receive_buf[self.receive_buf_left..], now)
      {
        Ok(pair) => pair,
        Err(_) => {
          // Metrics: rx_accepted_err
          return;
        }
      };

    if !status.is_empty() {
      // Metrics: rx_accepted_unusual
    ...
    if !self.response_buf.is_empty()
      && self.connection.highest_ack_received()
        == self.initial_response_seq + Wrapping(u32::try_from(self.response_buf.len()).unwrap())
    {
  ...
  pub fn write_next_segment<'a>(
    ...
    match self
      .connection
      .write_next_segment(buf, mss_reserved, tcp_payload_src, timestamp_cycles())
    {
  ...
  pub fn is_evictable(&self) -> bool {
    timestamp_cycles().wrapping_sub(self.last_segment_received_timestamp) > self.eviction_threshold
  }
...
fn parse_request_bytes<F: FnOnce(Request) -> Response>(
  ...
  match request {
    ...
    Err(err) => match err {
      ...
      RequestError::InvalidHttpVersion(err_msg) | RequestError::InvalidHttpMethod(err_msg) => {
...
mod tests {
  ...
  fn test_endpoint() {
    ...
    for i in 1..=max_iter {
      ...
      remote_first_not_sent = remote_first_not_sent.wrapping_add(request.len().try_into().unwrap());
