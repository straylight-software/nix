...
impl<T: NetworkBytesMut + Debug> TcpSegment<'_, T> {
  ...
  pub fn write_incomplete_segment<R: ByteBuffer + ?Sized + Debug>(
    ...
    let payload_bytes_count = if let Some((payload_buf, max_payload_bytes)) = payload {
      ...
      room_for_payload = u16::try_from(min(usize::from(room_for_payload), left_to_read)).unwrap();
      ...
      payload_buf.read_to_slice(
        0,
        &mut segment.bytes[usize::from(segment_len)..usize::from(segment_len + room_for_payload)],
      );
      room_for_payload
    } else {
      0
...
mod tests {
  ...
  fn test_constructors() {
    ...
    {
      let segment =
        TcpSegment::from_bytes(&a[..segment_len.into()], Some((src_addr, dst_addr))).unwrap();
      assert_eq!(
