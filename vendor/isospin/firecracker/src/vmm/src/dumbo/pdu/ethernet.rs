...
mod tests {
  ...
  fn test_ethernet_frame() {
    ...
    assert_eq!(
      EthernetFrame::new_with_header(bad_array.as_mut(), dst_mac, src_mac, ethertype).unwrap_err(),
      EthernetError::SliceTooShort
    );

    {
      let mut f1 = EthernetFrame::new_with_header(a.as_mut(), dst_mac, src_mac, ethertype).unwrap();
    ...
    {
      let f3 = EthernetFrame::write_incomplete(a.as_mut(), dst_mac, src_mac, ethertype).unwrap();
...
mod kani_proofs {
  use super::*;
  use vmm_utils::net::mac::MAC_ADDR_LEN;
  ...
  fn verify_from_bytes_unchecked() {
    ...
    assert!(!(ethernet.is_valid()) || (ethernet.payload().len() == slice_length - PAYLOAD_OFFSET));
  }
