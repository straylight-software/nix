    ...
    self
      ...
      .map_err(|err| match err {
        data_store::MmdsDatastoreError::DataStoreLimitExceeded => {
          VmmActionError::MmdsLimitExceeded(data_store::MmdsDatastoreError::DataStoreLimitExceeded)
        }
...
impl<'a> PrebootApiController<'a> {
  ...
  pub fn handle_preboot_request(&mut self, request: VmmAction) -> Result<VmmData, VmmActionError> {
    use self::VmmAction::*;
    ...
    match request {
      ...
      PutCpuConfiguration(custom_cpu_template) => self.set_custom_cpu_template(custom_cpu_template),
  ...
  fn insert_net_device(&mut self, cfg: NetworkInterfaceConfig) -> Result<VmmData, VmmActionError> {
    self.boot_path = true;
    self
      .vm_resources
  ...
  fn update_machine_config(&mut self, cfg: MachineConfigUpdate) -> Result<VmmData, VmmActionError> {
    self.boot_path = true;
    self
      .vm_resources
...
impl RuntimeApiController {
  ...
  fn update_block_device(
    ...
    if new_cfg.path_on_host.is_none() && new_cfg.rate_limiter.is_none() {
      vmm
        .update_vhost_user_block_config(&new_cfg.drive_id)
...
mod tests {
  ...
  fn test_preboot_patch_mmds() {
    ...
    assert_eq!(
      preboot_request_with_mmds(
        VmmAction::PutMMDS(serde_json::from_str(r#"{"key1": "value1", "key2": "val2"}"#).unwrap(),),
        mmds.clone()
      )
      .unwrap(),
      VmmData::Empty
    ...
    assert_eq!(
      preboot_request_with_mmds(VmmAction::GetMMDS, mmds.clone()).unwrap(),
      VmmData::MmdsValue(serde_json::from_str(r#"{"key1": "value1", "key2": "val2"}"#).unwrap())
    );

    assert_eq!(
      preboot_request_with_mmds(
        VmmAction::PatchMMDS(serde_json::from_str(r#"{"key1": null, "key2": "value2"}"#).unwrap(),),
        mmds.clone()
      )
      .unwrap(),
      VmmData::Empty
