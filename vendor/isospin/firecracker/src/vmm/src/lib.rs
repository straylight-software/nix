...
/// (re-exported from vmm_io_uring crate)
pub use vmm_io_uring as io_uring;
...
/// (re-exported from vmm_rate_limiter crate)
pub use vmm_rate_limiter as rate_limiter;
...
#[allow(missing_docs)]
pub mod devices;
/// minimalist HTTP/TCP/IPv4 stack named DUMBO (re-exported from vmm_dumbo crate)
pub use vmm_dumbo as dumbo;
...
/// Serialization and deserialization facilities (re-exported from vmm_snapshot crate)
pub use vmm_snapshot as snapshot;
...
use crate::vstate::memory::{GuestMemory, GuestMemoryBackend, GuestMemoryMmap, GuestMemoryRegion};
