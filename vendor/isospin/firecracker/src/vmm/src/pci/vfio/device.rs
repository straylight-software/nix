...
impl VfioPciDevice {
  ...
  fn get_region_offset(fd: RawFd, region_index: u32) -> VfioResult<u64> {
    ...
    let ret = unsafe { libc::ioctl(fd, VFIO_DEVICE_GET_REGION_INFO as _, &mut info) };
    if ret < 0 {
  ...
  fn find_interrupt_capabilities(&mut self) -> VfioResult<()> {
    ...
    log::info!(
      "find_interrupt_capabilities: cap_ptr at offset {:#x}",
      offset
    );
    ...
    while offset != 0 && offset < 0x100 {
      ...
      log::info!(
        "find_interrupt_capabilities: cap at {:#x}: id={:#x}, next={:#x}",
        offset,
        cap_id,
        next_cap
      );
  ...
  fn find_ltr_extended_capability(&mut self) -> VfioResult<()> {
    ...
    while offset != 0 && offset >= 0x100 && offset < 0x1000 && iterations < 50 {
      ...
      if next_offset != 0 && next_offset <= offset {
        log::warn!(
          "Extended cap chain loop detected: {:#x} -> {:#x}",
          offset,
          next_offset
        );
        break;
    ...
    if self.ltr_ext_cap_offset.is_none() {
      log::info!(
        "LTR Extended Capability not found (scanned {} caps)",
        iterations
      );
    }
  ...
  pub fn write_msix_table(&mut self, bar_offset: u64, data: &[u8]) -> Option<u16> {
    ...
    match entry_offset {
      ...
      _ => {
        ...
        log::debug!(
          "Partial MSI-X table write at offset {}, len {}",
          entry_offset,
          data.len()
        );
      }
  ...
  fn update_msix_routing(&self, vector_index: u16) {
    let Some(msix) = &self.msix_state else { return };
    let Some(vectors) = &self.msix_vectors else {
      return;
    };
    ...
    if let Err(e) = vectors.update(vector_index as usize, config, masked, true) {
      log::error!(
        "Failed to update MSI-X routing for vector {}: {:?}",
        vector_index,
        e
      );
    } else {
  ...
  fn update_all_msix_routing(&self) {
    ...
    if msix.enabled && !msix.function_masked {
      log::info!(
        "Enabling MSI-X routing for {} vectors",
        msix.table_entries.len()
      );
  ...
  pub fn map_mmio_regions(&mut self, vm: &crate::vstate::vm::Vm) -> VfioResult<()> {
    ...
    for bar_idx in 0..6 {
      ...
      let slot = vm.next_kvm_slot(1).ok_or(VfioError::NoKvmSlotAvailable {
        bar: bar_info.index,
      })?;
  ...
  pub fn reset(&self) -> VfioResult<()> {
    let ret = unsafe { libc::ioctl(self.fd.as_raw_fd(), VFIO_DEVICE_RESET as _) };
    if ret < 0 {
  ...
  pub fn setup_msix_irqs(&self, eventfds: &[RawFd]) -> VfioResult<()> {
    ...
    let table_size = self
      .msix_table_size
      .ok_or(VfioError::NoMsixCapability { bdf: self.bdf })?;
    ...
    // Call ioctl
    let ret = unsafe { libc::ioctl(self.fd.as_raw_fd(), VFIO_DEVICE_SET_IRQS as _, buf.as_ptr()) };
  ...
  pub fn disable_msix_irqs(&self) -> VfioResult<()> {
    ...
    let ret = unsafe { libc::ioctl(self.fd.as_raw_fd(), VFIO_DEVICE_SET_IRQS as _, &header) };
  ...
  pub fn setup_msi_irqs(&mut self, eventfds: &[RawFd]) -> VfioResult<()> {
    ...
    let max_vectors = self
      .msi_max_vectors()
      .ok_or(VfioError::NoMsiCapability { bdf: self.bdf })?;
  ...
  fn update_msi_routing(&self) {
    ...
    let Some(vectors) = &self.msi_vectors else {
      return;
    };
    ...
    for idx in 0..num_vectors {
      ...
      if let Err(e) = vectors.update(idx, config, masked, true) {
        ...
        log::debug!(
          ...
          msi.cap.msg_addr_hi,
          msi.cap.msg_addr_lo,
          msi.cap.msg_data as u32 + idx as u32,
          masked
        );
      }
    }
  ...
  fn enable_all_msi_routing(&self) {
    ...
    if msi.cap.is_enabled() {
      log::info!(
        "Enabling MSI routing for {} vectors",
        msi.cap.num_enabled_vectors()
      );
  ...
  fn read_config_register(&mut self, reg_idx: usize) -> u32 {
    ...
    if let Some(ltr_offset) = self.ltr_ext_cap_offset {
      ...
      if offset == ltr_offset as u64 || offset == (ltr_offset + 4) as u64 {
        log::info!(
          "Hiding LTR Extended Capability read at offset {:#x}",
          offset
        );
        val = 0;
      }
  ...
  impl MockConfigSpace {
    ...
    pub fn msix_info(&self) -> Option<(u16, usize, usize)> {
      ...
      while cap_ptr != 0 && cap_ptr < 256 {
        ...
        if cap_id == 0x11 {
          // MSI-X capability
          let msg_ctrl = u16::from_le_bytes([self.data[cap_ptr + 2], self.data[cap_ptr + 3]]);
          let table_size = (msg_ctrl & 0x7ff) + 1;
