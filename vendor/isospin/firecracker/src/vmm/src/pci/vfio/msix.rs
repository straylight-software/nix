...
impl MsixTable {
  ...
  pub fn new(num_vectors: u16) -> Self {
    let vectors = (0..num_vectors).map(|_| MsixVectorState::Masked).collect();
  ...
  pub fn enable(&mut self, index: u16, eventfd: RawFd, gsi: u32) -> Result<(), MsixError> {
    let max = self.len();
    let state = self
      .vectors
      .get_mut(index as usize)
  ...
  pub fn iter(&self) -> impl Iterator<Item = (u16, &MsixVectorState)> {
    self.vectors.iter().enumerate().map(|(i, s)| (i as u16, s))
  }
}
...
mod tests {
  ...
  fn test_multiple_vectors() {
    ...
    for i in 0..5 {
      table
        .enable(i as u16, 100 + i as RawFd, 200 + i as u32)
        .unwrap();
    }
