...
impl MockKvmRoutingTable {
  ...
  pub fn register_msi(&mut self, route: MockMsiRoute) {
    ...
    self.operation_log.push(RoutingOperation::RegisterMsi {
      gsi: route.gsi,
      op_id,
    });
  }
  ...
  pub fn set_gsi_routes(&mut self) {
    ...
    self
      .operation_log
      .push(RoutingOperation::SetGsiRoutes { op_id });
  }
  ...
  pub fn enable_irqfd(&mut self, gsi: u32) {
    ...
    self
      .operation_log
      .push(RoutingOperation::EnableIrqfd { gsi, op_id });
  }
  ...
  pub fn disable_irqfd(&mut self, gsi: u32) {
    ...
    self
      .operation_log
      .push(RoutingOperation::DisableIrqfd { gsi, op_id });
  }
...
impl MockVfioDevice {
  ...
  pub fn can_deliver(&self, vector_idx: u16) -> bool {
    ...
    self.msix_state.can_deliver(vector_idx)
      && self
        .routing_table
        .is_enabled(self.vector_gsis[vector_idx as usize])
  }
}
...
fn msix_entry_strategy() -> impl Strategy<Value = MockMsixTableEntry> {
  (
    msi_address_strategy(),
    any::<u32>(),
    msi_data_strategy(),
    vector_ctrl_strategy(),
  )
...
mod regression_tests {
  ...
  fn test_nvidia_verify_interrupts_scenario() {
    ...
    assert!(
      device.can_deliver(0),
      "Vector 0 must be deliverable for osVerifyInterrupts"
    );
