...
impl VfioContainer {
  ...
  pub fn new() -> VfioResult<Self> {
    ...
    let has_type1 = unsafe { libc::ioctl(fd.as_raw_fd(), VFIO_CHECK_EXTENSION as _, 1) };
    if has_type1 <= 0 {
  ...
  pub fn set_iommu_type(&mut self) -> VfioResult<()> {
    ...
    let ret = unsafe { libc::ioctl(self.fd.as_raw_fd(), VFIO_SET_IOMMU as _, 3) };
    if ret < 0 {
  ...
  pub fn map_dma(&mut self, iova: IovaAddr, size: u64, hva: HostVirtAddr) -> VfioResult<()> {
    ...
    let ret = unsafe { libc::ioctl(self.fd.as_raw_fd(), VFIO_IOMMU_MAP_DMA as _, &map) };
    if ret < 0 {
    ...
    self
      .mappings
      .insert(iova.raw(), DmaMapping { iova, size, hva });
  ...
  pub fn unmap_dma(&mut self, iova: IovaAddr, size: u64) -> VfioResult<()> {
    ...
    let ret = unsafe { libc::ioctl(self.fd.as_raw_fd(), VFIO_IOMMU_UNMAP_DMA as _, &unmap) };
    if ret < 0 {
