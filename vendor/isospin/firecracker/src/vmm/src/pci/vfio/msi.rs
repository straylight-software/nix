...
impl MsiCap {
  ...
  pub fn update(&mut self, offset: u64, data: &[u8]) -> bool {
    ...
    match data.len() {
      ...
      _ => {
        log::debug!(
          "MSI: unhandled {}-byte write at offset {:#x}",
          data.len(),
          offset
        );
      }
...
mod tests {
  ...
  fn test_msi_cap_size() {
    ...
    // 64-bit, no mask
    let cap = MsiCap {
      msg_ctl: MSI_CTL_64_BIT_CAPABLE,
      ..Default::default()
    };
    ...
    // 32-bit, with mask
    let cap = MsiCap {
      msg_ctl: MSI_CTL_PER_VECTOR_MASK,
      ..Default::default()
    };
    ...
    // 64-bit, with mask
    let cap = MsiCap {
      msg_ctl: MSI_CTL_64_BIT_CAPABLE | MSI_CTL_PER_VECTOR_MASK,
      ..Default::default()
    };
