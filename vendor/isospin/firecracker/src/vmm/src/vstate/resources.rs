...
impl ResourceAllocator {
  ...
  pub fn new() -> Self {
    ...
    // all allocators
    Self {
      gsi_legacy_allocator: IdAllocator::new(arch::GSI_LEGACY_START, arch::GSI_LEGACY_END).unwrap(),
      ...
      .unwrap(),
      system_memory: AddressAllocator::new(arch::SYSTEM_MEM_START, arch::SYSTEM_MEM_SIZE).unwrap(),
    }
  }
...
mod tests {
  ...
  fn clone_allocator(allocator: &ResourceAllocator) -> ResourceAllocator {
    ...
    Snapshot::new(SNAPSHOT_VERSION.clone(), allocator.save())
      .save(&mut buf.as_mut_slice())
      .unwrap();
    let restored_state: ResourceAllocator =
