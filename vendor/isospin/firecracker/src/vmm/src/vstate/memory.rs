...
pub use vm_memory::{
  Address, ByteValued, Bytes, FileOffset, GuestAddress, GuestMemory, GuestMemoryBackend,
  GuestMemoryRegion, GuestUsize, MemoryRegionAddress, MmapRegion, address,
};
...
impl GuestMemoryExtension for GuestMemoryMmap {
  ...
  fn mark_dirty(&self, addr: GuestAddress, len: usize) {
    ...
    // Use GuestMemoryBackend::get_slices explicitly to disambiguate
    for slice in GuestMemoryBackend::get_slices(self, addr, len).flatten() {
...
mod tests {
  ...
  fn test_snapshot_file_success() {
    for dirty_page_tracking in [true, false] {
      ...
      let guest_regions = snapshot_file(file, regions.into_iter(), dirty_page_tracking).unwrap();
  ...
  fn test_mark_dirty() {
    ...
    for (addr, len, dirty) in &dirty_map {
      for slice in
        GuestMemoryBackend::get_slices(&guest_memory, GuestAddress(*addr as u64), *len).flatten()
      {
  ...
  fn check_serde<M: GuestMemoryExtension>(guest_memory: &M) {
    ...
    Snapshot::new(SNAPSHOT_VERSION.clone(), &original_state)
      .save(&mut snapshot_data.as_mut_slice())
      .unwrap();
    let restored_state =
      Snapshot::load_without_crc_check(snapshot_data.as_slice(), &SNAPSHOT_VERSION)
        .unwrap()
        .data;
  ...
  fn test_describe() {
    ...
    let guest_memory =
      into_region_ext(anonymous(mem_regions.into_iter(), true, HugePageConfig::None).unwrap());
  ...
  fn test_discard_range() {
    ...
    mem
      .read(actual_page.as_mut_slice(), GuestAddress(0))
      .unwrap();
    assert_eq!(vec![0u8; page_size], actual_page);
    // Check that the second page still contains ones.
    mem
      .read(actual_page.as_mut_slice(), GuestAddress(page_size as u64))
      .unwrap();
    ...
    assert_match!(
      mem
        .discard_range(GuestAddress(0x20), page_size)
        .unwrap_err(),
