...
use crate::device_manager::{
  AttachDeviceError, DeviceManager, DeviceManagerCreateError, DevicePersistError, DeviceRestoreArgs,
};
...
pub fn build_microvm_for_boot(
  ...
  attach_vfio_devices(&mut device_manager, &vm, &vm_resources.vfio_devices)?;
...
pub fn build_microvm_from_snapshot(
  ...
  let kvm =
    Kvm::new(microvm_state.kvm_state.kvm_cap_modifiers.clone()).map_err(StartMicrovmError::Kvm)?;
  // Set up Kvm Vm and register memory regions.
  // Build custom CPU config if a custom template is provided.
  ...
  #[allow(unused_mut)]
  let mut device_manager = DeviceManager::restore(device_ctor_args, &microvm_state.device_states)?;
  ...
  fn test_attach_block_devices() {
    ...
    // Use case 4: rw root block device and other rw and ro drives.
    {
      let block_configs = vec![
        ...
        CustomBlockConfig::new(String::from("third"), false, None, false, CacheType::Unsafe),
      ];
      let mut vmm = default_vmm();
      let mut cmdline = default_kernel_cmdline();
      insert_block_devices(&mut vmm, &mut cmdline, &mut event_manager, block_configs);
