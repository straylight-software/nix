...
pub fn host_brand_string() -> [u8; BRAND_STRING_LENGTH] {
  ...
  let arr = [
    leaf_a.eax, leaf_a.ebx, leaf_a.ecx, leaf_a.edx, leaf_b.eax, leaf_b.ebx, leaf_b.ecx, leaf_b.edx,
    leaf_c.eax, leaf_c.ebx, leaf_c.ecx, leaf_c.edx,
  ];
  ...
  fn get_mut(&mut self, CpuidKey { leaf, subleaf }: &CpuidKey) -> Option<&mut CpuidEntry> {
    ...
    entry_opt.map(|entry| {
      ...
      unsafe {
        ...
        let arr2: &mut [u8; size_of::<CpuidEntry>()] = (&mut arr[8..28]).try_into().unwrap();
...
mod tests {
  ...
  fn test_invalid_kvmcpuid_to_cpuid() {
    ...
    let kvm_cpuid =
      kvm_bindings::CpuId::from_entries(&[kvm_bindings::kvm_cpuid_entry2::default()]).unwrap();
