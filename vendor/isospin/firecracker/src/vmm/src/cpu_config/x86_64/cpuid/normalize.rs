...
impl super::Cpuid {
  ...
  fn update_feature_info_entry(
    ...
    let max_cpus_per_package = u32::from(
      get_max_cpus_per_package(cpu_count).map_err(FeatureInformationError::GetMaxCpusPerPackage)?,
    );
...
mod tests {
  ...
  fn check_leaf_0xb_subleaf_0x1_added() {
    ...
    let result =
      intel_cpuid.update_extended_topology_entry(cpu_index, cpu_count, cpu_bits, cpus_per_core);
    result.unwrap();
