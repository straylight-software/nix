    ...
    match self {
      Some(template_type) => match template_type {
        ...
        CpuTemplateType::Static(template) => {
          ...
          if &get_vendor_id_from_host().map_err(GetCpuVendor)? != template.get_supported_vendor() {
  ...
  fn test_malformed_json() {
    ...
    assert!(
      cpu_template_result
        .unwrap_err()
        .to_string()
        .contains("Failed to parse string [0bx0?1_0_0x_?x1xxxx00xxx1xxxxxxxxxxx1] as a bitmap")
    );
