...
fn snapshot_state_to_file(
  ...
  let snapshot = Snapshot::new(SNAPSHOT_VERSION.clone(), microvm_state);
  ...
  let (guest_memory, uffd) = match params.mem_backend.backend_type {
    MemBackendType::File => {
      if vm_resources.machine_config.huge_pages.is_hugetlbfs() {
        return Err(
          RestoreFromSnapshotGuestMemoryError::File(GuestMemoryFromFileError::HugetlbfsSnapshot)
            .into(),
        );
      }
      (
...
fn snapshot_state_from_file(
  ...
  let snapshot = Snapshot::load(&mut snapshot_reader, &SNAPSHOT_VERSION)?;
...
fn guest_memory_from_uffd(
  ...
  for mem_region in guest_memory.iter() {
    uffd
      .register(mem_region.as_ptr().cast(), mem_region.size() as _)
...
mod tests {
  ...
  fn test_microvm_state_snapshot() {
    ...
    Snapshot::new(SNAPSHOT_VERSION.clone(), &microvm_state)
  ...
  fn test_create_guest_memory() {
    ...
    let (_, uffd_regions) = create_guest_memory(&mem_state, false, HugePageConfig::None).unwrap();
  ...
  fn test_send_uffd_handshake() {
    ...
    let deserialized: Vec<GuestRegionUffdMapping> = serde_json::from_slice(&message_buf).unwrap();
