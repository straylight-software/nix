...
impl SnapshotHdr {
  fn load(buf: &mut &[u8], current_version: &Version) -> Result<Self, SnapshotError> {
    ...
    if hdr.version.major != current_version.major || hdr.version.minor > current_version.minor {
...
impl<Data> Snapshot<Data> {
  /// Constructs a new snapshot with the given `data` and `version`.
...
impl<Data: DeserializeOwned> Snapshot<Data> {
  /// Loads a snapshot from a byte slice without performing CRC validation.
  ///
  /// This is useful for testing or when CRC validation is not needed.
  pub fn load_without_crc_check(
    mut buf: &[u8],
    current_version: &Version,
  ) -> Result<Self, SnapshotError> {
    let header = SnapshotHdr::load(&mut buf, current_version)?;
  ...
  ///
  /// The `current_version` is used to validate that the snapshot's version is compatible.
  pub fn load<R: Read>(reader: &mut R, current_version: &Version) -> Result<Self, SnapshotError> {
    // read_to_end internally right-sizes the buffer, so no reallocations due to growing buffers
    // will happen.
    let mut buf = Vec::new();
    reader.read_to_end(&mut buf)?;
    let snapshot = Self::load_without_crc_check(buf.as_slice(), current_version)?;
...
mod tests {
  ...
  /// Test version constant for snapshot tests
  const TEST_VERSION: Version = Version::new(8, 0, 0);

  #[test]
  fn test_parse_version_from_file() {
    let snapshot = Snapshot::new(TEST_VERSION.clone(), 42);
  ...
  fn test_bad_reader() {
    ...
    assert!(
      matches!(Snapshot::<()>::load(&mut reader, &TEST_VERSION), Err(SnapshotError::Io(inner)) if inner.kind() == std::io::ErrorKind::InvalidInput)
    );
  }
  ...
  fn test_bad_magic() {
    ...
    let snapshot = Snapshot::new(TEST_VERSION.clone(), ());
    ...
    data[7] = 0x45;
    assert!(matches!(
      SnapshotHdr::load(&mut data.as_slice(), &TEST_VERSION),
  ...
  fn test_bad_crc() {
    ...
    assert!(matches!(
      Snapshot::<()>::load(&mut std::io::Cursor::new(data.as_slice()), &TEST_VERSION),
  ...
  fn test_bad_version() {
    ...
    let mut snapshot = Snapshot::new(TEST_VERSION.clone(), ());
    snapshot.header.version.major = TEST_VERSION.major + 1;
    snapshot.save(&mut data.as_mut_slice()).unwrap();

    assert!(matches!(
        Snapshot::<()>::load_without_crc_check(data.as_slice(), &TEST_VERSION),
        Err(SnapshotError::InvalidFormatVersion(v)) if v.major == TEST_VERSION.major + 1
    ));

    //  minor > TEST_VERSION.minor: shouldn't work
    let mut snapshot = Snapshot::new(TEST_VERSION.clone(), ());
    snapshot.header.version.minor = TEST_VERSION.minor + 1;
    snapshot.save(&mut data.as_mut_slice()).unwrap();
    assert!(matches!(
        Snapshot::<()>::load_without_crc_check(data.as_slice(), &TEST_VERSION),
        Err(SnapshotError::InvalidFormatVersion(v)) if v.minor == TEST_VERSION.minor + 1
    ));
    ...
    let snapshot = Snapshot::new(TEST_VERSION.clone(), ());
    snapshot.save(&mut data.as_mut_slice()).unwrap();
    Snapshot::<()>::load_without_crc_check(data.as_slice(), &TEST_VERSION).unwrap();

    // Test with minor version 1 to test the "minor - 1" case
    let test_version_minor_1 = Version::new(8, 1, 0);
    let mut snapshot = Snapshot::new(test_version_minor_1.clone(), ());
    snapshot.header.version.minor = test_version_minor_1.minor - 1;
    snapshot.save(&mut data.as_mut_slice()).unwrap();
    Snapshot::<()>::load_without_crc_check(data.as_slice(), &test_version_minor_1).unwrap();
    ...
    let mut snapshot = Snapshot::new(TEST_VERSION.clone(), ());
    snapshot.header.version.patch = TEST_VERSION.patch + 1;
    snapshot.save(&mut data.as_mut_slice()).unwrap();
    Snapshot::<()>::load_without_crc_check(data.as_slice(), &TEST_VERSION).unwrap();
