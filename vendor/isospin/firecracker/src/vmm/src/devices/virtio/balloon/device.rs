...
use super::{
  ...
  FREE_PAGE_HINT_STOP, INFLATE_INDEX, MAX_PAGE_COMPACT_BUFFER, MAX_PAGES_IN_DESC, MIB_TO_4K_PAGES,
  STATS_INDEX, VIRTIO_BALLOON_F_DEFLATE_ON_OOM, VIRTIO_BALLOON_F_FREE_PAGE_HINTING,
  VIRTIO_BALLOON_F_FREE_PAGE_REPORTING, VIRTIO_BALLOON_F_STATS_VQ, VIRTIO_BALLOON_PFN_SHIFT,
  ...
  pub(crate) fn process_inflate(&mut self) -> Result<(), BalloonError> {
    ...
    while valid_descs_found {
      ...
      for (page_frame_number, range_len) in page_ranges {
        let guest_addr = GuestAddress(u64::from(page_frame_number) << VIRTIO_BALLOON_PFN_SHIFT);
...
pub(crate) mod tests {
  ...
  fn test_stats() {
    ...
    // Happy case.
    {
      ...
      mem
        .write_obj::<BalloonStat>(swap_out_stat, GuestAddress(page_addr))
        .unwrap();
      mem
        .write_obj::<BalloonStat>(mem_free_stat, GuestAddress(page_addr + SIZE_OF_STAT as u64))
        .unwrap();
      ...
      check_metric_after_block!(METRICS.event_fails, 0, {
        ...
        assert!(
          balloon
            .interrupt_trigger()
            .has_pending_interrupt(VirtioInterruptType::Queue(STATS_INDEX.try_into().unwrap()))
        );
      });
    }
  }
