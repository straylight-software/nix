    ...
    match offset {
      ...
      0x04 => {
        ...
        if self.device_feature_select < 2 {
          ((locked_device.avail_features() >> (self.device_feature_select * 32)) & 0xffff_ffff)
            as u32
        } else {
          0
        }
      ...
      0x20 => {
        let locked_device = device.lock().unwrap();
        self
          .with_queue(locked_device.queues(), |q| {
...
mod tests {
  ...
  fn test_bad_width_reads() {
    ...
    // 64-bit fields
    device.lock().unwrap().queues_mut()[0].desc_table_address = GuestAddress(0x0000_1312_0000_1110);
