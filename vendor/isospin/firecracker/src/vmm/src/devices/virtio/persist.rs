...
mod tests {
  ...
  fn test_queue_persistence() {
    ...
    Snapshot::new(SNAPSHOT_VERSION.clone(), queue.save())
    ...
    let restored_queue = Queue::restore(
      ca,
      &Snapshot::load_without_crc_check(bytes.as_slice(), &SNAPSHOT_VERSION)
        .unwrap()
        .data,
    )
    .unwrap();
  ...
  fn test_virtio_device_state_serde() {
    ...
    Snapshot::new(SNAPSHOT_VERSION.clone(), &state)
      .save(&mut mem.as_mut_slice())
      .unwrap();

    let restored_state: VirtioDeviceState =
  ...
  fn generic_mmiotransport_persistence_test(
    ...
    Snapshot::new(SNAPSHOT_VERSION.clone(), mmio_transport.save())
  ...
  fn create_default_block() -> (
    ...
    let mmio_transport = MmioTransport::new(mem.clone(), interrupt.clone(), block.clone(), false);
  ...
  fn default_vsock() -> (
    ...
    let mmio_transport = MmioTransport::new(mem.clone(), interrupt.clone(), vsock.clone(), false);
