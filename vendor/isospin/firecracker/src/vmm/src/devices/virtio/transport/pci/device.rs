...
impl VirtioPciDevice {
  ...
  pub fn new_from_state(
    ...
    let pci_config =
      PciConfiguration::type0_from_state(state.pci_configuration_state, Some(msix_config.clone()));
  ...
  fn read_cap_pci_cfg(&mut self, offset: usize, mut data: &mut [u8]) {
    ...
    if offset < std::mem::size_of::<VirtioPciCap>() {
      if let Some(end) = offset.checked_add(data_len) {
        ...
        data
          .write_all(&cap_slice[offset..cmp::min(end, cap_len)])
          .unwrap();
      }
    } else {
  ...
  pub fn register_notification_ioevent(&self, vm: &Vm) -> Result<(), errno::Error> {
      ...
      .enumerate()
    {
      ...
      let io_addr = IoEventAddress::Mmio(notify_base + i as u64 * NOTIFY_OFF_MULTIPLIER as u64);
...
mod tests {
  ...
  use crate::devices::virtio::transport::pci::device::{
    ...
    NOTIFY_OFF_MULTIPLIER, PciVirtioSubclass, VirtioPciCap, VirtioPciCfgCap, VirtioPciNotifyCap,
  };
