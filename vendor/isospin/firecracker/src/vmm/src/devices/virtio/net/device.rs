...
impl Net {
  ...
  pub fn new(
    ...
    tap
      .set_vnet_hdr_size(vnet_hdr_size)
  ...
  fn read_from_mmds_or_tap(&mut self) -> Result<Option<u32>, NetError> {
    ...
    if let Some(ns) = self.mmds_ns.as_mut()
      && let Some(len) = ns.write_next_frame(frame_bytes_from_buf_mut(&mut self.rx_frame_buf)?)
    {
      let len = len.get();
  ...
  fn process_tx(&mut self) -> Result<(), DeviceError> {
    ...
    while let Some(head) = tx_queue.pop_or_enable_notification()? {
      ...
      if !Self::rate_limiter_consume_op(&mut self.tx_rate_limiter, u64::from(self.tx_buffer.len()))
      {
        tx_queue.undo_pop();
...
pub mod tests {
  ...
  use crate::devices::virtio::net::test_utils::{
    NetEvent, NetQueue, TapTrafficSimulator, default_net, if_index, inject_tap_tx_frame, set_mac,
  };
