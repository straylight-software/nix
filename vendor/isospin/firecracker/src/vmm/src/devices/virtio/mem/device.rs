...
impl VirtioMem {
  ...
  pub fn update_requested_size(&mut self, requested_size_mib: usize) -> Result<(), VirtioMemError> {
...
impl VirtioDevice for VirtioMem {
  ...
  fn activate(
    ...
    if (self.acked_features & (1 << VIRTIO_MEM_F_UNPLUGGED_INACCESSIBLE)) == 0 {
      error!("virtio-mem: VIRTIO_MEM_F_UNPLUGGED_INACCESSIBLE feature not acknowledged by guest");
...
mod tests {
  ...
  fn emulate_request(
    ...
    mem
      .read_obj::<virtio_mem::virtio_mem_resp>(th.desc_address(MEM_QUEUE, 1))
      .unwrap()
      .into()
  }
