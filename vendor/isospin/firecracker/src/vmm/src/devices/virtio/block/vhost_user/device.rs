...
impl<T: VhostUserHandleBackend> VhostUserBlockImpl<T> {
  pub fn new(config: VhostUserBlockConfig) -> Result<Self, VhostUserBlockError> {
    ...
    let config_space = if acked_protocol_features & VhostUserProtocolFeatures::CONFIG.bits() != 0 {
    ...
    let activate_evt = EventFd::new(libc::EFD_NONBLOCK).map_err(VhostUserBlockError::EventFd)?;
