    ...
    match !self.device_status & status {
      ...
      DRIVER_OK if self.device_status == (ACKNOWLEDGE | DRIVER | FEATURES_OK) => {
        ...
        if !device_activated {
          ...
          let activate_result = locked_device.activate(self.mem.clone(), self.interrupt.clone());
...
impl IrqTrigger {
  pub fn new() -> Self {
    Self {
      ...
      irq_evt: EventFd::new(libc::EFD_NONBLOCK).expect("Could not create EventFd for IrqTrigger"),
    }
  }
