...
use vm_memory::{Address, GuestMemory, GuestMemoryBackend, GuestMemoryError, GuestMemoryRegion};
    ...
    for (queue_index, queue, queue_evt) in queues.iter() {
      let config_data = VringConfigData {
        ...
        desc_table_addr: mem
          ...
          .map_err(VhostUserError::DescriptorTableAddress)? as u64,
        used_ring_addr: mem
          ...
          *queue_index,
          interrupt
            .notifier(VirtioInterruptType::Queue(
              (*queue_index)
                .try_into()
                .unwrap_or_else(|_| panic!("vhost-user: invalid queue index: {}", *queue_index)),
            ))
            .as_ref()
            .unwrap(),
        )
...
pub(crate) mod tests {
  ...
  fn test_new() {
    ...
    let vuh = VhostUserHandleImpl::<MockFrontend>::new(&tmp_socket_path, max_queue_num).unwrap();
    assert_eq!(
      vuh
        .vu
        .sock
  ...
  fn test_set_protocol_features() {
    ...
    vuh
      .set_protocol_features(acked_features, acked_protocol_features.bits())
      .unwrap();
  ...
  fn test_setup_backend() {
    ...
    vuh
      .setup_backend(&guest_memory, &queues, interrupt.clone())
      .unwrap();
