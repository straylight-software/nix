...
impl VsockPacketTx {
  ...
  pub fn parse(&mut self, mem: &GuestMemoryMmap, chain: DescriptorChain) -> Result<(), VsockError> {
...
mod tests {
  ...
  fn test_packet_buf() {
    ...
    for offset in 0..pkt.buf_size() {
      ...
      pkt
        .read_at_offset_from(&mut data.as_slice(), offset, pkt.buf_size() - offset)
        .unwrap();
      ...
      pkt2
        .write_from_offset_to(&mut buf.as_mut_slice(), offset, pkt.buf_size() - offset)
        .unwrap();
      assert_eq!(
