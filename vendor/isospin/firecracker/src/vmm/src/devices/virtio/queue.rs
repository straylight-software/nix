...
use crate::vstate::memory::{Bitmap, ByteValued, GuestAddress, GuestMemory, GuestMemoryBackend};
...
impl DescriptorChain {
  ...
  pub fn next_descriptor(&self) -> Option<Self> {
    if self.has_next() {
      DescriptorChain::checked_new(self.desc_table_ptr, self.queue_size, self.next).map(|mut c| {
        c.ttl = self.ttl - 1;
        c
      })
    } else {
...
impl Queue {
  ...
  fn get_aligned_slice_ptr<T, M: GuestMemory + GuestMemoryBackend>(
    &self,
    mem: &M,
    addr: GuestAddress,
    len: usize,
  ...
  pub fn initialize<M: GuestMemory + GuestMemoryBackend>(
    &mut self,
    mem: &M,
  ) -> Result<(), QueueError> {
    if !self.ready {
...
mod verification {
  ...
  use crate::vstate::memory::{
    Bytes, FileOffset, GuestAddress, GuestMemory, GuestMemoryBackend, MmapRegion,
  };
  ...
  fn setup_kani_guest_memory() -> ProofGuestMemory {
    ...
    guest_memory(ManuallyDrop::new(kani::vec::exact_vec::<u8, GUEST_MEMORY_SIZE>()).as_mut_ptr())
  }
  ...
  const AVAIL_RING_BASE_ADDRESS: u64 = QUEUE_BASE_ADDRESS + FIRECRACKER_MAX_QUEUE_SIZE as u64 * 16;
