...
use vm_memory::{
  GuestMemory, GuestMemoryBackend, GuestMemoryError, ReadVolatile, VolatileMemoryError,
...
mod tests {
  ...
  fn test_iovec_mut_length() {
    ...
    // SAFETY: This descriptor chain is only loaded once in this test
    let mut iovec = unsafe { IoVecBufferMutDefault::from_descriptor_chain(&mem, head).unwrap() };
    assert_eq!(iovec.len(), 4 * 64);
...
mod verification {
  ...
  impl vm_memory::WriteVolatile for KaniBuffer<'_> {
    fn write_volatile<B: BitmapSlice>(
      ...
      let count = buf.len().min(self.0.len());
      unsafe {
        std::ptr::copy_nonoverlapping(buf.ptr_guard_mut().as_ptr(), self.0.as_mut_ptr(), count);
      }
      self.0 = std::mem::take(&mut self.0).split_at_mut(count).1;
      Ok(count)
    }
