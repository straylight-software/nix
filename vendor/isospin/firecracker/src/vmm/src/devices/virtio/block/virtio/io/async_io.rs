...
use crate::vstate::memory::{
  GuestAddress, GuestMemory, GuestMemoryBackend, GuestMemoryExtension, GuestMemoryMmap,
};
...
impl AsyncFileEngine {
  fn new_ring(file: &File, completion_fd: RawFd) -> Result<IoUring<WrappedRequest>, IoUringError> {
    IoUring::new(
  ...
  pub fn from_file(file: File) -> Result<AsyncFileEngine, AsyncIoError> {
    ...
    let ring = Self::new_ring(&file, completion_evt.as_raw_fd()).map_err(AsyncIoError::IoUring)?;
  ...
  pub fn update_file(&mut self, file: File) -> Result<(), AsyncIoError> {
    let ring =
      Self::new_ring(&file, self.completion_evt.as_raw_fd()).map_err(AsyncIoError::IoUring)?;
  ...
  pub fn pop(
    ...
    let cqe = self.do_pop()?.map(|cqe| {
      let count = cqe.count();
      cqe.map_user_data(|wrapped_user_data| wrapped_user_data.mark_dirty_mem_and_unwrap(mem, count))
    });
