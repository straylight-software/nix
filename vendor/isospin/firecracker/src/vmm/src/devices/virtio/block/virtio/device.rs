...
impl VirtioBlock {
  ...
  pub fn process_queue(&mut self, queue_index: usize) -> Result<(), InvalidAvailIdx> {
    ...
    while let Some(head) = queue.pop_or_enable_notification()? {
      ...
      let processing_result = match Request::parse(&head, &active_state.mem, self.disk.nsectors) {
        Ok(request) => {
          ...
          request.process(&mut self.disk, head.index, &active_state.mem, &self.metrics)
        }
        Err(err) => {
...
mod tests {
  ...
  fn test_disk_backing_file_helper() {
    ...
    for engine in [FileEngineType::Sync, FileEngineType::Async] {
      let disk_properties =
        DiskProperties::new(String::from(f.as_path().to_str().unwrap()), true, engine).unwrap();
  ...
  fn test_invalid_request() {
    for engine in [FileEngineType::Sync, FileEngineType::Async] {
      ...
      mem
        .write_obj::<u32>(VIRTIO_BLK_T_IN, request_type_addr)
        .unwrap();
  ...
  fn test_addr_out_of_bounds() {
    for engine in [FileEngineType::Sync, FileEngineType::Async] {
      ...
      // Write at out of bounds address.
      {
        ...
        mem
          .write_obj::<u32>(VIRTIO_BLK_T_OUT, request_type_addr)
          .unwrap();
  ...
  fn test_request_parse_failures() {
    for engine in [FileEngineType::Sync, FileEngineType::Async] {
...

      {
        ...
        mem
          .write_obj::<RequestHeader>(request_header, request_type_addr)
          .unwrap();
  ...
  fn test_read_write() {
    for engine in [FileEngineType::Sync, FileEngineType::Async] {
      ...
      // Read with error.
      {
        ...
        mem
          .write_obj(10, GuestAddress(request_type_addr.0 + 8))
          .unwrap();
      ...
      // Partial buffer error on read.
      {
        ...
        mem
          .write_obj(5, GuestAddress(request_type_addr.0 + 8))
          .unwrap();
...

      {
        ...
        mem
          .write_obj(1, GuestAddress(request_type_addr.0 + 8))
          .unwrap();
  ...
  fn test_flush() {
    for engine in [FileEngineType::Sync, FileEngineType::Async] {
      ...
      // Flush completes successfully without a data descriptor.
      {
        ...
        mem
          .write_obj::<u32>(VIRTIO_BLK_T_FLUSH, request_type_addr)
          .unwrap();
  ...
  fn test_get_device_id() {
    for engine in [FileEngineType::Sync, FileEngineType::Async] {
      ...
      // Test that the driver receives the correct device id.
      {
        ...
        mem
          .write_obj::<u32>(VIRTIO_BLK_T_GET_ID, request_type_addr)
          .unwrap();
  ...
  fn add_flush_requests_batch(block: &mut VirtioBlock, vq: &VirtQueue, count: u16) {
    ...
    mem
      .write_obj(RequestHeader::new(VIRTIO_BLK_T_FLUSH, 0), hdr_addr)
      .unwrap();
