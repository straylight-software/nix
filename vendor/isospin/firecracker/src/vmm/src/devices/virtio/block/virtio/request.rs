...
mod tests {
  ...
  use crate::vstate::memory::{Address, GuestAddress, GuestMemory, GuestMemoryBackend};
  ...
  fn test_read_request_header() {
    ...
    for request_type in supported_request_types {
      ...
      mem
        .write_obj::<RequestHeader>(expected_header, addr)
        .unwrap();
  ...
  impl RequestDescriptorChain<'_, '_> {
    ...
    fn check_parse(&self, check_data: bool) {
      let mut q = self.driver_queue.create_queue();
      let memory = self.driver_queue.memory();
      let request = Request::parse(&q.pop().unwrap().unwrap(), memory, NUM_DISK_SECTORS).unwrap();
