  ...
  S: VsockConnectionBackend + Debug,
{
  ...
  fn recv_pkt(&mut self, pkt: &mut VsockPacketRx) -> Result<(), VsockError> {
    ...
    if self.pending_rx.remove(PendingRx::Request) {
      self.expiry = Some(Instant::now() + Duration::from_millis(defs::CONN_REQUEST_TIMEOUT_MS));
    ...
    if evset.contains(EventSet::OUT) {
      ...
      let flushed = self
        ...
        .unwrap_or_else(|err| {
          ...
          match err {
            VsockCsmError::TxBufFlush(inner) if inner.kind() == ErrorKind::WouldBlock => {
...
mod tests {
  ...
  impl ReadVolatile for TestStream {
    fn read_volatile<B: BitmapSlice>(
      ...
      match self.read_state {
        ...
        StreamState::Error(kind) => Err(vm_memory::VolatileMemoryError::IOError(IoError::new(
          kind, "whatevs",
        ))),
        StreamState::Ready => {
        ...
        StreamState::WouldBlock => Err(vm_memory::VolatileMemoryError::IOError(IoError::new(
          ErrorKind::WouldBlock,
          "EAGAIN",
        ))),
      }
  ...
  impl Write for TestStream {
    fn write(&mut self, data: &[u8]) -> Result<usize, IoError> {
      self
        .write_volatile(&VolatileSlice::from(data.to_vec().as_mut_slice()))
  ...
  fn test_credit_update_to_peer() {
    ...
    let initial_fwd_cnt = csm_defs::CONN_TX_BUF_SIZE - csm_defs::CONN_CREDIT_UPDATE_THRESHOLD - 6;
