    ...
    vec![
      ...
      &aml::Name::new(
        ...
        &aml::ResourceTemplate::new(vec![
          &aml::Memory32Fixed::new(true, addr.try_into().unwrap(), len.try_into().unwrap()),
...
pub(crate) mod tests {
  ...
  fn test_register_virtio_device() {
    ...
    device_manager
      .register_virtio_test_device(&vm, vm.guest_memory().clone(), dummy, &mut cmdline, "dummy")
      .unwrap();
  ...
  fn test_register_too_many_devices() {
    ...
    assert_eq!(
      ...
      "Failed to allocate requested resource: The requested resource is not available.".to_string()
    );
  }
  ...
  fn test_device_info() {
    ...
    let mut count = 0;
    let _: Result<(), MmioError> = device_manager.for_each_virtio_device(|devtype, devid, _| {
