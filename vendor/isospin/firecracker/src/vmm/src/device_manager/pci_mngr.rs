...
use crate::pci::vfio::{
  GuestPhysAddr, HostVirtAddr, PciBdf, VfioContainer, VfioError, VfioPciDevice, open_vfio_device,
};
...
impl std::fmt::Debug for PciDevices {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    f.debug_struct("PciDevices")
      ...
      .field(
        "vfio_devices",
        &self.vfio_devices.keys().collect::<Vec<_>>(),
      )
...
impl PciDevices {
  ...
  pub(crate) fn attach_pci_virtio_device<T: 'static + VirtioDevice + MutEventSubscriber + Debug>(
    &mut self,
    vm: &Arc<Vm>,
    id: String,
    device: Arc<Mutex<T>>,
    ...
    let msix_num = u16::try_from(device.lock().expect("Poisoned lock").queues().len() + 1).unwrap();
  ...
  pub fn attach_vfio_device(
    ...
    let bdf = PciBdf::parse(pci_address)
      .ok_or_else(|| VfioError::InvalidPciAddress(pci_address.to_string()))?;
    ...
    for (bar_index, size, is_64bit, is_io) in bar_infos {
      ...
      let guest_addr = if is_64bit {
        ...
        log::info!(
          "Allocating 64-bit BAR{} size {:#x} in mmio64",
          bar_index.as_u8(),
          size
        );
        ...
        log::info!(
          "Allocating 32-bit BAR{} size {:#x} in mmio32",
          bar_index.as_u8(),
          size
        );
        let alloc_range = resource_allocator
    ...
    let pci_device_bdf = pci_segment.next_device_bdf()?;
    debug!(
      "Registering VFIO device on PCI bus at BDF {:?}",
      pci_device_bdf
    );
    ...
    // need the emulation fallback.
    {
      ...
      for bar_info in device_locked.bars() {
        if let Some(guest_addr) = bar_info.guest_addr {
          ...
          vm.common
            .mmio_bus
            .insert(device.clone(), guest_addr.raw(), bar_info.size)?;
        }
      }
    }
...
mod tests {
  ...
  fn test_device_manager_persistence() {
    ...
    // Set up a vmm with one of each device, and get the serialized DeviceStates.
    {
      ...
      _pmem_files = insert_pmem_devices(&mut vmm, &mut cmdline, &mut event_manager, pmem_configs);
      ...
      Snapshot::new(SNAPSHOT_VERSION.clone(), vmm.device_manager.save())
    ...
    let device_manager_state: device_manager::DevicesState =
      Snapshot::load_without_crc_check(buf.as_slice(), &SNAPSHOT_VERSION)
        .unwrap()
        .data;
