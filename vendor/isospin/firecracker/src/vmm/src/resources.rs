...
pub struct VmmConfig {
  ...
  vfio_devices: Vec<crate::vmm_config::vfio::VfioDeviceConfig>,
  #[serde(skip)]
...
pub struct VmResources {
  ...
  /// VFIO device configurations.
  pub vfio_devices: Vec<crate::vmm_config::vfio::VfioDeviceConfig>,
...
impl VmResources {
  ...
  pub fn from_json(
    ...
    if let Some(either) = vmm_config.cpu_config {
      match either {
        CustomCpuTemplateOrPath::Path(path) => {
          let cpu_config_json = std::fs::read_to_string(path).map_err(ResourcesError::File)?;
          let cpu_template = CustomCpuTemplate::try_from(cpu_config_json.as_str())?;
          resources.set_custom_cpu_template(cpu_template);
        }
        CustomCpuTemplateOrPath::Template(template) => resources.set_custom_cpu_template(template),
      }
    }
    ...
    // Note: pci_enabled for VFIO is handled in main.rs to avoid being overwritten
  ...
  pub fn allocate_guest_memory(&self) -> Result<Vec<GuestRegionMmap>, MemoryError> {
    let regions = crate::arch::arch_memory_regions(mib_to_bytes(self.machine_config.mem_size_mib));
...
mod tests {
  ...
  fn test_from_json() {
    ...
    let error = VmResources::from_json(r#"}"#, &default_instance_info, HTTP_MAX_PAYLOAD_SIZE, None)
      .unwrap_err();
    assert!(
    ...
    assert!(
      matches!(
        error,
        ResourcesError::BlockDevice(DriveError::CreateBlockDevice(BlockError::VirtioBackend(
