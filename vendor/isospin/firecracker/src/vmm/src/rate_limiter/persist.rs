...
impl Persist<'_> for TokenBucket {
  ...
  fn restore(_: Self::ConstructorArgs, state: &Self::State) -> Result<Self, Self::Error> {
    ...
    let mut token_bucket = TokenBucket::new(state.size, state.one_time_burst, state.refill_time)
...
mod tests {
  ...
  use vmm_snapshot::Snapshot;

  use super::*;

  /// Test version constant for rate_limiter persist tests
  const TEST_VERSION: semver::Version = semver::Version::new(8, 0, 0);
  ...
  fn test_token_bucket_persistence() {
    ...
    Snapshot::new(TEST_VERSION.clone(), tb.save())
      .save(&mut mem.as_mut_slice())
      .unwrap();

    let restored_tb = TokenBucket::restore(
      (),
      &Snapshot::load_without_crc_check(mem.as_slice(), &TEST_VERSION)
        .unwrap()
        .data,
    )
    .unwrap();
  ...
  fn test_rate_limiter_persistence() {
    ...
    // Test serialization.
    let mut mem = vec![0; 4096];
    Snapshot::new(TEST_VERSION.clone(), rate_limiter.save())
      .save(&mut mem.as_mut_slice())
      .unwrap();
