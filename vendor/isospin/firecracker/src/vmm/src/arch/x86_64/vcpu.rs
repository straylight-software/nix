...
mod tests {
  ...
  fn create_vcpu_config(
    ...
    let cpuid =
      Cpuid::try_from(kvm.supported_cpuid.clone()).map_err(GuestConfigError::CpuidFromKvmCpuid)?;
    let msrs = vcpu
  ...
  fn test_get_msrs_with_msrs_to_dump() {
    ...
    vcpu
      .get_msrs(msrs_to_dump.as_slice().iter().copied())
      .unwrap();
  }
