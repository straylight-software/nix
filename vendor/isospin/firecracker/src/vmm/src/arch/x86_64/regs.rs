...
mod tests {
  ...
  fn validate_segments_and_sregs(gm: &GuestMemoryMmap, sregs: &kvm_sregs, boot_prot: BootProtocol) {
    if let BootProtocol::LinuxBoot = boot_prot {
  ...
  fn test_write_gdt_table() {
    ...
    let gm = single_region_mem(BOOT_GDT_OFFSET as usize + (mem::size_of::<u64>() * BOOT_GDT_MAX));
