...
use crate::vstate::memory::{
  Address, GuestAddress, GuestMemory, GuestMemoryBackend, GuestMemoryMmap, GuestMemoryRegion,
  GuestRegionType,
};
    ...
    assert!(
      regions
        ...
        .all(|&(start, len)| (start.0 >= FIRST_ADDR_PAST_32BITS
          ...
          && (start.0 >= FIRST_ADDR_PAST_64BITS_MMIO || start.0 + len as u64 <= MMIO64_MEM_START))
    );
