...
impl CacheEntry {
  fn from_index(index: u8, store: &dyn CacheStore) -> Result<CacheEntry, CacheInfoError> {
    ...
    cache.level = level_str
      .parse::<u8>()
      .map_err(|err| CacheInfoError::InvalidCacheAttr("level".to_string(), err.to_string()))?;
...
fn to_bytes(cache_size_pretty: &mut String) -> Result<u32, CacheInfoError> {
  match cache_size_pretty.pop() {
    Some('K') => Ok(
      cache_size_pretty
        .parse::<u32>()
        .map_err(|err| CacheInfoError::InvalidCacheAttr("size".to_string(), err.to_string()))?
        * 1024,
    ),
    Some('M') => Ok(
      cache_size_pretty
...
mod tests {
  ...
  use super::*;
  use crate::arch::aarch64::cache_info::{CacheEngine, CacheEntry, CacheStore, read_cache_config};
  ...
  fn test_to_bytes() {
    ...
    assert!(
      res.is_err()
        && format!("{}", res.unwrap_err()) == "Invalid cache configuration found for size: 64G"
    );
