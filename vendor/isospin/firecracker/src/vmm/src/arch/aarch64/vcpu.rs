...
impl KvmVcpu {
  ...
  pub fn configure(
    ...
    for reg in vcpu_config.cpu_config.regs.iter() {
      self.fd.set_one_reg(reg.id, reg.as_slice()).map_err(|err| {
        KvmVcpuError::ApplyCpuTemplate(VcpuArchError::SetOneReg(reg.id, reg.value_str(), err))
      })?;
    }
  ...
  pub fn save_state(&self) -> Result<VcpuState, KvmVcpuError> {
    ...
    self
      .get_all_registers(&mut state.regs)
  ...
  pub fn setup_boot_regs(
    ...
    self
      ...
      .map_err(|err| VcpuArchError::SetOneReg(id, format!("{PSTATE_FAULT_BITS_64:#x}"), err))?;
...
mod tests {
  ...
  fn test_setup_regs() {
    ...
    vcpu
      .setup_boot_regs(0x0, &mem, &optional_capabilities)
      .unwrap();
