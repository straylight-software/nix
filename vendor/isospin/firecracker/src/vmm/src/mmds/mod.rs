...
fn respond_to_put_request(mmds: &mut Mmds, request: Request) -> Response {
  ...
  if let Some((header, _)) = get_header_value_pair(custom_headers, &[X_FORWARDED_FOR_HEADER]) {
    let error_msg =
      RequestError::HeaderError(HttpHeaderError::UnsupportedName(header.to_string())).to_string();
    return build_response(
      request.http_version(),
      StatusCode::BadRequest,
      MediaType::PlainText,
    ...
    Some((header, value)) => match value.parse::<u32>() {
      ...
      Err(_) => {
        return build_response(
          ...
          MediaType::PlainText,
          Body::new(
            RequestError::HeaderError(HttpHeaderError::InvalidValue(header.into(), value.into()))
              .to_string(),
          ),
        );
      }
...
mod tests {
  ...
  fn test_respond_to_request_mmdsv2() {
    ...
    let request = Request::try_from(b"GET http://169.254.169.254/ HTTP/1.0\r\n\r\n", None).unwrap();
