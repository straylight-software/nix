      ...
      if ip.protocol() == PROTOCOL_TCP {
        ...
        match &mut self.tcp_handler.receive_packet(&ip, move |request| {
          ...
          Ok(event) => {
            ...
            match event {
              RecvEvent::NewConnectionSuccessful => METRICS.mmds.connections_created.inc(),
...
mod tests {
  ...
  impl MmdsNetworkStack {
    ...
    fn write_incoming_tcp_segment(&self, buf: &mut [u8], addr: Ipv4Addr, flags: TcpFlags) -> usize {
  ...
  fn test_ns() {
    let mut ns = MmdsNetworkStack::new_with_defaults(None, Arc::new(Mutex::new(Mmds::default())));
  ...
  fn test_break_speculative_check_detour_arp() {
    ...
    let mut eth = EthernetFrame::write_incomplete(buf.as_mut(), mac, mac, ETHERTYPE_ARP).unwrap();
  ...
  fn test_break_speculative_check_detour_ipv4() {
    ...
    let other_ip = Ipv4Addr::new(5, 6, 7, 8);
    let mac = MacAddr::from_bytes_unchecked(&[0; 6]);
    let ns = MmdsNetworkStack::new_with_defaults(Some(ip), Arc::new(Mutex::new(Mmds::default())));

    let mut eth = EthernetFrame::write_incomplete(buf.as_mut(), mac, mac, ETHERTYPE_IPV4).unwrap();
