...
use crate::generated::io_uring_register_op;
...
impl<T: Debug> IoUring<T> {
  ...
  pub fn new(
    ...
    let slab = slab::Slab::with_capacity(params.sq_entries as usize + params.cq_entries as usize);
...
mod tests {
  ...
  use vm_memory::{Bytes, MmapRegion};
  ...
  fn proptest_read_write_correctness() {
    ...
    runner
      .run(
        ...
        |set| {
          let mut ring = IoUring::new(RING_SIZE, vec![&file_async], vec![], None).unwrap();
          ...
          for mut operation in set {
            ...
            let count = match operation.opcode {
              OpCode::Write => u32::try_from(
                SyscallReturnCode(unsafe {
                  libc::pwrite(
                    file_sync.as_raw_fd(),
                    write_mem_region.as_ptr().add(operation.addr.unwrap()) as *const libc::c_void,
            ...
            match operation.opcode {
              OpCode::Write => {
                operation.addr =
                  Some(unsafe { write_mem_region.as_ptr().add(operation.addr.unwrap()) as usize })
              }
              OpCode::Read => {
                operation.addr = Some(unsafe {
                  async_read_mem_region.as_ptr().add(operation.addr.unwrap()) as usize
                })
              }
              _ => unreachable!(),
            };
