...
fn build_microvm_from_json(
    ...
    // Enable PCI if explicitly requested OR if VFIO devices are configured
    // (VFIO passthrough requires PCI transport)
    vm_resources.pci_enabled = pci_enabled || !vm_resources.vfio_devices.is_empty();
