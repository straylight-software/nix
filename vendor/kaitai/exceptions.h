...
public:
  validation_failed_error(const std::string what, kstream* io, const std::string src_path)
      : kstruct_error("at pos " + kstream::to_string(io->pos()) + ": validation failed: " + what,
                      src_path),
        m_io(io) {}
...
public:
  validation_not_equal_error(const T& expected, const T& actual, kstream* io,
...
public:
  validation_less_than_error(const T& min, const T& actual, kstream* io, const std::string src_path)
      : validation_failed_error("not in range", io, src_path), m_min(min), m_actual(actual) {}
...
public:
  validation_greater_than_error(const T& max, const T& actual, kstream* io,
                                const std::string src_path)
      : validation_failed_error("not in range", io, src_path), m_max(max), m_actual(actual) {}
...
public:
  validation_not_any_of_error(const T& actual, kstream* io, const std::string src_path)
      : validation_failed_error("not any of the list", io, src_path), m_actual(actual) {}
...
public:
  validation_not_in_enum_error(const T& actual, kstream* io, const std::string src_path)
      : validation_failed_error("not in the enum", io, src_path), m_actual(actual) {}
...
public:
  validation_expr_error(const T& actual, kstream* io, const std::string src_path)
      : validation_failed_error("not matching the expression", io, src_path), m_actual(actual) {}
