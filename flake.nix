      ...
      perSystem =
        ...
        in
        {
          ...
          packages = {
            ...
            # ── straylight-nix-embedded binary ─────────────────────────────────────
            # Statically linked nix binary with embedded Firecracker VMM.
            # The VMM is linked as a library (via vmm_ffi), and the kernel/initrd
            # are embedded in the DATA section as zstd-compressed blobs.
            nix-embedded = pkgs.stdenvNoCC.mkDerivation {
              name = "nix-embedded";
              src = ./.;
              ...
              buildPhase = ''
                                ...
                                # Link isospin Rust vendor
                                rm -f vendor/isospin/third-party/rust/vendor
                                ln -s ${isospinRustVendor} vendor/isospin/third-party/rust/vendor
                                ...
                                # Set AWS_LC_LIB_DIR for aws-lc-sys
                                export AWS_LC_LIB_DIR="${awsLcSys}/lib"

                                # Generate buckconfig.local with full toolchain config
                                # Includes glibc paths for Rust proc-macro linking
                ...
                glibc_lib = ${toolchain.buck2.glibc-lib}
                glibc_gcc_lib = ${toolchain.buck2.glibc-gcc-lib}
                ...
                BUCKCONFIG
                                ...
                                # Build the embedded nix CLI binary
                                buck2 build //src/nix/cli:nix-embedded --show-full-output > $TMPDIR/buck2-output.txt 2>&1 || {
                                ...
                                CLI_PATH=$(grep "nix-embedded" $TMPDIR/buck2-output.txt | tail -1 | awk '{print $2}')
              ...
              meta = {
                description = "Nix package manager with embedded Firecracker VMM";
                homepage = "https://github.com/straylight-software/nix";
